...
**2026-09-20 — B2/C2 : le ✅ ci-dessus était partiel.**
- **B2 ⚠️→✅** Le balayage de `DatabaseConstraintGuardTest` n'énumère que les tables portant une colonne `uuid`. La clé d'idempotence de `treasury_transactions` est `(source_type, source_id)` : elle échappait donc au garde-fou, et ne portait qu'un index SIMPLE. Comblé par `2026_09_20_000001_unique_index_on_treasury_posting_source` (dédoublonnage + recrédit du solde, puis index UNIQUE), et surveillé par `TreasuryPostsOnceUnderConcurrencyTest`.
- **C2 ✅** Double validation d'une dépense prouvée par drill deux processus : 600 000 GNF sortis pour 300 000 (2 essais sur 5). Correctif `verrou → relecture verrouillante → contrôle → écriture` dans `ApproveExpense` **et** `PayrollController::markPaid` (même faille), plus l'index UNIQUE en dernière ligne. Contre-preuve 10/10. Détail : `docs/audit/drills-concurrence.md` §C2.
- **Enseignement** : un garde-fou qui dérive sa liste du schéma ne vaut que par son critère de sélection. Celui-ci cherchait les `uuid` ; il ne pouvait pas voir une clé d'idempotence qui n'en est pas un.
...
- **Clarification fiche lot (question terrain « le coût eau ne bouge pas après un pointage ») — comportement conçu, affichage clarifié** : le pointage enregistre des LITRES (→ niveau citerne), le coût imputé vient des RELEVÉS facturés taggés bâtiment (→ marge). La carte « Eau & Énergie » affiche désormais DEUX lignes jamais additionnées : « Facturé (relevés) » (comptable, entre dans la marge) et « Eau bue (pointages) » (litres + estimation au prix m³, bouge à chaque pointage — informatif, l'additionner au facturé compterait le même m³ deux fois). Test : `UtilityCostAttributionTest` +1.
...
**2026-09-24 — Grille §2.4 (sécurité) et B1 : les points restés « ❓ » balayés sur le code :**
- **S1 injection SQL ✅** : 81 appels SQL bruts, aucun n'interpole une variable dans la chaîne — ni en ligne, ni sur plusieurs lignes, ni via une chaîne préconstruite.
- **S2 XSS ✅** : 5 sorties non échappées dans toute l'application, toutes sûres — constantes (`batches/show`), helper qui échappe chaque ligne (`notif_html`), SVG de code-barres nettoyé (Code128 échappé, Code39 et EAN-13 restreints à leur jeu de caractères).
- **S3 CSRF ✅** : aucune vue ne porte plus de formulaires POST que de jetons.
- **S6 affectation de masse ✅** : aucun `$request->all()` ni `except()` vers une écriture ; aucun champ financier dérivé (payé, statut de paiement, total, solde) n'est accepté par une validation.
- **S7 données sensibles ✅** : aucun journal ne porte mot de passe, jeton, secret ni en-tête `Authorization`. **Durci** : l'alerte d'erreur WhatsApp ne transporte plus l'URL complète (jeton de réinitialisation dans le chemin, signature en paramètre).
- **S8 téléversements ✅** : aucun fichier stocké sous un nom ou une extension fournis par le client.
- **S9 authentification** : limitation des connexions ✅ web ET API. **Corrigé** : le formulaire « mot de passe oublié » servait d'annuaire (réponse différente selon que le compte existe, route sans plafond) — réponse unique et `throttle:6,1`.
- **S10 débogage ✅/corrigé** : aucun outil de débogage installé, `app.debug` coupé par défaut ; mais `.env.example` est livré en débogage et rien ne le détectait hors de l'assistant → `avismart:diagnostic` le signale désormais BLOQUANT sur une installation en service.
- **B1 intégrité référentielle** (mesuré sur MySQL, moteur de production) : **37** colonnes `*_id` sans clé étrangère sur 301. **29 par conception** : liaisons polymorphes, codes métier (`client_id`, `employee_id`, `provider_id`, `sensor_id`), table du framework, zone tampon de télémétrie, `farm_id` ajoutés après coup. **8 références** dont la cible reste protégée : `sale_items.product_id` / `dispatch_items.product_id` pointent vers **`stocks`** (et non `products` — nom trompeur, à ne pas confondre), dont la suppression est refusée dès qu'il y a un historique ; les lots avec activité sont retenus par leurs pointages. Résidu étroit, documenté : lot vendu sans pointage (`sale_items.batch_id`), bâtiment d'affectation d'un employé, auteur d'un ajustement de produit fini.
