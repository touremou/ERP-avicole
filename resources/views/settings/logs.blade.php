<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4 text-left">
            <a href="{{ route('settings.index') }}" class="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center text-slate-400 hover:bg-slate-900 hover:text-white transition-all no-underline"><i class="fa-solid fa-arrow-left"></i></a>
            <div>
                <h2 class="text-lg font-black text-slate-800 uppercase italic tracking-tighter leading-none">Journal Système</h2>
                <p class="text-[9px] font-bold text-slate-400 uppercase mt-1 tracking-widest italic">Historique des modifications de paramètres</p>
            </div>
        </div>
    </x-slot>
            ...
            <form method="GET" action="{{ route('settings.logs') }}" class="mb-6 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
                <div class="flex flex-wrap items-end gap-3">
                    <div class="flex-1 min-w-[120px]">
                        <label class="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-1">Module</label>
                    ...
                    <div class="flex-1 min-w-[120px]">
                        <label class="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-1">Utilisateur</label>
                    ...
                    <div class="min-w-[130px]">
                        <label class="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-1">Du</label>
                    ...
                    <div class="min-w-[130px]">
                        <label class="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-1">Au</label>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden text-left">
                <div class="px-6 py-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                    <h3 class="text-[9px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                        <i class="fa-solid fa-clock-rotate-left text-blue-500"></i> {{ $audits->total() }} entrée(s)
                    </h3>
                </div>
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="text-[7px] font-black text-slate-400 uppercase tracking-widest bg-slate-50/50 border-b border-slate-100">
                            ...
                            <th class="px-3 py-3 text-left">Paramètre</th>
                            <th class="px-3 py-3 text-center">Avant</th>
                            <th class="px-3 py-3 text-center">Après</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-all">
                            ...
                            <td class="px-3 py-3 text-center">
                                ...
                                @else
                                    <span class="text-[9px] text-slate-300 italic">vide</span>
                                @endif
                            </td>
                            <td class="px-3 py-3 text-center">
                                @if($a->new_value === '***')
                        ...
                        @empty
                        <tr><td colspan="5" class="px-8 py-12 text-center text-slate-300 text-[9px] uppercase italic tracking-widest">Aucune modification enregistrée</td></tr>
                        @endforelse
                    </tbody>
                </table>
