    ...
    <x-slot name="header">
        <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 w-full text-left">
            <div class="flex items-center gap-5">
                ...
                <div>
                    <h2 class="font-black text-2xl text-slate-800 leading-none uppercase italic tracking-tighter">Registre des Dépenses</h2>
            ...
            <div class="flex flex-wrap gap-3 w-full sm:w-auto">
                <div class="bg-white px-6 py-4 rounded-[1.5rem] border border-slate-100 text-right shadow-sm">
                    <p class="text-[8px] font-black text-emerald-400 uppercase italic mb-1">Validées (période)</p>
                ...
                <div class="bg-white px-6 py-4 rounded-[1.5rem] border border-slate-100 text-right shadow-sm">
                    <p class="text-[8px] font-black text-amber-400 uppercase italic mb-1">En attente ({{ $stats['count_attente'] }})</p>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 italic font-bold text-left">
            ...
            <form method="GET" class="mb-8 flex flex-wrap gap-3 items-center">
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="RECHERCHER..." class="bg-white border border-slate-100 rounded-2xl pl-10 pr-4 py-3 text-[10px] font-black uppercase tracking-widest shadow-sm w-48 outline-none">
                </div>
                ...
                <select name="status" class="bg-white border border-slate-100 rounded-2xl px-4 py-3 text-[10px] font-black uppercase tracking-widest shadow-sm outline-none appearance-none">
                    ...
                    <option value="valide" {{ request('status') === 'valide' ? 'selected' : '' }}>Validées</option>
                    <option value="annule" {{ request('status') === 'annule' ? 'selected' : '' }}>Annulées</option>
                </select>
                    ...
                    <table class="w-full border-collapse">
                        <thead>
                            <tr class="bg-slate-50 text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 italic">
                                <th class="px-8 py-6 text-left">Dépense</th>
                                <th class="px-6 py-6 text-left">Catégorie</th>
                                <th class="px-6 py-6 text-left">Date</th>
                                <th class="px-6 py-6 text-left">Lot</th>
                                <th class="px-6 py-6 text-right">Montant</th>
                        ...
                        <tbody class="divide-y divide-slate-50">
                                ...
                                <td class="px-8 py-5 text-right whitespace-nowrap">
                                    @can('depenses.M')
                                        ...
                                        <form method="POST" action="{{ route('expenses.approve', $expense) }}" class="inline">
                                            @csrf @method('PUT')
                                            <button type="submit" class="text-emerald-500 hover:text-emerald-700 mr-3 border-none bg-transparent cursor-pointer" title="Valider">
                                                <i class="fa-solid fa-check-double"></i>
                                            </button>
                                        </form>
                                        <a href="{{ route('expenses.edit', $expense) }}" class="text-blue-400 hover:text-blue-600 mr-3 no-underline" title="Modifier">
                                            <i class="fa-solid fa-pen"></i>
                                        </a>
                                        @endif
                                    @endcan
                                    <a href="{{ route('expenses.show', $expense) }}" class="text-slate-400 hover:text-slate-700 no-underline" title="Détails">
                            ...
                            <tr>
                                <td colspan="7" class="px-8 py-16 text-center">
                                    ...
                                    <p class="text-[10px] text-slate-400 uppercase tracking-widest font-black">Aucune dépense sur cette période</p>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
