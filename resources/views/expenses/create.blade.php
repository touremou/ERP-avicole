<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-5 text-left">
            ...
            <div>
                <h2 class="font-black text-2xl text-slate-800 leading-none uppercase italic tracking-tighter">Nouvelle Dépense</h2>
                <p class="text-[10px] font-black text-rose-600 uppercase tracking-[0.2em] mt-2 italic">Enregistrement d'une charge ponctuelle</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-10">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 italic font-bold text-left">
            ...
            <div id="offline-banner" class="hidden mb-6 p-4 bg-amber-100 text-amber-800 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-amber-200">
                <i class="fa-solid fa-wifi mr-2"></i> Mode hors-ligne : la dépense sera enregistrée localement puis synchronisée au retour du réseau.
            </div>
            ...
            <form id="expense-form" method="POST" action="{{ route('expenses.store') }}">
                ...
                <div class="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm mb-6">
                    <h3 class="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-6 flex items-center gap-2">
                        <i class="fa-solid fa-receipt text-rose-500"></i> Détail de la dépense
                    </h3>

                    <div class="space-y-6">
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Libellé *</label>
                            <input type="text" name="label" value="{{ old('label') }}" required placeholder="Ex: Carburant moto livraison"
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Montant ({{ setting('general.currency', 'GNF') }}) *</label>
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Mode de paiement *</label>
                ...
                <div class="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm mb-6">
                    <h3 class="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-6 flex items-center gap-2">
                        <i class="fa-solid fa-link text-blue-500"></i> Rattachement (optionnel)
                    </h3>

                    <div class="grid grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Lot concerné</label>
                            <select name="batch_id" class="w-full bg-slate-50 border-none rounded-2xl p-4 text-xs font-black uppercase shadow-inner outline-none appearance-none cursor-pointer">
                                <option value="">— Charge générale (ferme) —</option>
                                @foreach($batches as $batch)
                                    <option value="{{ $batch->id }}" {{ (string) old('batch_id') === (string) $batch->id ? 'selected' : '' }}>{{ $batch->code }}</option>
                                @endforeach
                            </select>
                            <p class="text-[8px] text-slate-400 ml-2 italic">Rattacher à un lot impacte directement sa marge nette.</p>
                        </div>
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Bénéficiaire / Fournisseur</label>
                            <input type="text" name="supplier_name" value="{{ old('supplier_name') }}" placeholder="Nom (facultatif)"
                    ...
                    <div class="space-y-2 mt-6">
                        ...
                        <textarea name="notes" rows="2" placeholder="Justificatif, référence reçu, contexte..."
                ...
                <button type="submit" class="w-full bg-rose-500 text-white py-6 rounded-[2rem] font-black text-xs uppercase tracking-[0.3em] hover:bg-rose-600 transition-all shadow-2xl italic border-none cursor-pointer">
                    <i class="fa-solid fa-floppy-disk mr-2"></i> Enregistrer la Dépense
                </button>
            </form>
        </div>
    </div>
    ...
    <script>
        (function () {
            ...
            form.addEventListener('submit', async function (e) {
                ...
                if (!label || amount <= 0) {
                    alert("Veuillez renseigner un libellé et un montant valide.");
                    return;
                }
                ...
                try {
                    ...
                    alert("📴 Hors-ligne : dépense enregistrée localement. Elle sera synchronisée (en attente de validation) au retour du réseau.");
                    window.location.href = "{{ route('expenses.index') }}";
                } catch (err) {
                    console.error("Échec de l'enregistrement hors-ligne de la dépense :", err);
