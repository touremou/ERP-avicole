    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 text-left">
            ...
            <div class="flex flex-wrap items-center gap-3">
                {{-- Filtre Statut (L) --}}
                <div class="relative">
                    {{-- REMPLACER le select statut par ce bloc (même style que health_finance) --}}
                    <div class="bg-slate-100 p-1.5 rounded-xl flex gap-1">
                        @foreach(['all' => 'Tous', 'actif' => '🟢 Actifs', 'termine' => '🔵 Terminés'] as $key => $label)
                            <a href="{{ request()->fullUrlWithQuery(['status' => $key]) }}"
                            @class([
                                'px-4 py-2 rounded-lg text-[9px] font-black uppercase italic transition-all no-underline',
                            ...
                            <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-4">
                                ...
                                <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-xl text-right">
                                    ...
                                    <p class="text-3xl font-black text-slate-900 tracking-tighter italic">
                                        {{ number_format(collect($monthlyData[$num])->sum('health') + collect($monthlyData[$num])->sum('feed_cost'), 0, ',', ' ') }} <small class="text-xs uppercase opacity-40">{{ $currency }}</small>
                                    </p>
                                </div>
                            </div>
                                        ...
                                        <div class="p-10">
                                            ...
                                            <div class="space-y-4">
                                                ...
                                                <div class="flex justify-between items-center bg-slate-50 p-5 rounded-[1.5rem] border border-slate-100 group-hover:bg-white transition-colors">
                                                    ...
                                                    <span class="text-base font-black text-rose-600 italic">{{ number_format($data['health'] ?? 0, 0, ',', ' ') }} <small class="text-[8px] opacity-60">{{ $currency }}</small></span>
                                                </div>
                                                ...
                                                <div class="bg-slate-50 p-5 rounded-[1.5rem] border border-slate-100 group-hover:bg-white transition-colors space-y-3">
                                                    <div class="flex justify-between items-center">
                                                        ...
                                                        <span class="text-base font-black text-orange-600 italic">{{ number_format($data['feed_cost'] ?? 0, 0, ',', ' ') }} <small class="text-[8px] opacity-60">{{ $currency }}</small></span>
                                                    </div>
                                            ...
                                            <div class="mt-10 pt-8 border-t-2 border-dashed border-slate-100 flex justify-between items-end">
                                                ...
                                                <p class="text-2xl font-black text-slate-900 tracking-tighter italic">
                                                    {{ number_format(($data['health'] ?? 0) + ($data['feed_cost'] ?? 0), 0, ',', ' ') }} <small class="text-[10px] opacity-40">{{ $currency }}</small>
                                                </p>
                                            </div>
                                        </div>
