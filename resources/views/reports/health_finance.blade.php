        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-10">
            ...
            <div class="flex flex-col md:flex-row justify-center items-center gap-6">
                <div class="bg-slate-100 p-2 rounded-[2.5rem] flex gap-2 shadow-inner border border-slate-200/50">
                    @foreach(['all' => __("Historique global"), 'year' => __("Exercice").' '.date('Y'), 'month' => __("Mois en cours")] as $key => $label)
                ...
                <div class="bg-blue-50 p-2 rounded-[2.5rem] flex gap-2 shadow-inner border border-blue-100">
                    @foreach(['all' => __("Tous les lots"), 'actif' => __("En cours"), 'clos' => __("Archives")] as $key => $label)
                ...
                <a href="{{ route('reports.health_finance.pdf', ['period' => $period, 'status' => $statusFilter]) }}" class="px-8 py-3 bg-slate-900 text-white rounded-[1.5rem] text-[10px] font-black uppercase italic no-underline hover:bg-blue-600 transition-all shadow-lg">
                    <i class="fa-solid fa-file-pdf mr-1"></i> {{ __("Export PDF") }}
                </a>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 text-left">
                ...
                <div class="bg-slate-900 p-10 rounded-[3.5rem] text-white shadow-2xl relative overflow-hidden group border-b-8 border-blue-600">
                    <div class="relative z-10">
                        <p class="text-[11px] uppercase text-blue-400 font-black mb-5 tracking-[0.3em] italic">{{ __("Dépense Sanitaire Totale") }}</p>
                        ...
                        <div class="flex items-center gap-8 border-t border-white/10 pt-8 italic">
                            <div>
                                <p class="text-[9px] uppercase opacity-40 mb-2 font-black tracking-widest leading-none">{{ __("Coût Moyen / Tête") }}</p>
                ...
                <div class="lg:col-span-2 bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-xl flex flex-col md:flex-row items-center gap-12 relative overflow-hidden">
                            ...
                            @foreach(['Vaccin', 'Traitement', 'Vitamine', 'Désinfection'] as $type)
                                ...
                                <div class="group/bar space-y-2">
                                    <div class="flex justify-between text-[10px] font-black uppercase italic tracking-tighter">
                                        <span class="text-slate-500 group-hover/bar:text-slate-900 transition-colors">{{ __($type) }}</span>
                    ...
                    <div class="w-full md:w-56 bg-emerald-600 rounded-[3rem] p-8 text-center border-b-8 border-emerald-800 shadow-2xl shadow-emerald-200 group transform hover:rotate-2 transition-all">
                        <p class="text-[10px] uppercase text-emerald-100 mb-4 font-black tracking-widest italic leading-none">{{ __("Batch d'Excellence") }}</p>
                        ...
                        <p class="text-[12px] font-black text-emerald-100 italic leading-none">{{ number_format($bestBatchCost, 0) }} <small class="font-black">{{ $currency }}/{{ __("tête") }}</small></p>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-900 border-b border-slate-800 text-[10px] font-black uppercase text-slate-400 italic">
                            <th class="px-12 py-8 tracking-[0.2em]">{{ __("Identifiant Lot") }}</th>
                            <th class="px-8 py-8 text-center tracking-[0.2em]">{{ __("Effectif Initial") }}</th>
                            <th class="px-8 py-8 text-right tracking-[0.2em]">{{ __("Total Investi") }}</th>
                            <th class="px-12 py-8 text-right tracking-[0.2em]">{{ __("Performance") }} ({{ $currency }}/{{ __("Tête") }})</th>
                        </tr>
                    </thead>
                        ...
                        @foreach($batches as $batch)
                            ...
                            <tr class="hover:bg-slate-50 transition-all group">
                                <td class="px-12 py-8">
                                    ...
                                    <p class="text-[9px] text-slate-400 uppercase tracking-widest leading-none">{{ __("Installation:") }} <span class="text-slate-600 font-black">{{ $batch->building->name ?? 'N/A' }}</span></p>
                                </td>
                                <td class="px-8 py-8 text-center text-slate-500 text-base">
                                    {{ number_format($batch->initial_quantity, 0, ',', ' ') }} <small class="text-[10px] opacity-40 italic uppercase">{{ __("Têtes") }}</small>
                                </td>
                                    ...
                                    <div class="flex flex-col items-end gap-2">
                                        ...
                                        @if($ratio > 0)
                                            <span class="text-[9px] font-black uppercase {{ $ratio > $averageCostPerHead ? 'text-rose-500' : 'text-emerald-500' }} tracking-tighter">
                                                {{ $ratio > $averageCostPerHead ? __("⚠️ SURCOÛT") : __("✔️ ÉCONOMIE") }} : {{ number_format(abs($diff), 1) }}% {{ __("vs moy.") }}
                                            </span>
                                        @endif
                                    </div>
                                </td>
