<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="font-black text-2xl text-slate-800 leading-none uppercase italic tracking-tighter">⚖️ Compte de résultat</h2>
                <p class="text-[10px] font-black text-amber-600 uppercase tracking-[0.2em] mt-2 italic">P&L consolidé — toutes activités</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
                    <p class="text-[9px] font-black text-emerald-500 uppercase tracking-widest mb-2 italic">Produits</p>
                ...
                <div class="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
                    <p class="text-[9px] font-black text-rose-500 uppercase tracking-widest mb-2 italic">Charges</p>
                ...
                <div class="p-8 rounded-[3rem] shadow-2xl {{ $netResult >= 0 ? 'bg-emerald-600' : 'bg-rose-600' }} text-white">
                    <p class="text-[9px] font-black uppercase tracking-widest mb-2 italic opacity-80">Résultat net</p>
                    <p class="text-3xl font-black italic tracking-tighter">{{ number_format($netResult) }}</p>
                    <p class="text-[8px] mt-2 uppercase font-black opacity-80">Marge {{ $marginPct }}% · GNF</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
                    <h3 class="text-[11px] font-black uppercase text-slate-800 tracking-[0.2em] italic flex items-center mb-6">
                        <span class="w-2 h-6 bg-emerald-500 rounded-full mr-3"></span> Produits
                    </h3>
                    ...
                    @empty
                        <p class="text-center text-slate-300 text-[10px] font-black uppercase tracking-widest italic py-6">Aucun produit sur la période.</p>
                    @endforelse
                    <div class="flex justify-between items-center pt-4 mt-2">
                        <span class="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Total produits</span>
                ...
                <div class="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
                    <h3 class="text-[11px] font-black uppercase text-slate-800 tracking-[0.2em] italic flex items-center mb-6">
                        <span class="w-2 h-6 bg-rose-500 rounded-full mr-3"></span> Charges
                    </h3>
                    ...
                    @endforeach
                    <div class="flex justify-between items-center pt-4 mt-2">
                        <span class="text-[10px] font-black text-rose-600 uppercase tracking-widest">Total charges</span>
            ...
            <div class="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
                <h3 class="text-[11px] font-black uppercase text-slate-800 tracking-[0.2em] italic flex items-center mb-2">
                    <span class="w-2 h-6 bg-amber-500 rounded-full mr-3"></span> Marge directe par espèce
                </h3>
                <p class="text-[9px] font-black text-slate-300 uppercase tracking-widest italic mb-6 ml-5">Revenus & coûts traçables au lot · hors frais généraux (paie, eau, énergie)</p>
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="text-[8px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                                <th class="pb-3">Espèce</th><th class="pb-3 text-right">Produits</th><th class="pb-3 text-right">Coûts directs</th><th class="pb-3 text-right">Marge directe</th>
                            </tr>
                        </thead>
                        <tbody>
                ...
                @else
                    <p class="text-center text-slate-300 text-[10px] font-black uppercase tracking-widest italic py-6">Aucune activité traçable par espèce sur la période.</p>
                @endif
            </div>

            <p class="text-[9px] font-black text-slate-300 uppercase tracking-widest italic text-center">
                Vue trésorerie sur la période — produits = ventes validées + lait collecté valorisé · charges = flux engagés sur la période.
            </p>
        </div>
    </div>
</x-app-layout>
