        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 text-left">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex items-center gap-2 px-3 py-1.5 bg-orange-50 text-orange-600 border border-orange-100 rounded-xl text-[8px] font-black uppercase italic shadow-sm">
                    <span class="w-1.5 h-1.5 bg-orange-500 rounded-full"></span> {{ __("Alerte") }} {{ $seuilAlerte }}-{{ $seuilCritique }}%
                </div>
                <div class="flex items-center gap-2 px-3 py-1.5 bg-rose-50 text-rose-600 border border-rose-100 rounded-xl text-[8px] font-black uppercase italic shadow-sm">
                    <span class="w-1.5 h-1.5 bg-rose-500 rounded-full"></span> {{ __("Critique") }} > {{ $seuilCritique }}%
                </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 gap-12">
                ...
                <div class="bg-white rounded-[4rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 relative overflow-hidden group">
                    ...
                    <div class="p-10 relative z-10 text-left">
                            ...
                            <div class="flex items-center gap-6">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-3">
                                        <span class="px-3 py-1 bg-blue-600 text-white rounded-xl text-[9px] font-black uppercase italic shadow-lg shadow-blue-200">{{ __("ÂGE") }} : {{ $batch['age'] }} J</span>
                                        <span class="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em] italic">{{ __("BAT") }} : {{ $batch['building'] }}</span>
                                    </div>
                                </div>
                            </div>

                            <div class="flex flex-col items-end gap-2">
                                <p class="text-[9px] text-slate-400 uppercase font-black tracking-widest italic leading-none">{{ __("Indicateur de Viabilité") }}</p>
                                <span @class([
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 mb-10">
                            ...
                            <div class="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 flex flex-col items-center justify-center text-center group/card hover:bg-slate-900 transition-all duration-500">
                                <p class="text-[9px] text-slate-400 uppercase mb-3 group-hover/card:text-slate-500 font-black">{{ __("Indice (FCR)") }}</p>
                                ...
                                <p class="text-[8px] text-blue-500 font-black mt-2 uppercase italic tracking-widest group-hover/card:text-blue-400">{{ __("Rendement Aliment") }}</p>
                            </div>

                            {{-- POIDS MOYEN --}}
                            <div class="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 flex flex-col items-center justify-center text-center">
                                <p class="text-[9px] text-slate-400 uppercase mb-3 font-black">{{ __("Poids Moyen") }}</p>
                                ...
                                <p class="text-[8px] text-emerald-500 font-black mt-2 uppercase italic tracking-widest">+{{ $batch['daily_gain'] ?? 0 }}g / {{ __("Jour") }}</p>
                            </div>

                            {{-- VIVANT ACTUEL --}}
                            <div class="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 flex flex-col items-center justify-center text-center">
                                <p class="text-[9px] text-slate-400 uppercase mb-3 font-black">{{ __("Stock Vivant") }}</p>
                                <p class="text-3xl font-black text-blue-600 italic tracking-tighter">{{ number_format($batch['current']) }}</p>
                                <p class="text-[8px] text-slate-400 font-black mt-2 uppercase italic tracking-widest">{{ __("Initial") }} : {{ number_format($batch['initial']) }}</p>
                            </div>

                            {{-- PERTES (TÊTES) --}}
                            <div class="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 flex flex-col items-center justify-center text-center">
                                <p class="text-[9px] text-rose-400 uppercase mb-3 font-black">{{ __("Mortalité") }}</p>
                                <p class="text-3xl font-black text-rose-600 italic tracking-tighter">{{ number_format($batch['mortality_count']) }}</p>
                                <p class="text-[8px] text-rose-400 font-black mt-2 uppercase italic tracking-widest">{{ __("Sujets perdus") }}</p>
                            </div>

                            {{-- TAUX DE MORTALITÉ (L) --}}
                            <div @class([
                                'p-8 rounded-[3rem] border-2 flex flex-col items-center justify-center text-center col-span-2 lg:col-span-2 shadow-inner',
                                'bg-rose-50 border-rose-100' => $batch['mortality_rate'] > $seuilCritique,
                                'bg-orange-50 border-orange-100' => $batch['mortality_rate'] > $seuilAlerte && $batch['mortality_rate'] <= $seuilCritique,
                                'bg-emerald-50 border-emerald-100' => $batch['mortality_rate'] <= $seuilAlerte,
                            ])>
                                <p @class([
                                    'text-[10px] font-black uppercase mb-2 italic tracking-[0.2em]',
                                    'text-rose-800' => $batch['mortality_rate'] > $seuilCritique,
                                    'text-emerald-800' => $batch['mortality_rate'] <= $seuilAlerte,
                                ])>
                                    {{ __("Taux de Mortalité Cumulé") }}
                                </p>
                                <div class="flex items-baseline gap-1">
                                    <p @class([
                                        'text-6xl font-black italic tracking-tighter',
                                        'text-rose-600' => $batch['mortality_rate'] > $seuilCritique,
                                        'text-orange-600' => $batch['mortality_rate'] > $seuilAlerte && $batch['mortality_rate'] <= $seuilCritique,
                                        'text-emerald-600' => $batch['mortality_rate'] <= $seuilAlerte,
                                    ])>
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between items-center px-4">
                                <p class="text-[9px] font-black uppercase text-slate-400 italic">{{ __("Progression des pertes vs seuil critique") }} ({{ $seuilCritique * 2 }}%)</p>
                                <p class="text-[9px] font-black text-slate-800 italic uppercase">{{ __("Limite Technique") }}</p>
                            </div>
                            <div class="relative h-6 bg-slate-50 rounded-full overflow-hidden border border-slate-100 p-1.5 shadow-inner">
                                <div class="h-full rounded-full transition-all duration-1000 shadow-sm {{ $batch['mortality_rate'] > $seuilCritique ? 'bg-rose-500' : ($batch['mortality_rate'] > $seuilAlerte ? 'bg-orange-500' : 'bg-emerald-500') }}" 
                                     style="width: {{ min($batch['mortality_rate'] * 10, 100) }}%">
                                </div>
                                {{-- Marqueur de danger {{ $seuilCritique }}% --}}
                    ...
                    <div class="bg-slate-50 px-10 py-6 flex justify-between items-center border-t border-slate-100 italic">
                        <div class="flex items-center gap-2">
                            ...
                            <p class="text-[9px] text-slate-400 font-black uppercase">{{ __("Calcul temps réel") }} • {{ now()->translatedFormat('d F Y - H:i') }}</p>
                        </div>
                        <a href="{{ route('batches.show', $batch['id']) }}" class="group/btn flex items-center gap-3 px-6 py-3 bg-white border border-slate-200 text-slate-900 rounded-2xl text-[10px] font-black uppercase tracking-widest no-underline hover:bg-slate-900 hover:text-white transition-all duration-300">
                            {{ __("Audit Complet du Lot") }} <i class="fa-solid fa-arrow-right group-hover/btn:translate-x-2 transition-transform"></i>
                        </a>
                    </div>
                </div>
                @empty
                <div class="bg-white p-32 rounded-[5rem] text-center border-4 border-dashed border-slate-50 italic">
                    ...
                    <p class="text-slate-300 uppercase text-xs font-black tracking-[0.4em] italic leading-none">{{ __("Aucun lot actif en cours d'analyse") }}</p>
                </div>
                @endforelse
            </div>
        </div>
