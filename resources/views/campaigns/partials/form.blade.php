...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    <div class="lg:col-span-2 space-y-8">
        <div class="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100">
            <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-8 italic leading-none">{{ __("01. Identification") }}</h3>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="md:col-span-2">
                    <label class="block text-[10px] font-black text-slate-500 uppercase mb-2 ml-1 italic leading-none">{{ __("Nom de la campagne *") }}</label>
                    <input type="text" name="name" value="{{ old('name', $c->name ?? '') }}" required placeholder="{{ __("Ex : Tabaski 2026 — Moutons Djallonké") }}"
                           class="w-full p-4 bg-slate-50 rounded-2xl border-none font-black text-slate-700 shadow-inner italic outline-none">
                </div>

                <div>
                    <label class="block text-[10px] font-black text-slate-500 uppercase mb-2 ml-1 italic leading-none">{{ __("Type *") }}</label>
                ...
                <div>
                    <label class="block text-[10px] font-black text-slate-500 uppercase mb-2 ml-1 italic leading-none">{{ __("Espèce ciblée *") }}</label>
                ...
                <div>
                    <label class="block text-[10px] font-black text-slate-500 uppercase mb-2 ml-1 italic leading-none">{{ __("Statut *") }}</label>
                ...
                <div>
                    <label class="block text-[10px] font-black text-slate-500 uppercase mb-2 ml-1 italic leading-none">{{ __("Début (achat / mise en place)") }}</label>
                    <input type="date" name="start_date" value="{{ old('start_date', isset($c) && $c->start_date ? $c->start_date->toDateString() : '') }}"
                           class="w-full p-4 bg-slate-50 rounded-2xl border-none font-black text-slate-700 shadow-inner italic outline-none">
                </div>

                <div class="md:col-span-2">
                    <label class="block text-[10px] font-black text-slate-500 uppercase mb-2 ml-1 italic leading-none">{{ __("Date du pic de vente (fête) *") }}</label>
                    ...
                    @if(!empty($nextEidDates))
                        <div class="flex flex-wrap gap-2 mt-3">
                            <span class="text-[8px] font-black text-slate-400 uppercase self-center">{{ __("Tabaski à venir :") }}</span>
        ...
        <div class="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100">
            <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-8 italic leading-none">{{ __("02. Objectifs (la marge se joue ici)") }}</h3>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div>
                    <label class="block text-[10px] font-black text-emerald-500 uppercase mb-2 ml-1 italic leading-none">{{ __("Objectif têtes") }}</label>
            ...
            <div class="mt-6">
                ...
                <textarea name="notes" rows="2" placeholder="{{ __("Informations complémentaires...") }}" class="w-full bg-slate-50 border-none rounded-2xl p-4 text-xs font-bold shadow-inner outline-none italic">{{ old('notes', $c->notes ?? '') }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-6">
        <div class="bg-slate-900 p-8 rounded-[3rem] text-white shadow-2xl">
            <h3 class="text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em] mb-4 italic leading-none">🐑 {{ __("Logique Tabaski") }}</h3>
            <p class="text-[11px] text-slate-300 leading-relaxed font-bold italic">
                {{ __("Achat groupé en amont, engraissement de 60 à 90 jours, vente groupée au pic.") }}
                {{ __("Rattachez vos lots ovins/caprins à la campagne pour suivre la marge en temps réel.") }}
            </p>
        </div>
