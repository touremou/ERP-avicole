        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 italic font-bold">
                        ...
                        @if($production->status === 'Planifié' || $production->status === 'En cours')
                            <form action="{{ route('production.complete', $production->id) }}" method="POST" onsubmit="return confirm('Confirmer la fin du mélange ? Les stocks et compteurs machines seront mis à jour.')">
                                ...
                                <button type="submit" class="bg-emerald-500 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-emerald-500/20 hover:bg-emerald-600 transition-all">
                                    <i class="fa-solid fa-check-double mr-2"></i> Clôturer & Stocker
                                </button>
                            </form>
                        @endif
                    @endcan
            ...
            @if($production->status === 'Terminé')
                <div class="mb-8 p-6 bg-emerald-50 border-2 border-emerald-200 rounded-[2rem] flex items-center justify-between print:hidden">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="text-left">
                            <p class="text-[10px] text-emerald-600 uppercase leading-none font-black italic">Production Clôturée</p>
                            <p class="text-xs font-black text-slate-800 uppercase mt-1 italic tracking-tighter">Stocks et Maintenance mis à jour</p>
                        </div>
                    </div>
                    <div class="text-right">
                        <p class="text-[10px] text-slate-400 uppercase leading-none italic font-black">Coût de revient réel</p>
            ...
            <div class="border-4 border-slate-900 p-10 rounded-[3rem] relative overflow-hidden bg-white shadow-2xl print:shadow-none print:border-2">
                ...
                <div class="flex justify-between items-start border-b-4 border-slate-900 pb-8 mb-8 text-left">
                    <div>
                        <h1 class="text-4xl font-black text-slate-900 uppercase italic tracking-tighter leading-none">Bon de Pesée</h1>
                        <p class="text-sm text-slate-500 uppercase mt-2 tracking-[0.3em] font-bold">Ordre : {{ $production->batch_number ?? 'OP-'.str_pad($production->id, 5, '0', STR_PAD_LEFT) }}</p>
                    </div>
                    <div class="text-right">
                        <p class="text-[10px] uppercase text-slate-400 leading-none mb-1 italic font-black">Date d'émission</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 text-left">
                    <div class="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                        <p class="text-[9px] uppercase text-slate-400 mb-2 font-black italic">Formule active</p>
                        <p class="text-xl font-black text-slate-900 uppercase italic leading-tight tracking-tighter">{{ $production->formula->name ?? 'N/A' }}</p>
                    </div>

                    <div class="bg-slate-900 p-6 rounded-3xl text-white shadow-xl">
                        <p class="text-[9px] uppercase text-slate-400 mb-2 font-black italic opacity-70">Masse Totale Cible</p>
                        ...
                        <p class="text-[10px] uppercase mt-2 opacity-50 font-black italic">{{ number_format($production->quantity_produced / 50, 1) }} Sacs (50kg)</p>
                    </div>

                    <div class="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                        <p class="text-[9px] uppercase text-slate-400 mb-2 font-black italic">Ligne de Production</p>
                        <div class="flex flex-col gap-1">
                            ...
                            @else
                                <p class="text-xs font-black text-slate-800 uppercase italic leading-none flex items-center">
                                    <i class="fa-solid fa-gear mr-1.5 text-[8px] text-slate-300"></i> {{ $production->machine->name ?? 'Standard' }}
                                </p>
                            @endif
                        </div>
                    </div>
                ...
                <div class="mb-6 flex items-center gap-4">
                    <h3 class="text-xs font-black uppercase tracking-[0.4em] text-slate-400 italic leading-none">Instructions de dosage (MP)</h3>
                ...
                <table class="w-full mb-12 text-left">
                    <thead>
                        <tr class="border-b-2 border-slate-200">
                            <th class="py-4 text-left text-[10px] uppercase font-black italic">Ingrédient</th>
                            <th class="py-4 text-center text-[10px] uppercase font-black italic px-4">Part %</th>
                            <th class="py-4 text-right text-[10px] uppercase font-black italic bg-slate-50 rounded-t-xl px-4">Poids à Peser</th>
                            <th class="py-4 text-center text-[10px] uppercase font-black italic px-4">Visa</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="group">
                            <td class="py-6">
                                ...
                                <p class="text-[8px] text-slate-400 uppercase mt-1 font-bold italic tracking-widest">Silo / Magasin MP</p>
                            </td>
                    ...
                    <tfoot>
                        <tr class="border-t-4 border-slate-900">
                            <td colspan="2" class="py-6 text-right uppercase text-[10px] font-black italic tracking-widest pr-8">Total Mélange Net :</td>
                ...
                <div class="grid grid-cols-2 gap-12 mt-12 pt-12 border-t-2 border-dashed border-slate-100 italic">
                    <div class="text-center">
                        <p class="text-[8px] uppercase text-slate-400 mb-10 font-black tracking-widest">Opérateur de Pesée ({{ $production->user->name ?? 'Admin' }})</p>
                    ...
                    <div class="text-center">
                        <p class="text-[8px] uppercase text-slate-400 mb-10 font-black tracking-widest">Responsable Provenderie</p>
                        <div class="h-px bg-slate-200 w-48 mx-auto"></div>
                    </div>
                </div>
            </div>
