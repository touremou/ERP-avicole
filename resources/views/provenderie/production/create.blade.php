<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div class="text-left">
                    <h2 class="font-black text-2xl text-slate-800 uppercase italic tracking-tighter leading-none">Lancer Production</h2>
                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1 italic">Provenderie • Ordre de Fabrication</p>
                </div>
            </div>
    ...
    <div class="py-12 italic font-bold">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
                ...
                <form action="{{ route('production.store') }}" method="POST" class="space-y-8 relative" id="prodForm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-6">
                            <div class="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 space-y-6 text-left">
                                <h3 class="text-[10px] font-black uppercase text-emerald-500 tracking-widest italic">01. Configuration de la Ligne</h3>

                                {{-- SÉLECTEUR DE FORMULE --}}
                                <div class="space-y-2">
                                    <label class="block text-[10px] font-black text-slate-400 uppercase italic tracking-widest ml-2">Recette à produire</label>
                                ...
                                <div class="space-y-4">
                                    <div class="flex justify-between items-center ml-2">
                                        <label class="block text-[10px] font-black text-slate-400 uppercase italic tracking-widest">Machines activées (Ligne)</label>
                                        <span id="machine_count" class="text-[9px] bg-slate-100 px-2 py-0.5 rounded-lg text-slate-500 font-black italic">0 sélectionnée</span>
                                    </div>
                                    <div class="grid grid-cols-1 gap-2 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                                        ...
                                        @empty
                                            <div class="p-4 bg-red-50 rounded-2xl border border-red-100 text-center">
                                                <p class="text-[9px] text-red-600 font-black uppercase italic">⚠️ Aucune machine opérationnelle</p>
                                            </div>
                                        @endforelse
                                    </div>
                                    <p id="machine_error" class="text-[8px] text-red-500 italic hidden text-center uppercase font-black">⚠️ Veuillez sélectionner au moins une machine.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-6">
                            <div class="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 space-y-6 text-left">
                                <h3 class="text-[10px] font-black uppercase text-blue-500 tracking-widest italic">02. Volume de Production</h3>
                                
                                <div class="grid grid-cols-1 gap-4">
                                    <div>
                                        <label class="block text-[10px] font-black text-slate-400 uppercase italic tracking-widest ml-2">Nb de sacs (50kg)</label>
                                    ...
                                    <div>
                                        <label class="block text-[10px] font-black text-slate-400 uppercase italic tracking-widest ml-2">Poids Total Estimé</label>
                                        <div class="relative">
                            ...
                            <div class="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 space-y-6 text-left">
                                <h3 class="text-[10px] font-black uppercase text-amber-500 tracking-widest italic">03. Responsabilité</h3>
                                
                                <div class="space-y-2">
                                    <label class="block text-[10px] font-black text-slate-400 uppercase italic tracking-widest ml-2">Responsable Production</label>
                                    ...
                                    <p class="text-[8px] text-slate-400 italic px-2 font-black uppercase">Ce responsable valide la conformité du mélange.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="stock_check_container" class="bg-slate-50 p-8 rounded-[3rem] border border-slate-100 transition-all text-left">
                        <div class="flex justify-between items-center mb-6 px-4">
                            <h4 class="text-[10px] font-black text-slate-400 uppercase italic tracking-widest">Analyse de Faisabilité (Silos MP)</h4>
                        ...
                        <div id="material_needs" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <p class="text-[9px] text-slate-300 italic uppercase py-8 text-center w-full col-span-3">Veuillez sélectionner une formule pour analyser les stocks...</p>
                        </div>
                    </div>
                    ...
                    <button type="submit" id="submit_btn" disabled class="w-full bg-slate-100 text-slate-300 font-black py-8 rounded-[2.5rem] uppercase tracking-[0.4em] text-sm italic transition-all cursor-not-allowed shadow-inner border-2 border-transparent">
                        <i class="fa-solid fa-lock mr-2"></i> Sélectionner les paramètres
                    </button>
                </form>
            </div>
            @else
            ...
            <div class="bg-white p-20 rounded-[4rem] border border-slate-100 shadow-xl text-center italic font-bold">
                ...
                <p class="text-slate-400 text-[10px] font-black uppercase tracking-widest italic">Vous n'avez pas les droits (C) pour créer un ordre de production.</p>
                <a href="{{ route('production.index') }}" class="inline-block mt-8 px-10 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase italic no-underline hover:bg-emerald-500 transition-all">Retour au Journal</a>
            </div>
            @endcan
    ...
    <script>
        function updateMachineCount() {
            ...
            if(display) {
                display.innerText = `${checkedCount} sélectionnée${checkedCount > 1 ? 's' : ''}`;
                if(checkedCount > 0) {
        ...
        function checkStocks() {
            ...
            if (!select || !select.value || totalWeight <= 0) {
                if(container) container.innerHTML = '<p class="text-[9px] text-slate-300 italic uppercase py-8 text-center w-full col-span-3">Veuillez choisir une formule valide...</p>';
            ...
            try {
                ...
                items.forEach(item => {
                    ...
                    const matName = item.raw_material ? item.raw_material.name : 'Inconnu';
                        ...
                        <div class="p-5 rounded-[2rem] ${enough ? 'bg-white shadow-sm' : 'bg-red-50 border border-red-100'} transition-all text-left">
                            ...
                            <div class="flex justify-between items-end">
                                ...
                                <span class="text-[8px] font-black uppercase ${enough ? 'text-emerald-500' : 'text-red-400'} italic">
                                    ${enough ? '<i class="fa-solid fa-check"></i> Prêt' : 'Manque: ' + (need - stock).toFixed(1) + 'kg'}
                                </span>
                            </div>
                        </div>
                    `;
                ...
                if (submitBtn && badge) {
                    if (isPossible && machineSelected) {
                        ...
                        submitBtn.innerHTML = '<i class="fa-solid fa-play-circle mr-2 text-emerald-400"></i> Lancer la Production';
                        ...
                        badge.innerText = "Ligne de Silos : OK";
                    } else {
                        ...
                        if(!machineSelected) {
                            submitBtn.innerHTML = '<i class="fa-solid fa-cog fa-spin mr-2"></i> Configurer la ligne machine';
                            if(document.getElementById('machine_error')) document.getElementById('machine_error').classList.remove('hidden');
                        } else {
                            submitBtn.innerHTML = '<i class="fa-solid fa-lock mr-2"></i> Rupture de Stock MP';
                        }
                        ...
                        badge.innerText = !isPossible ? "Achat Requis" : "Config Machine";
                    }
                    badge.classList.remove('hidden');
                }
            } catch (e) {
                if(container) container.innerHTML = '<p class="text-red-500 text-[9px] col-span-3">Erreur de structure des données.</p>';
            }
        }
