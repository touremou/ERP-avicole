<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 italic font-bold">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 text-left">
                <div class="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm relative overflow-hidden group">
                    ...
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 italic">Valeur Totale Stock</p>
                ...
                <div class="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm relative overflow-hidden">
                    <p class="text-[10px] font-black text-red-500 uppercase tracking-widest mb-2 italic">Alertes Rupture</p>
                ...
                <div class="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
                    <p class="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-2 italic">Ingrédients Actifs</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 text-left">
                    ...
                    <div class="bg-white p-8 rounded-[3.5rem] border-2 {{ $isLow ? 'border-red-100 shadow-red-50' : 'border-slate-50 shadow-sm' }} hover:shadow-2xl transition-all relative overflow-hidden group">
                        ...
                        <div class="flex justify-between items-start mb-6 pr-16">
                            ...
                            <div class="text-right">
                                <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 italic leading-none">P.M.P Actuel</p>
                        ...
                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-6 italic leading-none">Unité : {{ strtoupper($material->unit) }}</p>
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between items-end leading-none">
                                ...
                                <p class="text-[10px] font-black text-slate-400 uppercase italic mb-1">En Stock</p>
                            </div>
                ...
                @empty
                    <div class="col-span-full py-20 bg-white rounded-[4rem] border-2 border-dashed border-slate-100 text-center">
                        ...
                        <p class="text-slate-300 uppercase italic font-black text-xs">Aucun ingrédient en catalogue</p>
                    </div>
                @endforelse
            </div>
        </div>
    ...
    <div id="modalAddMaterial" class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
        <div class="bg-white w-full max-w-lg rounded-[3.5rem] shadow-2xl p-10 italic relative overflow-hidden">
            <h3 class="text-2xl font-black text-slate-900 uppercase italic tracking-tighter mb-8">Nouveau Composant</h3>
            <form action="{{ route('raw-materials.store') }}" method="POST" class="space-y-5 text-left relative">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="col-span-full">
                        <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-2 italic">Désignation</label>
                    ...
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-2 italic">Unité</label>
                    ...
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-2 italic">Alerte (kg)</label>
                ...
                <div class="flex gap-3">
                    <button type="submit" class="flex-[2] bg-slate-900 text-white font-black py-5 rounded-2xl shadow-xl uppercase italic hover:bg-amber-600 transition-colors">Enregistrer</button>
                    <button type="button" onclick="document.getElementById('modalAddMaterial').classList.add('hidden')" class="flex-1 bg-slate-100 text-slate-400 rounded-2xl uppercase font-black text-[10px]">Fermer</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="modalEditMaterial" class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
        <div class="bg-white w-full max-w-lg rounded-[3.5rem] shadow-2xl p-10 italic relative overflow-hidden">
            ...
            <h3 class="text-2xl font-black text-slate-900 uppercase italic tracking-tighter mb-8 relative">Modifier Ingrédient</h3>
            ...
            <form id="edit_form" method="POST" class="space-y-5 text-left relative">
                ...
                <div class="bg-blue-50/50 p-6 rounded-[2.5rem] border border-blue-100 space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[8px] font-black text-blue-400 uppercase mb-2 ml-2 italic text-center">Qté Actuelle (kg)</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-black text-blue-600 uppercase mb-2 ml-2 text-center italic">Coût/kg (GNF)</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-2 italic">Énergie (kcal)</label>
                    ...
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-2 italic">Protéines (%)</label>
                ...
                <div class="flex gap-3">
                    <button type="submit" class="flex-[2] bg-slate-900 text-white font-black py-5 rounded-2xl shadow-xl uppercase italic">Mettre à jour</button>
                    <button type="button" onclick="el('modalEditMaterial').classList.add('hidden')" class="flex-1 bg-slate-100 text-slate-400 rounded-2xl uppercase font-black text-[10px]">Fermer</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="modalLoss" class="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-50 hidden flex items-center justify-center p-4">
        <div class="bg-white w-full max-w-md rounded-[3.5rem] shadow-2xl p-10 italic relative overflow-hidden text-left">
            ...
            <h3 id="loss_title" class="text-2xl font-black text-slate-900 uppercase italic tracking-tighter mb-8 leading-none">Sortie de Stock</h3>
            ...
            <form id="loss_form" method="POST" class="space-y-6">
                ...
                <div class="bg-red-50 p-6 rounded-[2.5rem] border border-red-100 space-y-4">
                    <div>
                        <label class="block text-[10px] font-black text-red-600 uppercase mb-2 ml-2 italic">Quantité à retirer (kg)</label>
                ...
                <div>
                    <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-2 italic">Motif de la sortie</label>
                    <select name="reason" required class="w-full bg-slate-50 border-none rounded-2xl p-4 font-black text-slate-800 shadow-inner italic appearance-none">
                        <option value="perte">PERTE / AVARIE (MOISISSURE)</option>
                        <option value="vol">ÉCART D'INVENTAIRE / VOL</option>
                        <option value="vente">VENTE DIRECTE (SANS TRANSFORMATION)</option>
                        <option value="don">DON / AUTRE</option>
                    </select>
                </div>

                <div class="flex gap-3 pt-4">
                    <button type="submit" class="flex-[2] bg-red-600 text-white font-black py-5 rounded-2xl shadow-xl uppercase italic hover:bg-red-700 transition-all">Confirmer la sortie</button>
                    <button type="button" onclick="el('modalLoss').classList.add('hidden')" class="flex-1 bg-slate-100 text-slate-400 rounded-2xl uppercase font-black text-[10px]">Annuler</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="modalLabo" class="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-50 hidden flex items-center justify-center p-4">
        <div class="bg-white w-full max-w-md rounded-[3.5rem] shadow-2xl p-10 italic relative overflow-hidden text-left">
            ...
            <h3 id="labo_title" class="text-2xl font-black text-slate-900 uppercase italic tracking-tighter mb-8 leading-none italic">Analyse Labo</h3>
            <form id="labo_form" method="POST" class="space-y-6">
                @csrf @method('PUT')
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-2 italic">Énergie (kcal/kg)</label>
                ...
                <button type="submit" class="w-full bg-slate-900 text-white font-black py-5 rounded-2xl shadow-xl italic uppercase">Enregistrer Valeurs</button>
            </form>
        </div>
    </div>
    ...
    <div id="modalReorder" class="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-50 hidden flex items-center justify-center p-4">
        <div class="bg-white w-full max-w-lg rounded-[3.5rem] shadow-2xl p-10 relative italic text-left overflow-hidden">
            ...
            <h3 id="reorder_title" class="text-2xl font-black text-slate-900 uppercase italic tracking-tighter mb-8 leading-none">Réception Magasin</h3>
            <form id="reorder_form" method="POST" class="space-y-6">
                @csrf @method('PUT')
                <div>
                    <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-2 italic tracking-widest">Fournisseur</label>
                ...
                <div class="bg-blue-50 p-6 rounded-[2.5rem] border border-blue-100 space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[8px] font-black text-blue-400 uppercase mb-2 ml-2 italic">Nb sacs</label>
                        ...
                        <div>
                            <label class="block text-[8px] font-black text-blue-400 uppercase mb-2 ml-2 italic">Format</label>
                    ...
                    <input type="number" step="0.1" min="0" placeholder="0.0" name="added_qty" id="final_qty" placeholder="Total kg" required oninput="calculateUnitCost()" class="w-full bg-white border-2 border-blue-200 rounded-2xl p-4 font-black text-3xl text-blue-600 text-center italic shadow-lg">
                </div>
                <div>
                    <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-2 italic tracking-widest">Montant Facturé (GNF)</label>
                    ...
                    <p class="text-center mt-3 text-[9px] text-slate-400 uppercase italic font-black">Coût unitaire estimé : <span id="unit_cost_display" class="text-blue-500">0</span> GNF/kg</p>
                </div>
                <button type="submit" class="w-full bg-slate-900 text-white font-black py-5 rounded-2xl shadow-xl uppercase italic hover:bg-blue-600 transition-colors">Valider l'entrée</button>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function openReorderModal(id, name) {
            el('reorder_title').innerText = `📦 RÉCEPTION : ${name}`;
        ...
        function openLossModal(id, name) {
            el('loss_title').innerText = `📉 AJUSTEMENT : ${name}`;
        ...
        function openLaboModal(id, name, energy, protein) {
            el('labo_title').innerText = `🔬 ANALYSE : ${name}`;
        ...
        function calculateUnitCost() {
            ...
            display.innerText = (qty > 0) ? Math.round(total / qty).toLocaleString() : "0";
        }
    </script>
</x-app-layout>