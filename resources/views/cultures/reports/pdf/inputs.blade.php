...
<head>
    <meta charset="utf-8">
    <title>{{ __("Intrants") }} {{ $year }}</title>
    <style>
        @page { margin: 25px 30px; }
        body { font-family: 'DejaVu Sans', sans-serif; font-size: 11px; color: #1e293b; }
        h1 { font-size: 18px; margin: 0 0 4px 0; text-transform: uppercase; }
        h2 { font-size: 13px; margin: 16px 0 6px 0; text-transform: uppercase; color: #92400e; }
        ...
        .kpi-val { font-size: 18px; font-weight: bold; color: #92400e; }
...
<body>
    <h1>{{ __("Coûts des Intrants") }} — {{ $year }}</h1>
    <div class="subtitle">{{ __("Production végétale") }} · {{ __("Généré le") }} {{ now()->format('d/m/Y H:i') }}</div>

    <div class="kpis">
        <div class="kpi"><div class="kpi-val">{{ $inputs->count() }}</div><div class="kpi-lbl">{{ __("Lignes saisies") }}</div></div>
        <div class="kpi"><div class="kpi-val">{{ number_format($totalCost, 0) }} {{ setting('general.currency', 'GNF') }}</div><div class="kpi-lbl">{{ __("Coût total") }}</div></div>
        <div class="kpi"><div class="kpi-val">{{ $byCrop->count() }}</div><div class="kpi-lbl">{{ __("Cultures concernées") }}</div></div>
    </div>

    <h2>{{ __("Répartition par type") }}</h2>
    <table class="data">
        <thead><tr><th>{{ __("Type") }}</th><th>{{ __("Lignes") }}</th><th>{{ __("Coût") }} ({{ setting('general.currency', 'GNF') }})</th><th>%</th></tr></thead>
        <tbody>
    ...
    <h2>{{ __("Répartition par culture") }}</h2>
    <table class="data">
        <thead><tr><th>{{ __("Culture") }}</th><th>{{ __("Lignes") }}</th><th>{{ __("Coût") }} ({{ setting('general.currency', 'GNF') }})</th></tr></thead>
        <tbody>
