<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4 text-left">
                ...
                <div>
                    ...
                    <p class="text-[10px] font-black text-orange-500 uppercase tracking-widest mt-1 italic leading-none">
                        Session du {{ \Carbon\Carbon::parse($check->check_date)->format('d/m/Y') }} • Lot : {{ $check->batch->code }}
                    </p>
                </div>
            </div>
            <div class="hidden md:block text-right">
                <span class="px-4 py-2 bg-orange-900 text-amber-400 rounded-xl text-[10px] font-black uppercase italic tracking-widest border border-orange-800 shadow-lg leading-none">
                    <i class="fas fa-exclamation-triangle mr-2"></i> Mode Correction (Secteur {{ $check->batch->type }})
                </span>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
                ...
                <form action="{{ route('daily-checks.update', $check->id) }}" method="POST" class="space-y-8" id="edit-precision-form">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
                        ...
                        <div id="mortality-card" class="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm transition-all relative">
                            <label class="block text-[10px] font-black text-red-500 uppercase mb-4 tracking-widest italic ml-1 leading-none">Mortalité rectifiée (Têtes)</label>
                            ...
                            <div class="pt-4 border-t border-slate-50">
                                ...
                                <select name="feed_type" id="feed_type" required onchange="updateEditStats()" 
                                    ...
                                    @foreach($phases ?? [] as $phaseName)
                                        @php 
                                            // Utilisation des données préparées par le controller
                                            $qtyInStock = $stockData[$phaseName] ?? 0;
                                            
                                            // LOGIQUE ERP : Reconstitution du stock disponible
                                ...
                                <p id="stock-warning" class="hidden text-[8px] text-red-500 font-black uppercase mt-2 italic leading-none animate-bounce text-left">⚠️ Stock Magasin insuffisant !</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 text-left">
                        ...
                        <textarea name="observations" rows="2" class="w-full bg-slate-50 rounded-[2rem] p-6 outline-none focus:bg-white border-2 border-transparent focus:border-blue-500 font-bold text-slate-600 transition shadow-inner text-xs uppercase italic" placeholder="JUSTIFICATION DE LA RECTIFICATION...">{{ old('observations', $check->observations) }}</textarea>
                    </div>

                    <div class="flex flex-col md:flex-row gap-4 pt-6">
                        <a href="{{ url()->previous() }}" class="flex-1 bg-white border-2 border-slate-100 text-slate-400 font-black py-6 rounded-[2rem] shadow-sm hover:bg-slate-50 text-center uppercase tracking-widest text-[10px] italic transition no-underline flex items-center justify-center">
                            Annuler
                        </a>
            ...
            @else
                <div class="bg-white p-20 rounded-[4rem] border border-slate-100 shadow-xl text-center">
                    ...
                    <p class="text-slate-400 text-[10px] font-black uppercase tracking-widest italic leading-none">Vous n'avez pas la permission (M) pour rectifier les relevés journaliers.</p>
                    <a href="{{ route('batches.show', $check->batch_id) }}" class="inline-block mt-8 px-10 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase italic no-underline hover:bg-orange-500 transition-all">Retour au Lot</a>
                </div>
            @endcan
        </div>
    </div>
    ...
    <script>
        ...
        function updateEditStats() {
            const currentStockBirds = parseFloat(el('current_stock_birds').value);
            const originalMortality = parseFloat(el('original_mortality').value);
            const newMortality = parseFloat(el('mortality').value) || 0;
            ...
            if (newMortality > maxAllowed) {
                ...
                alert("Erreur ERP : La mortalité rectifiée dépasse l'effectif total disponible.");
            }
            ...
            if (consumed > stockAvailable) {
                ...
                warning.innerText = `⚠️ Stock insuffisant (Libre : ${new Intl.NumberFormat('fr-FR').format(stockAvailable)} kg)`;
