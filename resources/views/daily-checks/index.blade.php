        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 text-left italic font-bold">
            <div>
                ...
                <p class="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] mt-3 italic leading-none">
                    Pointage technique & Consommation — Registre des Suivis Quotidiens
                </p>
            </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('daily-checks.index') }}" method="GET" class="mb-8 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-wrap gap-4 items-center text-left">
                <div class="flex-1 min-w-[200px]">
                    <label class="text-[8px] uppercase text-slate-400 ml-2 mb-1 block font-black">Rechercher un Lot</label>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="CODE DU LOT..." class="w-full bg-slate-50 border-none rounded-xl text-[10px] font-black uppercase shadow-inner focus:ring-2 focus:ring-blue-500 transition-all italic">
                </div>
                <div class="w-48">
                    <label class="text-[8px] uppercase text-slate-400 ml-2 mb-1 block font-black">Trier par</label>
                    <select name="sort" onchange="this.form.submit()" class="w-full bg-slate-50 border-none rounded-xl text-[10px] font-black uppercase shadow-inner focus:ring-2 focus:ring-blue-500 transition-all italic appearance-none cursor-pointer">
                        ...
                        <option value="critical" {{ request('sort') == 'critical' ? 'selected' : '' }}>Mortalité critique</option>
                    </select>
                </div>
            </form>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100 text-[10px] font-black uppercase text-slate-400 italic tracking-widest">
                            ...
                            <th class="px-8 py-6">📦 Lot / Bâtiment</th>
                            <th class="px-8 py-6 text-center text-red-500">💀 Mortalité</th>
                            <th class="px-8 py-6 text-center text-blue-500">🌾 Aliment (Kg)</th>
                            <th class="px-8 py-6 text-center text-cyan-500">💧 Eau (L)</th>
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="px-8 py-6 text-right">
                                <div class="flex justify-end gap-2">
                                    @can('elevage.M')
                                    <a href="{{ route('daily-checks.edit', $check->id) }}" class="p-3 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all shadow-sm hover:shadow-md" title="Modifier">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>
                                    @endcan

                                    @can('elevage.S')
                                    <form action="{{ route('daily-checks.destroy', $check->id) }}" method="POST" onsubmit="return confirm('⚠️ Action irréversible. Le stock d\'aliment sera restitué. Supprimer ?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="p-3 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all shadow-sm hover:shadow-md cursor-pointer" title="Supprimer">
                        ...
                        <tr>
                            <td colspan="6" class="px-8 py-32 text-center">
                                <div class="flex flex-col items-center opacity-10">
                                    ...
                                    <p class="uppercase text-sm tracking-[0.5em] font-black italic">Registre Vierge</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
