        ...
        <div class="space-y-6 text-left italic font-bold" x-data>
            ...
            @if($countAlerts > 0)
                <div class="bg-rose-600 text-white p-5 rounded-[2rem] shadow-xl flex items-center justify-between animate-pulse italic border-b-8 border-rose-800 transition-all">
                    <div>
                        <p class="text-[10px] font-black uppercase tracking-[0.3em] leading-none mb-1">Alerte Couvoir</p>
                        <p class="text-xl font-black uppercase italic leading-none">⚠️ {{ $countAlerts }} Éclosion(s) en attente de clôture</p>
                    </div>
                    <span class="bg-white text-rose-600 px-5 py-2 rounded-2xl text-[10px] font-black uppercase italic shadow-lg">Action requise</span>
                </div>
            @endif
            ...
            <div class="flex flex-col md:flex-row items-center justify-between bg-white border border-slate-100 p-5 rounded-[3rem] shadow-sm gap-4">
                <div class="flex items-center gap-8 ml-6 w-full md:w-auto">
                    <div class="text-left">
                        <span class="text-[9px] text-slate-400 uppercase block leading-none mb-2 tracking-widest italic font-black">Unités Libres</span>
                    ...
                    <div class="text-left border-l border-slate-100 pl-8">
                        <span class="text-[9px] text-rose-400 uppercase block leading-none mb-2 tracking-widest italic font-black">En Maintenance</span>
                    ...
                    <div class="text-left border-l border-slate-100 pl-8 hidden md:block">
                        <span class="text-[9px] text-blue-400 uppercase block leading-none mb-2 tracking-widest italic font-black">Charge Globale</span>
            ...
            <div class="bg-slate-900 p-10 rounded-[3.5rem] text-white flex flex-col md:flex-row justify-between items-center shadow-2xl relative overflow-hidden group">
                <div class="relative z-10 text-left">
                    <h2 class="text-3xl font-black uppercase italic tracking-tighter leading-none mb-4">Flux d'Incubation</h2>
                    <form method="GET" class="flex items-center gap-3">
                        <label class="text-[9px] uppercase opacity-40 tracking-widest italic">Trier par :</label>
                        <select name="sort" onchange="this.form.submit()" class="text-[10px] font-black uppercase italic bg-white/10 border-none rounded-xl px-4 py-2 text-blue-400 focus:ring-0 cursor-pointer hover:bg-white/20 transition-all">
                            <option value="date" {{ $sort=='date'?'selected':'' }}>📅 Échéance Éclosion</option>
                            <option value="progress" {{ $sort=='progress'?'selected':'' }}>📊 État d'avancement</option>
                            <option value="eggs" {{ $sort=='eggs'?'selected':'' }}>🥚 Volume de charge</option>
                        </select>
                    </form>
                </div>
                ...
                <button @click.stop="$dispatch('open-launch-modal')" class="relative z-10 bg-blue-600 hover:bg-blue-500 px-10 py-5 rounded-[2rem] text-[11px] uppercase font-black shadow-2xl italic flex items-center gap-4 transition-all hover:scale-105 active:scale-95 border-none cursor-pointer mt-6 md:mt-0">
                    <i class="fa-solid fa-plus-circle text-lg"></i> Nouveau Lancement
                </button>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-12">
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4 md:gap-6 font-bold italic">
                ...
                <div class="bg-indigo-50 p-6 md:p-8 rounded-[2.5rem] border border-indigo-100 shadow-sm hover:shadow-md transition-shadow col-span-2 md:col-span-1">
                    <span class="text-[9px] text-indigo-400 uppercase font-black tracking-[0.2em] block mb-3">Encours Actuel</span>
                ...
                <div class="bg-slate-900 p-6 md:p-8 rounded-[2.5rem] shadow-2xl text-white relative overflow-hidden col-span-2 md:col-span-1">
                    <div class="relative z-10">
                        <span class="text-[9px] text-blue-400 uppercase font-black tracking-[0.2em] block mb-3 italic">Poussins (30j)</span>
                ...
                <div class="bg-blue-600 p-6 md:p-8 rounded-[2.5rem] shadow-2xl text-white text-left col-span-2 md:col-span-1">
                    <span class="text-[9px] text-blue-200 uppercase font-black tracking-[0.2em] block mb-3">Top Unité</span>
                    <p class="text-base font-black uppercase truncate leading-none mb-2 italic tracking-tighter">{{ $topMachine['name'] ?? 'N/A' }}</p>
                    <span class="text-[11px] opacity-90 font-black italic">{{ number_format($topMachine['avg_hatch'] ?? 0, 1) }}% de réussite</span>
                </div>
            </div>
            ...
            <div class="space-y-4 font-bold italic">
                    ...
                    <div class="bg-white px-8 py-6 rounded-[2.5rem] border border-slate-100 flex flex-wrap md:flex-nowrap items-center gap-8 transition-all hover:shadow-2xl relative group">
                        ...
                        <div class="flex-1 min-w-[250px] text-left">
                            <div class="flex justify-between text-[10px] font-black uppercase italic text-slate-400 mb-3 leading-none tracking-widest">
                                <span>Jour {{ $daysElapsed }} <span class="text-[8px] opacity-40">/ {{ $duration }}</span></span>
                        ...
                        <div class="flex gap-8 px-6 text-center">
                            <div>
                                <span class="text-[8px] text-slate-400 uppercase block leading-none mb-2 font-black italic tracking-widest">Volume Mis</span>
                        ...
                        <div class="flex items-center gap-4 ml-auto" x-data="{ openAction: false }">
                            @if($inc->status == 'incubation')
                                <button @click.stop="openAction = true" class="px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white text-[10px] font-black uppercase rounded-2xl transition-all flex items-center gap-3 shadow-xl border-none cursor-pointer italic">
                                    <i class="fa-solid fa-magnifying-glass"></i> Mirage J-10
                                </button>
                                {{-- Modal Mirage --}}
                                <div x-show="openAction" x-cloak x-transition.opacity class="fixed inset-0 z-[150] flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4">
                                    <div @click.outside="openAction = false" class="bg-white p-12 rounded-[4rem] w-full max-w-sm text-center relative font-bold text-slate-800 italic shadow-2xl">
                                        ...
                                        <h3 class="font-black uppercase text-xl mb-6 leading-none italic tracking-tighter underline decoration-orange-500 decoration-4">🔍 Contrôle de Fertilité</h3>
                                        <form action="{{ route('incubations.mirage', $inc->id) }}" method="POST" class="space-y-6 text-left">
                                            @csrf
                                            <div class="space-y-2">
                                                <label class="text-[9px] font-black text-slate-400 uppercase ml-2 italic">Nombre d'œufs fertiles</label>
                                            ...
                                            <button type="submit" class="w-full bg-slate-900 text-white py-6 rounded-[1.5rem] font-black uppercase text-[11px] italic shadow-2xl border-none cursor-pointer hover:bg-orange-600 transition-all tracking-[0.2em]">Enregistrer Mirage</button>
                                        </form>
                                    </div>
                                </div>
                            @endif
                            ...
                            @if($inc->status == 'mirage_fait')
                                <button @click.stop="openAction = true" class="px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white text-[10px] font-black uppercase rounded-2xl transition-all flex items-center gap-3 shadow-xl border-none cursor-pointer italic">
                                    <i class="fa-solid fa-egg"></i> Clôture Éclosion
                                </button>
                                {{-- Modal Éclosion --}}
                                <div x-show="openAction" x-cloak x-transition.opacity class="fixed inset-0 z-[150] flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4">
                                    <div @click.outside="openAction = false" class="bg-white p-12 rounded-[4rem] w-full max-w-sm text-center relative font-bold text-slate-800 italic shadow-2xl">
                                        <button @click="openAction = false" type="button" class="absolute top-8 right-10 text-slate-200 hover:text-rose-500 border-none bg-transparent cursor-pointer"><i class="fa-solid fa-circle-xmark text-3xl"></i></button>
                                        <h3 class="font-black uppercase text-xl mb-6 text-emerald-600 leading-none italic tracking-tighter underline decoration-emerald-200 decoration-4">🐣 Bilan d'Éclosion</h3>
                                        <form action="{{ route('incubations.hatch', $inc->id) }}" method="POST" class="space-y-6 text-left">
                                            @csrf
                                            <div class="space-y-2">
                                                <label class="text-[9px] font-black text-slate-400 uppercase ml-2 italic">Poussins viables nés</label>
                                            ...
                                            <button type="submit" class="w-full bg-slate-900 text-white py-6 rounded-[1.5rem] font-black uppercase text-[11px] italic shadow-2xl border-none cursor-pointer hover:bg-emerald-600 transition-all tracking-[0.2em]">Clôturer le Cycle</button>
                                        </form>
                                    </div>
                                </div>
                            @endif
                            ...
                            @if($inc->status == 'clos' && ($inc->hatched_chicks ?? 0) > 0)
                                ...
                                @class([
                                    ...
                                    @if($chickRemaining > 0)
                                        <i class="fa-solid fa-paper-plane"></i> Dispatcher ({{ $chickRemaining }})
                                    @else
                                        <i class="fa-solid fa-check-double"></i> Dispatches ({{ $alreadyDispatched }})
                                    @endif
                                </a>
                            @endif

                            <form action="{{ route('incubations.destroy', $inc->id) }}" method="POST" onsubmit="return confirm('DÉCISION CRITIQUE : Annuler définitivement ce cycle ?')">
                                @csrf @method('DELETE')
                ...
                @empty
                    <div class="text-center py-24 bg-white rounded-[4rem] border-4 border-dashed border-slate-50 italic">
                        ...
                        <p class="text-[11px] font-black text-slate-300 uppercase tracking-[0.4em]">Flux de production à l'arrêt</p>
                    </div>
                @endforelse
            </div>
            ...
            <div class="mt-24 border-t border-slate-100 pt-16 text-left italic font-bold">
                <div class="flex items-center gap-6 mb-12">
                    ...
                    <div>
                        <h3 class="text-3xl font-black text-slate-800 uppercase leading-none tracking-tighter italic">Registre d'Accouvage</h3>
                        <p class="text-[11px] text-slate-400 font-black uppercase tracking-[0.3em] mt-2 italic leading-none">Archives de performance consolidées</p>
                    </div>
                </div>
                            ...
                            <button @click="activeMonth = (activeMonth === '{{ $month }}' ? null : '{{ $month }}')" 
                                ...
                                <div class="flex items-center gap-10">
                                    <div class="hidden md:flex items-center gap-6">
                                        <span class="text-[9px] text-slate-400 uppercase font-black italic">Moy. Éclosion : <span class="text-emerald-500 ml-1">{{ number_format($data['avg_hatchability'], 1) }}%</span></span>
                                        <span class="text-[9px] text-slate-400 uppercase font-black italic">Productivité : <span class="text-blue-500 ml-1">{{ number_format($data['total_chicks'], 0, ',', ' ') }}</span></span>
                                    </div>
                                    ...
                                    <table class="w-full text-[11px] font-black text-left italic">
                                        <thead>
                                            <tr class="text-[8px] text-slate-400 uppercase tracking-widest border-b border-slate-50 italic">
                                                <th class="py-5">Code Cycle</th>
                                                <th class="py-5 text-center">Volume Mis</th>
                                                <th class="py-5 text-center">Nés Viables</th>
                                                <th class="py-5 text-center">Succès Brute</th>
                                                <th class="py-5 text-right">Actions</th>
                                            </tr>
                                        </thead>
                                            ...
                                            @foreach($data['items'] as $item)
                                                <tr class="text-slate-600 hover:bg-slate-50 transition-colors">
                                                    <td class="py-5">
                                                        ...
                                                        <span class="text-[8px] text-slate-300 font-bold tracking-tighter italic uppercase">{{ $item->incubator->name ?? 'N/A' }}</span>
                                                    </td>
                                                        ...
                                                        <div class="flex items-center justify-end gap-2">
                                                            ...
                                                            @if(($item->hatched_chicks ?? 0) > 0)
                                                                ...
                                                                @class([
                                                                    ...
                                                                    @if($left > 0)
                                                                        <i class="fa-solid fa-paper-plane"></i> {{ $left }} restants
                                                                    @else
            ...
            <div @click.outside="openLaunch = false" 
                ...
                <form action="{{ route('incubations.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            <label class="text-[10px] font-black text-slate-400 uppercase italic ml-2">Unité d'Incubation</label>
                            <div class="relative">
                        ...
                        <div x-show="isExternal" x-cloak class="space-y-2">
                            <label class="text-[10px] font-black text-orange-500 uppercase italic ml-2">Fournisseur Externe</label>
                            <div class="relative">
                                <select name="provider_id" x-model="selectedProvider" :required="isExternal" class="w-full bg-orange-50 border border-orange-100 rounded-2xl px-5 py-4 font-black text-xs italic shadow-sm outline-none focus:ring-2 focus:ring-orange-500/50 transition-all appearance-none cursor-pointer text-orange-900">
                                    ...
                                    @endforeach
                                    <option value="new" class="font-black text-rose-600 bg-white">➕ NOUVEAU FOURNISSEUR...</option>
                                </select>
                    ...
                    <div x-show="isExternal && selectedProvider === 'new'" x-collapse>
                        <div class="bg-orange-50 p-6 rounded-[2rem] border border-orange-100 shadow-inner">
                            <div class="flex items-center gap-3 mb-4">
                                ...
                                <p class="text-[10px] font-black text-orange-700 uppercase tracking-widest italic leading-none">Création Express</p>
                            </div>
                            
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <input type="text" name="new_provider_name" placeholder="Nom de l'entreprise *" class="w-full bg-white border border-slate-100 rounded-xl px-4 py-3 font-black text-xs italic shadow-sm outline-none focus:ring-2 focus:ring-orange-500 text-slate-800" :required="selectedProvider === 'new'">
                                <input type="text" name="new_provider_phone" placeholder="Téléphone *" class="w-full bg-white border border-slate-100 rounded-xl px-4 py-3 font-black text-xs italic shadow-sm outline-none focus:ring-2 focus:ring-orange-500 text-slate-800" :required="selectedProvider === 'new'">
                                ...
                                <div class="relative">
                                    <select name="new_provider_type" class="w-full bg-white border border-slate-100 rounded-xl px-4 py-3 font-black text-xs italic shadow-sm outline-none focus:ring-2 focus:ring-orange-500 text-slate-800 appearance-none cursor-pointer" :required="selectedProvider === 'new'">
                                        <option value="Poussins">Type: Poussins/Œufs</option>
                    ...
                    <button type="submit" class="w-full bg-blue-600 text-white font-black py-6 rounded-[2rem] uppercase italic shadow-[0_15px_30px_-10px_rgba(37,99,235,0.5)] hover:bg-blue-500 hover:shadow-[0_20px_40px_-10px_rgba(37,99,235,0.6)] transition-all transform hover:-translate-y-1 active:scale-95 active:translate-y-0 flex items-center justify-center gap-3 tracking-[0.2em] text-xs border-none cursor-pointer mt-4">
                        <i class="fa-solid fa-power-off text-blue-200"></i> Initialiser la Production
                    </button>
                </form>
            </div>
        </div>
