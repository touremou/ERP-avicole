<x-app-layout>
                        ...
                        <div class="divide-y divide-slate-50">
                            ...
                            <div @class(['px-5 py-3 flex items-center gap-4 transition-all group',
                                ...
                                <div class="flex items-center gap-3 shrink-0">
                                    ...
                                    {{-- NOUVEAU : Boutons Modifier & Supprimer --}}
                                    <div class="flex items-center gap-1 border-l border-slate-100 pl-3">
                                        <button type="button" 
                                            @click="openEdit('{{ $task->id }}', '{{ addslashes($task->title) }}', '{{ $task->category }}', '{{ $task->priority }}', '{{ $task->scheduled_time ? \Carbon\Carbon::parse($task->scheduled_time)->format('H:i') : '' }}', '{{ $task->employee_id }}', '{{ $task->building_id }}')" 
                                            class="w-7 h-7 rounded text-slate-400 hover:bg-blue-50 hover:text-blue-500 transition-all border-none bg-transparent cursor-pointer">
                                            <i class="fa-solid fa-pen text-[10px]"></i>
                                        </button>

                                        <form method="POST" action="{{ route('tasks.destroy', $task) }}" class="m-0" onsubmit="return confirm('Voulez-vous vraiment supprimer cette tâche ?')">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="w-7 h-7 rounded text-slate-400 hover:bg-red-50 hover:text-red-500 transition-all border-none bg-transparent cursor-pointer">
    ...
    {{-- MODALE D'ÉDITION (Cachée par défaut) --}}
        <div x-show="isEditing" class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" x-cloak>
            <div class="bg-white w-full max-w-md rounded-[2rem] shadow-2xl p-6 text-left relative" @click.away="isEditing = false">
                <button @click="isEditing = false" class="absolute top-4 right-4 text-slate-400 hover:text-slate-800 border-none bg-transparent cursor-pointer"><i class="fa-solid fa-xmark text-xl"></i></button>
                
                <h3 class="text-sm font-black text-slate-800 uppercase italic tracking-tighter mb-4"><i class="fa-solid fa-pen text-blue-500 mr-2"></i> Modifier la tâche</h3>
                
                <form :action="'/tasks/' + editForm.id" method="POST" class="space-y-3">
                    @csrf
                    @method('PUT')
                    
                    <input type="text" name="title" x-model="editForm.title" required class="w-full bg-slate-50 border-none rounded-xl p-3 text-xs font-black shadow-inner outline-none">
                    
                    <div class="grid grid-cols-2 gap-3">
                        <select name="category" x-model="editForm.category" required class="w-full bg-slate-50 border-none rounded-xl p-2.5 text-[10px] font-black uppercase shadow-inner outline-none">
                            <option value="alimentation">🌾 Alimentation</option>
                            <option value="collecte">🥚 Collecte</option>
                            <option value="controle">📋 Contrôle</option>
                            <option value="nettoyage">🧹 Nettoyage</option>
                        ...
                        <select name="priority" x-model="editForm.priority" required class="w-full bg-slate-50 border-none rounded-xl p-2.5 text-[10px] font-black uppercase shadow-inner outline-none">
                            <option value="normale">Normale</option>
                            <option value="haute">Haute</option>
                            <option value="critique">Critique</option>
                            <option value="basse">Basse</option>
                    ...
                    <select name="employee_id" x-model="editForm.employee" class="w-full bg-slate-50 border-none rounded-xl p-2.5 text-[10px] font-black shadow-inner outline-none">
                        <option value="">Employé (Non assigné)</option>
                        @foreach($employees as $e)<option value="{{ $e->id }}">{{ $e->first_name }} {{ $e->last_name }}</option>@endforeach
                    </select>
                    
                    <div class="grid grid-cols-2 gap-3">
                        <select name="building_id" x-model="editForm.building" class="w-full bg-slate-50 border-none rounded-xl p-2.5 text-[10px] font-black shadow-inner outline-none">
                            <option value="">Bâtiment...</option>
                            @foreach($buildings as $b)<option value="{{ $b->id }}">{{ $b->name }}</option>@endforeach
                        </select>
                        <input type="time" name="scheduled_time" x-model="editForm.time" class="w-full bg-slate-50 border-none rounded-xl p-2.5 text-xs font-black shadow-inner outline-none">
                    </div>

                    <button type="submit" class="w-full bg-blue-600 text-white py-3 mt-4 rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-slate-900 border-none cursor-pointer italic transition-all">Mettre à jour</button>
                </form>
            </div>
        </div>
    ...
    <script>
        function taskBoard() { 
            return {
                isEditing: false,
                editForm: { id: '', title: '', category: '', priority: '', time: '', employee: '', building: '' },
                
                openEdit(id, title, cat, prio, time, emp, build) {
                    this.editForm = { id: id, title: title, category: cat, priority: prio, time: time, employee: emp, building: build };
                    this.isEditing = true;
                }
            } 
        }
