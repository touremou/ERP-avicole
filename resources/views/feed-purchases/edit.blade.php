    ...
    <x-slot name="header">
        <div class="flex items-center gap-4 text-left">
            ...
            <h2 class="text-xl font-black text-slate-800 uppercase italic tracking-tighter leading-none">
                <i class="fa-solid fa-pen-to-square text-orange-500 mr-2"></i> Modifier Ravitaillement
            </h2>
        </div>
    </x-slot>
        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 text-left">
            ...
            <form action="{{ route('feed-purchases.update', $feedPurchase->id) }}" method="POST" 
                ...
                <div class="flex items-center gap-3 px-5 py-2.5 bg-slate-900 rounded-full w-fit border border-slate-800 mb-6 shadow-lg">
                    ...
                    <span class="text-[9px] uppercase font-black tracking-[0.2em] text-white italic">Secteur {{ $batch->type }} • Bande {{ $batch->code }}</span>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {{-- DÉSIGNATION DE L'ARTICLE (L) --}}
                    <div class="space-y-3">
                        <label class="block text-[10px] font-black text-slate-400 uppercase ml-2 italic tracking-widest">Désignation de l'Article</label>
                        <select name="feed_type" required class="w-full bg-slate-50 border-none rounded-2xl p-5 text-sm font-black uppercase focus:ring-4 focus:ring-blue-500/10 shadow-inner appearance-none transition-all italic text-slate-800 cursor-pointer">
                            ...
                            @if(!in_array($currentValue, $phases) && !empty($currentValue))
                                <option value="{{ $currentValue }}" selected>⭐ {{ strtoupper($currentValue) }} (Valeur actuelle)</option>
                            @endif
                        </select>
                    </div>
                    ...
                    {{-- DATE ACHAT (M) --}}
                    <div class="space-y-3">
                        <label class="block text-[10px] font-black text-slate-400 uppercase ml-2 italic tracking-widest">Date de l'opération</label>
                ...
                {{-- FOURNISSEUR --}}
                <div class="space-y-3">
                    <label class="block text-[10px] font-black text-slate-400 uppercase ml-2 italic tracking-widest">Fournisseur Partenaire</label>
                ...
                <div class="grid grid-cols-2 gap-8 bg-slate-900 p-8 rounded-[3rem] shadow-xl relative overflow-hidden">
                    ...
                    <div class="relative z-10">
                        <label class="block text-[10px] font-black text-blue-400 uppercase mb-4 text-center italic tracking-widest">Poids Total (KG)</label>
                        ...
                        <p class="text-[8px] text-slate-500 mt-3 text-center uppercase font-black italic">Précision à 1 gramme (± 0.001)</p>
                    </div>

                    {{-- MONTANT TOTAL (GNF) --}}
                    <div class="relative z-10">
                        <label class="block text-[10px] font-black text-slate-500 uppercase mb-4 text-center italic tracking-widest">Montant Payé (GNF)</label>
                        ...
                        <p class="text-[8px] text-emerald-500 mt-3 text-center uppercase font-black italic">Coût : <span x-text="unitCost.toLocaleString()"></span> GNF / KG</p>
                    </div>
