            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-[9px] font-bold text-slate-400 uppercase mt-1 tracking-widest italic">
                        {{ $period->start_date->format('d/m') }} → {{ $period->end_date->format('d/m/Y') }} — {{ ucfirst($period->status) }}
                    </p>
                </div>
            </div>
    ...
    <div class="py-8 italic font-bold" x-data="payrollUI()">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm text-center">
                    <p class="text-[7px] font-black text-slate-400 uppercase tracking-widest">Employés</p>
                    <p class="text-2xl font-black text-slate-900">{{ $kpi['total_employees'] }}</p>
                </div>
                <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm text-center">
                    <p class="text-[7px] font-black text-blue-500 uppercase tracking-widest">Masse brute</p>
                ...
                <div class="bg-emerald-50 p-4 rounded-2xl border border-emerald-200 shadow-sm text-center">
                    <p class="text-[7px] font-black text-emerald-500 uppercase tracking-widest">Net à payer</p>
                    <p class="text-lg font-black text-emerald-600">{{ number_format($kpi['total_net'], 0, ',', '.') }}</p>
                </div>
                <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm text-center">
                    <p class="text-[7px] font-black text-slate-400 uppercase tracking-widest">Payés</p>
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-slate-50 text-[7px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                            ...
                            <th class="px-3 py-3 text-right">Déductions</th>
                            <th class="px-3 py-3 text-right">Net</th>
                            <th class="px-3 py-3 text-center">Jours</th>
                            <th class="px-3 py-3 text-center">Paiement</th>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-all">
                            ...
                            <td class="px-3 py-3 text-center text-[9px] font-black text-slate-500">
                                {{ $slip->days_worked }}j
                                @if($slip->days_absent > 0)<span class="text-red-500 ml-1">-{{ $slip->days_absent }}abs</span>@endif
                            </td>
                            <td class="px-3 py-3 text-center">
                                <span @class(['text-[8px] font-black uppercase px-2 py-1 rounded-full',
                                    ...
                                    {{ $slip->payment_status === 'paye' ? '✓ Payé' : 'En attente' }}
                                </span>
                            </td>
                            <td class="px-5 py-3 text-center">
                                    <div class="flex items-center justify-center gap-1.5">
                                        ...
                                        <a href="{{ route('payroll.print', ['payslip' => $slip, 'type' => $slip->payment_status === 'paye' ? 'fiche' : 'bon']) }}" target="_blank"
                                            class="w-7 h-7 rounded-lg bg-slate-50 text-slate-400 hover:bg-blue-50 hover:text-blue-600 flex items-center justify-center no-underline transition-all" title="{{ $slip->payment_status === 'paye' ? 'Fiche de paie' : 'Bon de paie' }}">
        ...
        <div x-show="lineModal" x-transition class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" x-cloak>
            <div class="bg-white rounded-2xl w-full max-w-md p-8 text-left font-bold italic" @click.outside="lineModal = false">
                <h3 class="text-lg font-black text-slate-800 uppercase tracking-tighter mb-4" x-text="'Prime/Déduction — ' + lineEmployee"></h3>
                <form :action="'/payroll/payslip/' + lineSlipId + '/line'" method="POST" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <label class="cursor-pointer"><input type="radio" name="type" value="prime" class="hidden peer" checked>
                            <div class="peer-checked:bg-emerald-50 peer-checked:border-emerald-400 bg-slate-50 border-2 border-transparent rounded-xl p-3 text-center transition-all text-[9px] font-black uppercase">✅ Prime</div></label>
                        <label class="cursor-pointer"><input type="radio" name="type" value="deduction" class="hidden peer">
                            <div class="peer-checked:bg-red-50 peer-checked:border-red-400 bg-slate-50 border-2 border-transparent rounded-xl p-3 text-center transition-all text-[9px] font-black uppercase">➖ Déduction</div></label>
                    </div>
                    <input type="text" name="label" required placeholder="Ex: Prime performance, Avance..." class="w-full bg-slate-50 border-none rounded-xl p-3 text-xs font-black shadow-inner outline-none">
                    <input type="number" name="amount" required min="1" placeholder="Montant GNF" class="w-full bg-slate-50 border-none rounded-xl p-3 text-lg font-black shadow-inner outline-none text-center">
                    <select name="category" class="w-full bg-slate-50 border-none rounded-xl p-3 text-xs font-black uppercase shadow-inner outline-none italic">
                        ...
                        <option value="nuit">Nuit / Astreinte</option>
                    ...
                    <button type="submit" class="w-full bg-slate-900 text-white py-4 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-600 border-none cursor-pointer italic">Ajouter</button>
                </form>
            </div>
        </div>
        ...
        <div x-show="payModal" x-transition class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" x-cloak>
            <div class="bg-white rounded-2xl w-full max-w-md p-8 text-left font-bold italic" @click.outside="payModal = false">
                <h3 class="text-lg font-black text-emerald-600 uppercase tracking-tighter mb-2">💰 Paiement</h3>
                ...
                <form :action="'/payroll/payslip/' + paySlipId + '/pay'" method="POST" class="space-y-4">
                    ...
                    <button type="submit" class="w-full bg-emerald-500 text-white py-4 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-600 border-none cursor-pointer italic">Confirmer le paiement</button>
                </form>
            </div>
        </div>
    </div>
