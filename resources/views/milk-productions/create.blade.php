<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="text-xl font-black text-slate-800 uppercase italic tracking-tighter leading-none">🐐 Collecte — {{ $batch->code }}</h2>
        </div>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            @if($existingToday)
                <div class="mb-6 p-5 bg-amber-50 text-amber-700 rounded-[2rem] text-[10px] font-black uppercase tracking-widest border border-amber-200">
                    <i class="fa-solid fa-circle-info mr-1"></i> Une collecte existe déjà pour ce lot aujourd'hui — l'enregistrement la mettra à jour.
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('milk-productions.store') }}">
                ...
                <div class="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 space-y-8">
                    <div>
                        <label class="block text-[10px] font-black text-slate-500 uppercase mb-2 ml-1 italic">Date de collecte *</label>
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <label class="block text-[10px] font-black text-emerald-500 uppercase mb-2 ml-1 italic">Traite matin (L)</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-black text-emerald-500 uppercase mb-2 ml-1 italic">Traite soir (L)</label>
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <label class="block text-[10px] font-black text-amber-500 uppercase mb-2 ml-1 italic">Prix / litre (GNF)</label>
                            ...
                            @if($lastPrice && !$existingToday)
                                <p class="text-[8px] text-slate-300 ml-2 uppercase font-bold mt-1">Dernier prix connu pré-rempli</p>
                            @endif
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-slate-500 uppercase mb-2 ml-1 italic">Femelles traites</label>
                            <input type="number" step="1" min="0" name="milking_females" value="{{ old('milking_females', $existingToday?->milking_females ?? '') }}" placeholder="Optionnel"
                                   class="w-full p-4 bg-slate-50 rounded-2xl border-none font-black text-slate-700 shadow-inner italic outline-none">
                        </div>
                    </div>
                    ...
                    <div class="bg-slate-900 p-6 rounded-[2rem] text-white flex justify-between items-center">
                        <div>
                            <p class="text-[9px] font-black text-emerald-400 uppercase tracking-widest">Total collecté</p>
                            <p class="text-3xl font-black italic" x-text="(morning + evening).toFixed(1) + ' L'"></p>
                        </div>
                        <div class="text-right">
                            <p class="text-[9px] font-black text-amber-400 uppercase tracking-widest">Valorisation</p>
                    ...
                    <button type="submit" class="w-full bg-emerald-600 text-white font-black py-6 rounded-[2rem] hover:bg-emerald-700 transition-all uppercase tracking-[0.3em] text-[10px] italic shadow-2xl border-none cursor-pointer">
                        <i class="fa-solid fa-droplet mr-2"></i> Enregistrer la collecte
                    </button>
                </div>
            </form>
        </div>
