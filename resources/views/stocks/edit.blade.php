    ...
    <x-slot name="header">
        <div class="flex justify-between items-center text-left">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-2xl font-black text-slate-800 uppercase italic tracking-tighter leading-none">Ajustement Fiche</h2>
            ...
            @can('stocks.M')
                @if ($errors->any())
                    <div class="mb-6 p-6 bg-red-600 text-white rounded-[2.5rem] shadow-lg text-[10px] font-black uppercase italic animate-pulse">
                        <p class="mb-2 border-b border-white/20 pb-2">⚠️ Erreur(s) détectée(s) :</p>
                ...
                <div class="bg-white p-12 rounded-[4rem] shadow-xl border border-slate-100 relative overflow-hidden">
                    <form action="{{ route('stocks.update', $stock->id) }}" method="POST" class="space-y-8 relative z-10">
                        ...
                        <div>
                            <label class="text-[10px] uppercase text-slate-400 ml-6 mb-2 block tracking-widest font-black italic">Désignation de l'article</label>
                            @if($isSynced)
                        ...
                        <div class="bg-slate-900 p-8 rounded-[3rem] shadow-2xl relative">
                            <label class="text-[10px] uppercase text-emerald-400 mb-3 block tracking-widest font-black italic leading-none">⚖️ Correction de l'Inventaire Physique</label>
                            <div class="flex items-center gap-4">
                            ...
                            <template x-if="unit === 'Alvéole'">
                                <div class="mt-4 p-4 bg-white/5 rounded-2xl border border-white/5 flex justify-between items-center">
                                    <p class="text-[9px] text-slate-500 uppercase italic font-black">Lecture inventaire :</p>
                                    <p class="text-xs text-blue-400 font-black italic tracking-tighter uppercase">
                                        <span x-text="Math.floor(qty)"></span> Plateaux + 
                                        <span x-text="Math.round((qty - Math.floor(qty)) * setting('general.eggs_per_tray', 30))"></span> Œufs
                                    </p>
                                </div>
                            </template>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
                            {{-- UNITÉ --}}
                            <div>
                                <label class="text-[10px] uppercase text-slate-400 ml-6 mb-2 block tracking-widest font-black italic">Unité de mesure</label>
                            ...
                            {{-- SEUIL D'ALERTE --}}
                            <div>
                                <label class="text-[10px] uppercase text-slate-400 ml-6 mb-2 block tracking-widest font-black italic">Seuil critique d'alerte</label>
                            ...
                            <div>
                                <label class="text-[10px] uppercase text-slate-400 ml-6 mb-2 block tracking-widest font-black italic">Prix Unitaire (GNF)</label>
