        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 italic font-bold text-left">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                ...
                <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm text-center">
                    <p class="text-[8px] font-black text-blue-500 uppercase tracking-widest mb-2">
                        <i class="fa-solid fa-receipt mr-1"></i> {{ __("Transactions") }}
                    </p>
            ...
            <form method="GET" class="mb-8 flex flex-wrap gap-3 items-center">
                <select name="method" class="bg-white border border-slate-100 rounded-2xl px-4 py-3 text-[10px] font-black uppercase tracking-widest shadow-sm outline-none appearance-none">
                    <option value="">{{ __("Tous modes") }}</option>
                    <option value="especes" {{ request('method') === 'especes' ? 'selected' : '' }}>{{ __("Espèces") }}</option>
                    <option value="orange_money" {{ request('method') === 'orange_money' ? 'selected' : '' }}>{{ __("Orange Money") }}</option>
                    <option value="virement" {{ request('method') === 'virement' ? 'selected' : '' }}>{{ __("Virement") }}</option>
                    <option value="cheque" {{ request('method') === 'cheque' ? 'selected' : '' }}>{{ __("Chèque") }}</option>
                </select>
                <div class="flex items-center gap-2">
                    <span class="text-[9px] font-black text-slate-400 uppercase">{{ __("Du") }}</span>
                    <input type="date" name="date_from" value="{{ request('date_from') }}"
                        class="bg-white border border-slate-100 rounded-2xl px-4 py-3 text-[10px] font-black shadow-sm outline-none">
                    <span class="text-[9px] font-black text-slate-400 uppercase">{{ __("au") }}</span>
                ...
                @if(request()->hasAny(['method', 'date_from', 'date_to']))
                    <a href="{{ route('payments.index') }}" class="text-[9px] font-black text-slate-400 uppercase tracking-widest no-underline hover:text-slate-700">
                        <i class="fa-solid fa-xmark mr-1"></i> {{ __("Réinitialiser") }}
                    </a>
                @endif
            </form>
                    ...
                    <table class="w-full border-collapse">
                        <thead>
                            <tr class="bg-slate-50 text-[8px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 italic">
                                ...
                                <th class="px-4 py-5 text-left">{{ __("Vente") }}</th>
                                <th class="px-4 py-5 text-left">{{ __("Client") }}</th>
                                <th class="px-4 py-5 text-center">{{ __("Mode") }}</th>
                                <th class="px-4 py-5 text-left">{{ __("Référence") }}</th>
                                <th class="px-4 py-5 text-right">{{ __("Montant") }}</th>
                                <th class="px-6 py-5 text-left">{{ __("Reçu par") }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-50">
                            ...
                            <tr>
                                <td colspan="7" class="px-8 py-16 text-center">
                                    ...
                                    <p class="text-[10px] text-slate-400 uppercase tracking-widest font-black">{{ __("Aucun paiement enregistré") }}</p>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
