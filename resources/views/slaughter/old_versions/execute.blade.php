<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-5 text-left">
            ...
            <div>
                <h2 class="font-black text-2xl text-slate-800 leading-none uppercase italic tracking-tighter">Exécution Abattage</h2>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 italic font-bold text-left" x-data="slaughterForm()" x-cloak>
            ...
            <div class="bg-rose-50 p-6 rounded-[2.5rem] border border-rose-200 mb-6">
                <div class="grid grid-cols-3 gap-4 text-center">
                    <div><p class="text-[8px] font-black text-rose-400 uppercase">Lot</p><p class="text-sm font-black text-slate-900">{{ $order->batch->code ?? '—' }}</p></div>
                    <div><p class="text-[8px] font-black text-rose-400 uppercase">Prévu</p><p class="text-sm font-black text-slate-900">{{ $order->planned_quantity }} sujets</p></div>
                    <div><p class="text-[8px] font-black text-rose-400 uppercase">Bâtiment</p><p class="text-sm font-black text-slate-900">{{ $order->batch->building->name ?? '—' }}</p></div>
                </div>
            </div>
            ...
            <form method="POST" action="{{ route('slaughter.execute.store', $order) }}">
                ...
                <div class="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm mb-6">
                    <h3 class="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-6 flex items-center gap-2"><i class="fa-solid fa-scale-balanced text-rose-500"></i> Pesées & Résultats</h3>
                    <div class="grid grid-cols-2 gap-6 mb-6">
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Sujets abattus *</label>
                        ...
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Date d'exécution *</label>
                    ...
                    <div class="grid grid-cols-2 gap-6 mb-6">
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-emerald-600 tracking-widest ml-2">Poids vif total (kg) *</label>
                        ...
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-blue-600 tracking-widest ml-2">Poids carcasse total (kg) *</label>
                    ...
                    <div class="grid grid-cols-3 gap-4 mb-6" x-show="liveWeight > 0 && carcassWeight > 0">
                        <div class="bg-slate-50 p-4 rounded-2xl text-center">
                            <p class="text-[8px] font-black text-slate-400 uppercase">Rendement carcasse</p>
                            <p class="text-xl font-black" :class="yieldPercent >= 70 ? 'text-emerald-600' : (yieldPercent >= 65 ? 'text-amber-600' : 'text-red-600')" x-text="yieldPercent + '%'"></p>
                            <p class="text-[8px] text-slate-400">norme : 70-75%</p>
                        </div>
                        <div class="bg-slate-50 p-4 rounded-2xl text-center">
                            <p class="text-[8px] font-black text-slate-400 uppercase">Poids moyen vif</p>
                            <p class="text-xl font-black text-slate-900" x-text="avgLive + ' kg'"></p>
                        </div>
                        <div class="bg-slate-50 p-4 rounded-2xl text-center">
                            <p class="text-[8px] font-black text-slate-400 uppercase">Poids moyen carcasse</p>
                            <p class="text-xl font-black text-slate-900" x-text="avgCarcass + ' kg'"></p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-red-500 tracking-widest ml-2">Saisies sanitaires</label>
                        ...
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Motif de saisie</label>
                            <input type="text" name="condemned_reason" placeholder="Abcès, cachexie..." class="w-full bg-slate-50 border-none rounded-2xl p-4 text-xs font-black shadow-inner outline-none">
                        </div>
                    </div>
                    <div class="mt-6 space-y-2">
                        <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Notes inspecteur</label>
                        <textarea name="inspector_notes" rows="2" placeholder="Observations sanitaires..." class="w-full bg-slate-50 border-none rounded-2xl p-4 text-xs font-bold shadow-inner outline-none"></textarea>
                    </div>
                </div>
                ...
                <button type="submit" class="w-full bg-rose-500 text-white py-6 rounded-[2rem] font-black text-xs uppercase tracking-[0.3em] hover:bg-rose-600 transition-all shadow-2xl italic border-none cursor-pointer">
                    <i class="fa-solid fa-check-double mr-2"></i> Valider l'Abattage & Mettre en Stock
                </button>
            </form>
        </div>
    </div>
    ...
    function slaughterForm() {
        return {
            actualQty: {{ $order->planned_quantity }}, liveWeight: 0, carcassWeight: 0,
