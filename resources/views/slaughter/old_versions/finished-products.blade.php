    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 text-left">
            <div class="flex items-center gap-5">
                ...
                <div>
                    <h2 class="font-black text-2xl text-slate-800 leading-none uppercase italic tracking-tighter">Produits Finis</h2>
                    <p class="text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] mt-2 italic">Stock abattoir — Frais, Congelé, Transformé</p>
                </div>
            </div>
            <div class="flex gap-3">
                <a href="{{ route('slaughter.orders.create') }}" class="bg-rose-500 text-white px-5 py-3 rounded-2xl font-black text-[9px] uppercase tracking-widest hover:bg-rose-600 transition-all shadow-lg italic no-underline"><i class="fa-solid fa-plus mr-1"></i> Abattage</a>
                <a href="{{ route('slaughter.transform.form') }}" class="bg-amber-500 text-white px-5 py-3 rounded-2xl font-black text-[9px] uppercase tracking-widest hover:bg-amber-600 transition-all shadow-lg italic no-underline"><i class="fa-solid fa-fire mr-1"></i> Transformation</a>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-10" x-data="fpManager()">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 italic font-bold text-left">
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
                <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm text-center">
                    <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Stock total</p>
                    <p class="text-2xl font-black text-slate-900">{{ number_format($kpi['total_kg'], 1) }} <small class="text-xs text-slate-400">kg</small></p>
                </div>
                <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm text-center">
                    <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Pièces</p>
                    <p class="text-2xl font-black text-slate-900">{{ number_format($kpi['total_pieces']) }}</p>
                </div>
                <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm text-center">
                    <p class="text-[8px] font-black text-emerald-500 uppercase tracking-widest mb-1">Valeur stock</p>
                    <p class="text-xl font-black text-emerald-600">{{ number_format($kpi['total_value'], 0, ',', '.') }} <small class="text-[8px]">GNF</small></p>
                </div>
                <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm text-center">
                    <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Types</p>
                ...
                <div @class(['p-5 rounded-[2rem] border shadow-sm text-center',
                    ...
                    <p class="text-[8px] font-black text-red-500 uppercase tracking-widest mb-1">Périmés</p>
            ...
            <div class="mb-6 p-5 bg-red-50 border border-red-200 rounded-[2rem]">
                <p class="text-[9px] font-black text-red-600 uppercase tracking-widest mb-2"><i class="fa-solid fa-clock mr-1"></i> Proches de la péremption</p>
                @foreach($expiring as $ep)
                    <p class="text-[9px] text-red-700">{{ $ep->product_name }} — expire le {{ $ep->expiry_date->format('d/m/Y') }} — {{ number_format($ep->current_quantity_kg, 1) }} kg</p>
                @endforeach
            </div>
            @endif
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-slate-50 text-[8px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 italic">
                            ...
                            <th class="px-3 py-4 text-center">Type</th>
                            <th class="px-3 py-4 text-center">Lieu</th>
                            <th class="px-3 py-4 text-right">Quantité</th>
                            <th class="px-3 py-4 text-right">Prix/kg</th>
                            <th class="px-3 py-4 text-right">Valeur</th>
                            <th class="px-3 py-4 text-center">Péremption</th>
                            <th class="px-3 py-4 text-center">État</th>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr @class(['hover:bg-slate-50/50 transition-all',
                            ...
                            <td class="px-5 py-4">
                                ...
                                @if($fp->batch_reference)<p class="text-[7px] text-slate-400">Réf: {{ $fp->batch_reference }}</p>@endif
                            </td>
                            ...
                            <td class="px-3 py-4 text-right text-[10px] font-black text-slate-600">
                                {{ $fp->unit_price > 0 ? number_format($fp->unit_price, 0, ',', '.') . ' GNF' : '—' }}
                            </td>
                            ...
                            <td class="px-3 py-4 text-center text-[10px] font-black">
                                @if($fp->expiry_date)
                                    <span class="{{ $fp->is_expired ? 'text-red-600' : ($fp->is_expiring_soon ? 'text-amber-600' : 'text-slate-500') }}">
                                        {{ $fp->expiry_date->format('d/m/Y') }}
                                    </span>
                                @else
                                    <span class="text-slate-300">Non défini</span>
                                @endif
                            </td>
                            <td class="px-3 py-4 text-center">
                                @if($fp->is_expired)
                                    <span class="text-[8px] font-black text-red-600 bg-red-50 px-2 py-1 rounded-full animate-pulse">PÉRIMÉ</span>
                                @elseif($fp->is_expiring_soon)
                                    <span class="text-[8px] font-black text-amber-600 bg-amber-50 px-2 py-1 rounded-full">URGENT</span>
                                @elseif($fp->is_low)
                                    <span class="text-[8px] font-black text-red-600 bg-red-50 px-2 py-1 rounded-full">BAS</span>
                                @elseif($fp->current_quantity_kg > 0)
                                    <span class="text-[8px] font-black text-emerald-600 bg-emerald-50 px-2 py-1 rounded-full">OK</span>
                                @else
                                    <span class="text-[8px] font-black text-slate-300">VIDE</span>
                                @endif
                            </td>
                            <td class="px-5 py-3 text-center">
                                ...
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <button @click="openEdit({{ $fp->id }}, '{{ addslashes($fp->product_name) }}', {{ $fp->unit_price }}, '{{ $fp->expiry_date?->format('Y-m-d') ?? '' }}', {{ $fp->alert_threshold_kg }}, '{{ $fp->storage_location }}')"
                                        class="w-8 h-8 rounded-lg bg-slate-50 text-slate-400 hover:bg-blue-50 hover:text-blue-600 flex items-center justify-center border-none cursor-pointer transition-all" title="Modifier">
                                    ...
                                    <button @click="openTransfer({{ $fp->id }}, '{{ addslashes($fp->product_name) }}', {{ $fp->current_quantity_kg }})"
                                        class="w-8 h-8 rounded-lg bg-slate-50 text-slate-400 hover:bg-emerald-50 hover:text-emerald-600 flex items-center justify-center border-none cursor-pointer transition-all" title="→ Stock magasin">
                                    ...
                                    <button @click="openAdjust({{ $fp->id }}, '{{ addslashes($fp->product_name) }}', {{ $fp->current_quantity_kg }})"
                                        class="w-8 h-8 rounded-lg bg-slate-50 text-slate-400 hover:bg-amber-50 hover:text-amber-600 flex items-center justify-center border-none cursor-pointer transition-all" title="Ajuster quantité">
                                    ...
                                    <button @click="openDispose({{ $fp->id }}, '{{ addslashes($fp->product_name) }}', {{ $fp->current_quantity_kg }})"
                                        class="w-8 h-8 rounded-lg bg-slate-50 text-slate-400 hover:bg-red-50 hover:text-red-600 flex items-center justify-center border-none cursor-pointer transition-all" title="Éliminer">
                        ...
                        <tr>
                            <td colspan="9" class="px-8 py-16 text-center">
                                ...
                                <p class="text-[10px] text-slate-400 uppercase tracking-widest font-black">Aucun produit fini</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="mt-4 flex flex-wrap gap-4 justify-center">
                <span class="text-[8px] font-black text-slate-400 uppercase flex items-center gap-1"><i class="fa-solid fa-pen text-blue-400"></i> Modifier</span>
                <span class="text-[8px] font-black text-slate-400 uppercase flex items-center gap-1"><i class="fa-solid fa-arrow-right-from-bracket text-emerald-400"></i> → Magasin</span>
                <span class="text-[8px] font-black text-slate-400 uppercase flex items-center gap-1"><i class="fa-solid fa-scale-balanced text-amber-400"></i> Ajuster</span>
                <span class="text-[8px] font-black text-slate-400 uppercase flex items-center gap-1"><i class="fa-solid fa-trash text-red-400"></i> Éliminer</span>
            </div>
        </div>
        ...
        <div x-show="editModal" x-transition class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" x-cloak>
            <div class="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl p-8 text-left italic font-bold" @click.outside="editModal = false">
                <h3 class="text-lg font-black text-slate-800 uppercase tracking-tighter mb-6" x-text="'Modifier — ' + editName"></h3>
                <form :action="'/slaughter/finished-products/' + editId" method="POST">
                    @csrf @method('PUT')
                    <div class="space-y-4">
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Prix / kg (GNF)</label>
                        ...
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Seuil d'alerte (kg)</label>
                        ...
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Lieu de stockage</label>
                            <select name="storage_location" x-model="editLocation" class="w-full bg-slate-50 border-none rounded-2xl p-4 text-xs font-black uppercase shadow-inner outline-none">
                                ...
                                <option value="fumoir">Fumoir</option>
        ...
        <div x-show="transferModal" x-transition class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" x-cloak>
            <div class="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl p-8 text-left italic font-bold" @click.outside="transferModal = false">
                <h3 class="text-lg font-black text-emerald-600 uppercase tracking-tighter mb-2">→ Transfert Magasin</h3>
                <p class="text-[9px] text-slate-500 mb-6 normal-case" x-text="transferName + ' — ' + transferMax.toFixed(1) + ' kg disponibles'"></p>
                <form :action="'/slaughter/finished-products/' + transferId + '/transfer'" method="POST">
                    @csrf
                    <div class="space-y-2 mb-4">
                        <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Quantité à transférer (kg)</label>
                    ...
                    <button type="button" @click="transferQty = transferMax" class="w-full mb-4 bg-slate-100 text-slate-600 py-2 rounded-xl text-[8px] font-black uppercase border-none cursor-pointer hover:bg-slate-200">Tout transférer</button>
                    <button type="submit" class="w-full bg-emerald-500 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-600 transition-all border-none cursor-pointer italic">
                        <i class="fa-solid fa-arrow-right-from-bracket mr-2"></i> Transférer au Stock Magasin
                    </button>
                </form>
            </div>
        </div>
        ...
        <div x-show="adjustModal" x-transition class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" x-cloak>
            <div class="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl p-8 text-left italic font-bold" @click.outside="adjustModal = false">
                <h3 class="text-lg font-black text-amber-600 uppercase tracking-tighter mb-2">Ajustement Inventaire</h3>
                <p class="text-[9px] text-slate-500 mb-6 normal-case" x-text="adjustName + ' — Actuel : ' + adjustCurrent.toFixed(1) + ' kg'"></p>
                <form :action="'/slaughter/finished-products/' + adjustId + '/adjust'" method="POST">
                    @csrf
                    <div class="space-y-4">
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Nouvelle quantité (kg)</label>
                        ...
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-red-500 tracking-widest ml-2">Raison de l'ajustement *</label>
                            <input type="text" name="reason" required placeholder="Inventaire physique, perte, casse..." class="w-full bg-slate-50 border-none rounded-2xl p-4 text-xs font-black shadow-inner outline-none">
                        </div>
                    </div>
                    <button type="submit" class="w-full mt-4 bg-amber-500 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-amber-600 transition-all border-none cursor-pointer italic">
                        <i class="fa-solid fa-scale-balanced mr-2"></i> Appliquer l'Ajustement
                    </button>
                </form>
            </div>
        </div>
        ...
        <div x-show="disposeModal" x-transition class="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" x-cloak>
            <div class="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl p-8 text-left italic font-bold" @click.outside="disposeModal = false">
                <h3 class="text-lg font-black text-red-600 uppercase tracking-tighter mb-2">⚠ Élimination</h3>
                <p class="text-[9px] text-slate-500 mb-6 normal-case" x-text="disposeName + ' — ' + disposeQty.toFixed(1) + ' kg seront éliminés'"></p>
                <form :action="'/slaughter/finished-products/' + disposeId + '/dispose'" method="POST">
                    @csrf
                    <div class="p-4 bg-red-50 rounded-2xl mb-4">
                        <p class="text-[9px] font-black text-red-600"><i class="fa-solid fa-triangle-exclamation mr-1"></i> Cette action est irréversible. Tout le stock sera mis à zéro.</p>
                    </div>
                    <div class="space-y-2 mb-4">
                        <label class="text-[9px] font-black uppercase text-red-500 tracking-widest ml-2">Motif d'élimination *</label>
                        <input type="text" name="reason" required placeholder="Périmé, avarié, contamination..." class="w-full bg-slate-50 border-none rounded-2xl p-4 text-xs font-black shadow-inner outline-none">
                    </div>
                    <button type="submit" class="w-full bg-red-600 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-red-700 transition-all border-none cursor-pointer italic">
                        <i class="fa-solid fa-trash mr-2"></i> Confirmer l'Élimination
                    </button>
                </form>
            </div>
        </div>
