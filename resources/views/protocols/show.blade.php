        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div class="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 h-fit sticky top-8 text-left">
                ...
                <form action="{{ route('protocols.addStep', $protocol->id) }}" method="POST" class="space-y-6">
                    @csrf
                    <div class="space-y-3">
                        <label class="text-[10px] font-black text-slate-400 uppercase italic ml-2 tracking-widest leading-none">{{ __("Jour cible (Chronologie)") }}</label>
                        <div class="relative">
                            <span class="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 text-2xl font-black italic">J</span>
                            <input type="number" name="day_number" class="w-full pl-12 p-5 bg-slate-50 border-none rounded-2xl shadow-inner text-2xl font-black text-blue-600 italic outline-none focus:ring-4 focus:ring-blue-500/10 transition-all" placeholder="{{ __('7') }}" required>
                        </div>
                    </div>

                    <div class="space-y-3">
                        <label class="text-[10px] font-black text-slate-400 uppercase italic ml-2 tracking-widest leading-none">{{ __("Nom de l'acte / Produit") }}</label>
                        <input type="text" name="action_name" placeholder="{{ __('EX: VACCIN GUMBORO') }}" class="w-full p-5 bg-slate-50 border-none rounded-2xl shadow-inner uppercase font-black text-slate-700 italic outline-none focus:ring-4 focus:ring-blue-500/10 transition-all" required>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-3">
                            <label class="text-[10px] font-black text-slate-400 uppercase italic ml-2 tracking-widest leading-none">{{ __("Type") }}</label>
                            <select name="type" class="w-full p-5 bg-slate-50 border-none rounded-2xl shadow-inner text-[10px] font-black uppercase italic appearance-none cursor-pointer outline-none focus:ring-4 focus:ring-blue-500/10">
                                <option value="Vaccin">💉 {{ __("Vaccin") }}</option>
                                <option value="Traitement">💊 {{ __("Traitement") }}</option>
                                <option value="Vitamine">✨ {{ __("Vitamine") }}</option>
                                <option value="Désinfection">🧼 {{ __("Hygiène") }}</option>
                            </select>
                        </div>
                        <div class="space-y-3">
                            <label class="text-[10px] font-black text-slate-400 uppercase italic ml-2 tracking-widest leading-none">{{ __("Méthode") }}</label>
                            <input type="text" name="method" placeholder="{{ __('Eau, Spray...') }}" class="w-full p-5 bg-slate-50 border-none rounded-2xl shadow-inner text-[10px] uppercase font-black italic outline-none focus:ring-4 focus:ring-blue-500/10">
                        </div>
                    </div>
            ...
            <div class="lg:col-span-2 space-y-6 text-left">
                <h3 class="text-[10px] font-black uppercase text-slate-400 mb-6 tracking-[0.2em] ml-6 italic">{{ __("Séquence chronologique du protocole AviSmart") }}</h3>
                    ...
                    <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 flex items-center justify-between group hover:border-blue-500 hover:shadow-xl hover:shadow-blue-500/5 transition-all duration-300 relative overflow-hidden">
                        ...
                        <div class="flex items-center gap-8 ml-4">
                            <div class="w-16 h-16 bg-slate-900 text-white rounded-[1.5rem] flex flex-col items-center justify-center leading-none shadow-xl transform group-hover:rotate-3 transition-transform">
                                <span class="text-[8px] font-black uppercase opacity-50 mb-1 italic">{{ __("Jour") }}</span>
                            ...
                            <div>
                                ...
                                <div class="flex gap-3">
                                    ...
                                    <span class="text-[8px] px-3 py-1 bg-blue-50 rounded-lg text-blue-600 uppercase font-black tracking-widest italic border border-blue-100">{{ $step->method ?? __("Indéfini") }}</span>
                                </div>
                            </div>
                        </div>
                ...
                @empty
                    <div class="p-24 border-4 border-dashed border-slate-100 rounded-[4rem] text-center bg-white/50 italic flex flex-col items-center justify-center">
                        ...
                        <p class="text-slate-400 uppercase text-[11px] font-black tracking-[0.3em] leading-loose">
                            {{ __("Séquence vierge.") }}<br>{{ __("Commence par indexer l'étape J1.") }}
                        </p>
                    </div>
                @endforelse
            </div>
