<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 text-left">
            <div class="flex items-center gap-5">
                ...
                <div>
                    <h2 class="font-black text-2xl text-slate-800 uppercase italic tracking-tighter leading-none">
                        📜 Bibliothèque des Protocoles
                    </h2>
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-2 italic">Standards de prophylaxie AviSmart</p>
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-4 w-full md:w-auto">
                ...
                <div class="relative flex-1 md:w-48">
                    ...
                    <input type="text" id="protocolSearch" placeholder="RECHERCHER..." 
                ...
                <form action="{{ route('protocols.import') }}" method="POST" enctype="multipart/form-data" id="importForm" class="relative group">
                    ...
                    <label for="protocol_file" id="dropzone"
                        ...
                        <span id="dropzone-text">Importer (JSON)</span>
                ...
                <button onclick="document.getElementById('modal-protocol').classList.remove('hidden')" class="px-6 py-3 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase italic tracking-widest hover:bg-blue-600 transition-all shadow-xl shadow-slate-900/20 whitespace-nowrap h-[46px] border-none cursor-pointer">
                    <i class="fa-solid fa-plus-circle mr-2 text-blue-400"></i> Créer un modèle
                </button>
                @endcan
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" id="protocolGrid">
                @forelse($protocols as $protocol)
                    <div class="protocol-card bg-white p-10 rounded-[4rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 group relative overflow-hidden flex flex-col justify-between min-h-[440px]" data-name="{{ strtoupper($protocol->name) }}">
                        ...
                        <div class="absolute top-8 left-8 flex flex-col gap-3 opacity-0 group-hover:opacity-100 transition-all transform -translate-x-4 group-hover:translate-x-0 z-20">
                            <button onclick="openPreview({{ json_encode($protocol->name) }}, {{ json_encode($protocol->steps) }})" class="w-10 h-10 bg-white text-blue-500 rounded-xl hover:bg-blue-500 hover:text-white transition-all flex items-center justify-center shadow-lg" title="Aperçu rapide">
                                <i class="fa-solid fa-eye text-xs"></i>
                            </button>
                            @can('elevage.M')
                            <a href="{{ route('protocols.export', $protocol->id) }}" class="w-11 h-11 bg-white text-emerald-600 rounded-2xl hover:bg-emerald-600 hover:text-white transition-all flex items-center justify-center shadow-xl text-decoration-none" title="Exporter">
                                <i class="fa-solid fa-download text-sm"></i>
                            </a>
                            @endcan
                            @can('elevage.S')
                            <form action="{{ route('protocols.destroy', $protocol->id) }}" method="POST" onsubmit="return confirm('ALERTE : Supprimer ce standard master ?')">
                                @csrf @method('DELETE')
                        ...
                        <div class="relative text-left italic">
                            <div class="flex justify-between items-start mb-8">
                                ...
                                <div class="text-right">
                                    ...
                                    <p class="text-[8px] text-slate-400 font-black uppercase tracking-widest mt-1 italic leading-none">Étapes</p>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-wrap gap-2 mb-6">
                                <span class="px-3 py-1.5 bg-slate-50 text-slate-500 rounded-xl text-[8px] font-black uppercase border border-slate-100 italic">SOUCHE: {{ $protocol->strain ?? 'MIXTE' }}</span>
                                <span class="px-3 py-1.5 bg-blue-50 text-blue-500 rounded-xl text-[8px] font-black uppercase border border-blue-100 italic">OFFICIEL AVISMART</span>
                            </div>

                            <p class="text-[11px] text-slate-400 uppercase leading-relaxed mb-8 line-clamp-3 font-black italic opacity-80">
                                {{ $protocol->description ?? 'Modèle de prophylaxie standardisé pour le suivi sanitaire rigoureux.' }}
                            </p>
                        </div>
                        ...
                        <div class="space-y-6 relative text-left">
                            <div class="flex items-center gap-4 py-6 border-y border-slate-50">
                                ...
                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest italic leading-tight">
                                    Appliqué à <span class="text-slate-900">{{ $protocol->active_batches_count }} {{ Str::plural('Lot', $protocol->active_batches_count) }}</span>
                                </p>
                            </div>
                            ...
                            <div class="flex gap-4">
                                ...
                                <a href="{{ route('protocols.show', $protocol->id) }}" title="Details" class="w-14 py-5 bg-slate-100 text-slate-400 rounded-[1.5rem] hover:bg-slate-600 hover:text-white transition-all shadow-sm flex items-center justify-center border-none cursor-pointer">
                                    <i class="fa-solid fa-info-circle text-sm"></i>
                                </a>
                                <form action="{{ route('protocols.duplicate', $protocol->id) }}" method="POST" class="shrink-0">
                                    @csrf
                                    <button type="submit" title="Dupliquer" class="w-14 py-5 bg-slate-100 text-slate-400 rounded-[1.5rem] hover:bg-emerald-600 hover:text-white transition-all shadow-sm flex items-center justify-center border-none cursor-pointer">
                ...
                @empty
                    <div class="col-span-full py-40 border-4 border-dashed border-slate-100 rounded-[5rem] text-center bg-slate-50/50 italic flex flex-col items-center">
                        ...
                        <p class="text-slate-400 uppercase text-xs font-black tracking-[0.4em] italic">Bibliothèque Vierge</p>
                    </div>
                @endforelse
            </div>
        </div>
            ...
            <div class="p-12 text-left">
                <div class="flex justify-between items-start mb-10">
                    <div>
                        ...
                        <p class="text-[10px] font-black text-blue-500 uppercase tracking-[0.2em] mt-2 italic font-black">Contenu détaillé du programme</p>
                    </div>
    ...
    <div id="modal-protocol" class="hidden fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-50 flex items-center justify-center p-4 italic font-bold">
        <div class="bg-white w-full max-w-lg rounded-[4rem] shadow-2xl p-12 relative overflow-hidden border border-white/20 text-left">
            <h3 class="text-3xl font-black uppercase italic tracking-tighter mb-2 text-slate-800 font-black">Nouveau Modèle</h3>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-12 italic font-black">Initialisation du standard</p>
            <form action="{{ route('protocols.store') }}" method="POST" class="space-y-8">
                @csrf
                <div class="space-y-6">
                    <div>
                        <label class="text-[10px] font-black text-slate-400 uppercase ml-4 tracking-[0.2em] mb-3 block italic leading-none font-black">Nom du protocole</label>
                        <input type="text" name="name" placeholder="EX: CHAIR EXPORT 45J" class="w-full p-6 bg-slate-50 rounded-[2rem] border-none shadow-inner font-black uppercase text-slate-700 italic" required>
                    </div>
                    <div>
                ...
                <div class="flex gap-4 pt-6 italic">
                    <button type="button" onclick="document.getElementById('modal-protocol').classList.add('hidden')" class="flex-1 py-6 text-[10px] font-black uppercase text-slate-400 italic border-none bg-transparent font-black">Annuler</button>
                    <button type="submit" class="flex-[2] py-6 bg-slate-900 text-white rounded-[2rem] text-[10px] font-black uppercase italic shadow-2xl hover:bg-blue-600 transition-all font-black">Créer <i class="fa-solid fa-arrow-right ml-2 text-blue-400"></i></button>
                </div>
            </form>
        </div>
    </div>
    ...
    function openPreview(name, steps) {
        ...
        if (!steps || steps.length === 0) {
            container.innerHTML = '<div class="p-6 bg-slate-50 rounded-2xl text-slate-400 italic">Aucune étape configurée pour ce protocole.</div>';
        } else {
            ...
            steps.forEach(step => {
                ...
                stepEl.innerHTML = `
                    <div class="w-12 h-12 bg-blue-600 text-white rounded-xl flex flex-col items-center justify-center shrink-0 shadow-lg shadow-blue-200">
                        <span class="text-[8px] leading-none mb-1 opacity-70">JOUR</span>
                        <span class="text-xs leading-none font-black">${step.day_number}</span>
                    </div>
                    <div class="flex-1">
                        <div class="flex justify-between items-start mb-1">
                            <h4 class="text-slate-800 font-black">${step.action_name || 'INTERVENTION'}</h4>
                            <span class="px-2 py-1 bg-blue-100 text-blue-600 rounded-lg text-[7px] font-black">${step.type || 'SANTÉ'}</span>
                        </div>
                        <p class="text-[9px] text-slate-400 font-bold lowercase">Méthode : ${step.method || 'Non spécifiée'}</p>
                    </div>
                `;
    ...
    function handleFileSelect(input) {
        if (input.files && input.files[0]) {
            document.getElementById('dropzone-text').innerText = "VÉRIFICATION...";
