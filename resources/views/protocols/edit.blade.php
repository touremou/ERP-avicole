        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4 text-left">
                ...
                <h2 class="text-xl font-black text-slate-800 uppercase italic tracking-tighter leading-none">
                    🛠️ Architecte : {{ $protocol->name }}
                </h2>
            </div>
            <div class="flex items-center gap-3">
                <span class="px-4 py-2 bg-slate-900 text-yellow-500 rounded-xl text-[10px] font-black uppercase italic tracking-widest shadow-xl ring-1 ring-white/20">Édition Master</span>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            @if ($errors->any())
                <div class="mb-8 p-6 bg-rose-600 text-white rounded-[2rem] shadow-xl animate-pulse text-left italic">
                    <p class="font-black uppercase text-xs mb-2">⚠️ Conflit de validation détecté :</p>
            ...
            <form action="{{ route('protocols.update', $protocol->id) }}" method="POST" id="protocolForm">
                ...
                <div class="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm mb-8 relative overflow-hidden text-left">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
                        {{-- Nom du Modèle --}}
                        <div class="space-y-3">
                            <label class="block text-[10px] font-black text-slate-400 uppercase ml-2 italic tracking-widest leading-none">Désignation du Modèle</label>
                        ...
                        {{-- Souche / Race --}}
                        <div class="space-y-3">
                            <label class="block text-[10px] font-black text-emerald-600 uppercase ml-2 italic tracking-widest leading-none">Souche / Référentiel</label>
                        ...
                        <div class="md:col-span-3 space-y-3">
                            <label class="block text-[10px] font-black text-slate-400 uppercase ml-2 italic tracking-widest leading-none">Objectifs & Notes Techniques</label>
                ...
                <div class="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm text-left italic">
                    <div class="flex justify-between items-center mb-10 border-b border-slate-50 pb-6">
                        <div>
                            <h3 class="text-[10px] font-black text-slate-800 uppercase tracking-[0.2em] leading-none italic">Chronologie Master</h3>
                            <p class="text-[8px] text-slate-400 uppercase mt-2 italic font-black">Planification séquentielle des interventions</p>
                        </div>
                        <button type="button" onclick="addStep()" class="px-8 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase italic hover:bg-blue-600 transition-all shadow-xl shadow-slate-900/10 tracking-[0.2em] border-none cursor-pointer">
                            <i class="fa-solid fa-calendar-plus mr-2 text-blue-400"></i> Insérer un jour
                        </button>
                    </div>
                    ...
                    <div class="mt-16 flex flex-col md:flex-row gap-6">
                        ...
                        <button type="submit" class="flex-[3] bg-slate-900 text-white py-7 rounded-[2.5rem] font-black uppercase italic tracking-[0.3em] text-[11px] hover:bg-blue-600 transition-all shadow-2xl transform active:scale-95 border-none cursor-pointer">
                            <i class="fa-solid fa-sync-alt mr-3 text-blue-400"></i> Mettre à jour le protocole master
                        </button>
                    </div>
                </div>
            </form>
    ...
    <script>
        ...
        function addStep() {
            ...
            newRow.innerHTML = `
                ...
                <div class="flex-1 min-w-[250px]">
                    <input type="text" name="steps[${stepCount}][action_name]" placeholder="NOUVELLE ACTION" required class="w-full bg-white rounded-2xl border-none font-black text-slate-800 uppercase shadow-sm p-4 italic text-sm outline-none">
                </div>
                <div class="w-52 shrink-0">
