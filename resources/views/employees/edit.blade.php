        ...
        <div class="flex items-center justify-between">
            ...
            <div class="hidden md:block">
                <span class="px-4 py-2 bg-blue-50 text-blue-600 rounded-xl text-[10px] font-black uppercase italic tracking-widest border border-blue-100 shadow-sm">
                    <i class="fas fa-pen-nib mr-2"></i> Mode Modification (M)
                </span>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
                ...
                <form action="{{ route('employees.update', $employee->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8" id="edit-profile-form">
                    ...
                    <div class="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 relative overflow-hidden text-left italic">
                        ...
                        <h3 class="text-[10px] font-black text-blue-500 uppercase tracking-widest mb-10 flex items-center italic">
                            <i class="fas fa-id-card-alt mr-3 text-xs"></i> 01. État Civil & Contact
                        </h3>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="space-y-3">
                                <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 italic tracking-widest">Prénom</label>
                            ...
                            <div class="space-y-3">
                                <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 italic tracking-widest">Nom de famille</label>
                            ...
                            <div class="space-y-3">
                                <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 italic tracking-widest">Genre</label>
                            ...
                            <div class="space-y-3">
                                <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 italic tracking-widest">N° de Téléphone</label>
                    ...
                    <div class="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 relative overflow-hidden text-left italic">
                        ...
                        <h3 class="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-10 flex items-center italic">
                            <i class="fas fa-briefcase mr-3 text-xs"></i> 02. Poste & Contrat
                        </h3>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="space-y-3 md:col-span-2">
                                <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 italic tracking-widest">Intitulé Exact du Poste</label>
                            ...
                            <div class="space-y-3">
                                <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 italic tracking-widest">Type de Contrat</label>
                                <select name="contract_type" class="w-full p-5 bg-slate-50 rounded-2xl font-black outline-none border-none focus:ring-4 focus:ring-emerald-500/10 shadow-inner appearance-none text-slate-700 italic cursor-pointer">
                                    <option value="CDI" {{ old('contract_type', $employee->contract_type) == 'CDI' ? 'selected' : '' }}>📄 CDI (Indéterminé)</option>
                                    <option value="CDD" {{ old('contract_type', $employee->contract_type) == 'CDD' ? 'selected' : '' }}>⏳ CDD (Déterminé)</option>
                                    <option value="Journalier" {{ old('contract_type', $employee->contract_type) == 'Journalier' ? 'selected' : '' }}>☀️ Journalier</option>
                                </select>
                            </div>
                            <div class="space-y-3">
                                <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 italic tracking-widest">Département</label>
                                <select name="department" class="w-full p-5 bg-slate-50 rounded-2xl font-black outline-none border-none focus:ring-4 focus:ring-emerald-500/10 shadow-inner appearance-none text-slate-700 italic cursor-pointer">
                                    <option value="Elevage" {{ old('department', $employee->department) == 'Elevage' ? 'selected' : '' }}>Élevage & Production</option>
                                    <option value="Administration" {{ old('department', $employee->department) == 'Administration' ? 'selected' : '' }}>Administration</option>
                                    <option value="Logistique" {{ old('department', $employee->department) == 'Logistique' ? 'selected' : '' }}>Logistique</option>
                                </select>
                            </div>
                            <div class="space-y-3">
                                <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 italic tracking-widest">Salaire de base ({{ setting('general.currency', 'GNF') }})</label>
                            ...
                            <div class="space-y-3">
                                <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 italic tracking-widest">N° Orange Money</label>
                            ...
                            <div class="space-y-3">
                                <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 italic tracking-widest">Bâtiment assigné</label>
                                <select name="assigned_building_id" class="w-full p-5 bg-slate-50 rounded-2xl font-black outline-none border-none focus:ring-4 focus:ring-emerald-500/10 shadow-inner appearance-none text-slate-700 italic cursor-pointer">
                                    <option value="">Aucun (polyvalent)</option>
                            ...
                            <div class="space-y-3">
                                <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 italic tracking-widest">Statut RH</label>
                                <select name="status" class="w-full p-5 bg-slate-50 rounded-2xl font-black outline-none border-none focus:ring-4 focus:ring-emerald-500/10 shadow-inner appearance-none text-slate-700 italic cursor-pointer">
                                    <option value="Actif" {{ old('status', $employee->status) == 'Actif' ? 'selected' : '' }}>🟢 Actif / Présent</option>
                                    <option value="Suspendu" {{ old('status', $employee->status) == 'Suspendu' ? 'selected' : '' }}>🔴 Suspendu</option>
                                    <option value="Congé" {{ old('status', $employee->status) == 'Congé' ? 'selected' : '' }}>🟡 En Congé</option>
                                </select>
                            </div>
                            <div class="space-y-3">
                                <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 italic tracking-widest">Date d'embauche</label>
                    ...
                    <div class="bg-slate-900 p-10 rounded-[3rem] shadow-xl text-white relative overflow-hidden text-left italic">
                        ...
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
                            <h3 class="text-[10px] font-black text-slate-500 uppercase tracking-widest italic relative z-10">03. Documents de bord</h3>
                            <div class="flex gap-2 relative z-10">
                                @if($employee->photo_path) <span class="text-[7px] bg-blue-500 text-white px-2 py-1 rounded-md font-black uppercase tracking-tighter shadow-sm"><i class="fas fa-image mr-1"></i> Photo Actuelle</span> @endif
                                @if($employee->cv_path) <span class="text-[7px] bg-amber-500 text-slate-900 px-2 py-1 rounded-md font-black uppercase tracking-tighter shadow-sm"><i class="fas fa-file-pdf mr-1"></i> CV Présent</span> @endif
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                            <div class="border-2 border-dashed border-slate-700 p-10 rounded-[2.5rem] text-center hover:border-blue-500 transition-all bg-white/5 group">
                                <label class="block text-[9px] font-black text-slate-400 uppercase mb-6 tracking-widest italic group-hover:text-blue-400 leading-none">Remplacer la photo de profil</label>
                            ...
                            <div class="border-2 border-dashed border-slate-700 p-10 rounded-[2.5rem] text-center hover:border-amber-500 transition-all bg-white/5 group">
                                <label class="block text-[9px] font-black text-slate-400 uppercase mb-6 tracking-widest italic group-hover:text-amber-400 leading-none">Mettre à jour le CV (PDF)</label>
                    ...
                    <div class="flex flex-col md:flex-row gap-6">
                        <a href="{{ route('employees.show', $employee->id) }}" class="flex-1 bg-white border-2 border-slate-100 text-slate-400 font-black py-8 rounded-[3rem] shadow-sm hover:bg-rose-50 hover:text-rose-600 transition-all text-center uppercase tracking-widest text-[10px] italic flex items-center justify-center gap-3 no-underline">
                            <i class="fas fa-times"></i> Abandonner les modifications
                        </a>
            ...
            @else
                ...
                <div class="bg-white p-24 rounded-[4rem] border border-slate-100 shadow-xl text-center italic font-black">
                    ...
                    <h3 class="text-2xl font-black text-slate-800 uppercase italic mb-4 tracking-tighter">Action Restreinte</h3>
                    <p class="text-slate-400 text-[10px] font-black uppercase tracking-widest max-w-sm mx-auto leading-relaxed">
                        Vous ne disposez pas des permissions suffisantes (M) pour modifier les fiches du personnel.
                    </p>
