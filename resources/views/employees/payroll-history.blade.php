    ...
    <x-slot name="header">
        <div class="flex items-center gap-4 text-left">
            <a href="{{ route('employees.show', $employee) }}" class="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-slate-500 hover:text-slate-800 rounded-xl transition-all shadow-sm group no-underline">
                ...
                <span class="text-[10px] font-black uppercase italic tracking-widest">Fiche Agent</span>
            </a>
            <div>
                ...
                <p class="text-[9px] font-bold text-blue-500 uppercase mt-1 tracking-widest italic">Historique Paie & Congés</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
                <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm text-center">
                    <p class="text-[7px] font-black text-slate-400 uppercase tracking-widest">Mois payés</p>
                ...
                <div class="bg-emerald-50 p-4 rounded-2xl border border-emerald-200 shadow-sm text-center">
                    <p class="text-[7px] font-black text-emerald-500 uppercase tracking-widest">Total perçu</p>
                    <p class="text-lg font-black text-emerald-600">{{ number_format($totals['total_earned'], 0, ',', '.') }}</p>
                </div>
                <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm text-center">
                    <p class="text-[7px] font-black text-blue-500 uppercase tracking-widest">Primes cumulées</p>
                    <p class="text-lg font-black text-blue-600">{{ number_format($totals['total_primes'], 0, ',', '.') }}</p>
                </div>
                <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm text-center">
                    <p class="text-[7px] font-black text-red-500 uppercase tracking-widest">Déductions</p>
                    <p class="text-lg font-black text-red-500">{{ number_format($totals['total_deductions'], 0, ',', '.') }}</p>
                </div>
                <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm text-center">
                    <p class="text-[7px] font-black text-amber-500 uppercase tracking-widest">Congés utilisés</p>
                    <p class="text-2xl font-black text-amber-600">{{ $totals['leave_days_used'] }}j</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden text-left">
                    <div class="px-6 py-4 bg-slate-50 border-b border-slate-100">
                        <h3 class="text-[10px] font-black uppercase tracking-widest text-slate-500 flex items-center gap-2"><i class="fa-solid fa-money-bill-wave text-blue-500"></i> Fiches de paie</h3>
                    </div>
                        ...
                        <tbody class="divide-y divide-slate-50">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-all">
                                <td class="px-5 py-3">
                                    ...
                                    <p class="text-[8px] text-slate-400">{{ $slip->days_worked }}j travaillés</p>
                                </td>
                                ...
                                <td class="px-5 py-3 text-center">
                                    ...
                                    <div class="flex items-center justify-center gap-1.5">
                                        ...
                                        <a href="{{ route('payroll.print', ['payslip' => $slip, 'type' => 'bon']) }}" target="_blank"
                                           class="w-7 h-7 rounded-lg bg-amber-50 text-amber-500 hover:bg-amber-100 flex items-center justify-center no-underline transition-all" title="Bon de paie (comptable)">
                                        ...
                                        <a href="{{ route('payroll.print', ['payslip' => $slip, 'type' => 'fiche']) }}" target="_blank"
                                           class="w-7 h-7 rounded-lg bg-blue-50 text-blue-500 hover:bg-blue-100 flex items-center justify-center no-underline transition-all" title="Fiche de paie (imprimer)">
                            ...
                            @empty
                            <tr><td colspan="5" class="px-8 py-10 text-center text-slate-300 text-[9px] uppercase italic tracking-widest">Aucune fiche</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden text-left">
                    <div class="px-6 py-4 bg-slate-50 border-b border-slate-100">
                        <h3 class="text-[10px] font-black uppercase tracking-widest text-slate-500 flex items-center gap-2"><i class="fa-solid fa-calendar-xmark text-amber-500"></i> Congés</h3>
                    </div>
                        ...
                        <div class="px-5 py-3">
                            <div class="flex justify-between items-start">
                                ...
                                <div class="text-right">
                                    <span class="text-sm font-black text-slate-800">{{ $l->days_count }}j</span>
