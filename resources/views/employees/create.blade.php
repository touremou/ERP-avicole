    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="text-left">
                ...
                <p class="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-2 italic leading-none">Ajouter un nouveau membre à l'équipe</p>
            </div>
            ...
            @can('annuaire.C')
                ...
                <form action="{{ route('employees.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8" id="recruitment-form">
                    ...
                    <div class="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 relative overflow-hidden text-left">
                        ...
                        <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-10 italic">01. Identité de l'agent</h3>
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Nom de famille</label>
                            ...
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Prénom(s)</label>
                            ...
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Genre</label>
                                <div class="grid grid-cols-2 gap-4">
                                    <label class="cursor-pointer">
                                        <input type="radio" name="gender" value="M" class="peer sr-only" {{ old('gender', 'M') == 'M' ? 'checked' : '' }}>
                                        <div class="p-5 text-center bg-slate-50 rounded-2xl font-black text-[10px] uppercase text-slate-400 peer-checked:bg-blue-600 peer-checked:text-white transition-all shadow-inner italic">Homme</div>
                                    </label>
                                    <label class="cursor-pointer">
                                        <input type="radio" name="gender" value="F" class="peer sr-only" {{ old('gender') == 'F' ? 'checked' : '' }}>
                                        <div class="p-5 text-center bg-slate-50 rounded-2xl font-black text-[10px] uppercase text-slate-400 peer-checked:bg-pink-600 peer-checked:text-white transition-all shadow-inner italic">Femme</div>
                                    </label>
                                </div>
                            </div>
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Contact Téléphonique</label>
                    ...
                    <div class="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 relative overflow-hidden text-left">
                        ...
                        <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-10 italic">02. Poste & Contrat</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Poste occupé</label>
                            ...
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Type de Contrat</label>
                                <select name="contract_type" class="w-full p-5 bg-slate-50 rounded-2xl border-none focus:ring-4 focus:ring-emerald-500/10 outline-none shadow-inner appearance-none font-black text-slate-800 italic cursor-pointer" required>
                                    <option value="CDI" {{ old('contract_type') == 'CDI' ? 'selected' : '' }}>📄 CDI (Indéterminé)</option>
                                    <option value="CDD" {{ old('contract_type') == 'CDD' ? 'selected' : '' }}>⏳ CDD (Déterminé)</option>
                                    <option value="Journalier" {{ old('contract_type') == 'Journalier' ? 'selected' : '' }}>☀️ Journalier</option>
                                </select>
                            </div>
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Département</label>
                                <select name="department" class="w-full p-5 bg-slate-50 rounded-2xl border-none focus:ring-4 focus:ring-emerald-500/10 outline-none shadow-inner appearance-none font-black text-slate-800 italic cursor-pointer">
                                    <option value="Elevage" {{ old('department') == 'Elevage' ? 'selected' : '' }}>🐔 Élevage / Technique</option>
                                    <option value="Administration" {{ old('department') == 'Administration' ? 'selected' : '' }}>📂 Administration / RH</option>
                                    <option value="Logistique" {{ old('department') == 'Logistique' ? 'selected' : '' }}>🚚 Logistique & Ventes</option>
                                </select>
                            </div>
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Salaire Mensuel ({{ setting('general.currency', 'GNF') }})</label>
                            ...
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">N° Orange Money</label>
                            ...
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Bâtiment assigné</label>
                            ...
                            <div class="space-y-3">
                                <label class="text-[10px] font-black text-slate-500 uppercase ml-1 tracking-widest">Date d'embauche effective</label>
                    ...
                    <div class="bg-slate-900 p-10 rounded-[3rem] shadow-2xl text-white text-left italic">
                        <h3 class="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-10 italic">03. Documents & Profil</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="p-10 border-2 border-dashed border-slate-700 rounded-[2.5rem] bg-white/5 text-center group hover:border-blue-500 transition-all">
                                ...
                                <label class="block text-[10px] font-black uppercase text-slate-400 mb-6 tracking-widest">Photo d'Identité</label>
                            ...
                            <div class="p-10 border-2 border-dashed border-slate-700 rounded-[2.5rem] bg-white/5 text-center group hover:border-emerald-500 transition-all">
                                ...
                                <label class="block text-[10px] font-black uppercase text-slate-400 mb-6 tracking-widest">Dossier PDF (Contrat/CV)</label>
                    ...
                    <div class="flex flex-col md:flex-row gap-4 pt-8">
                        <a href="{{ url()->previous() }}" class="flex-1 bg-white border-2 border-slate-100 text-slate-400 font-black py-7 rounded-[2.5rem] shadow-sm hover:bg-slate-50 transition-all text-center uppercase tracking-widest text-[10px] italic flex items-center justify-center gap-3 no-underline">
                        ...
                        <button type="submit" class="flex-[2] bg-slate-900 text-white font-black py-7 rounded-[2.5rem] hover:bg-blue-600 transition-all uppercase tracking-[0.3em] text-[10px] italic shadow-2xl group">
                            ...
                            Enregistrer le nouveau collaborateur
                        </button>
                    </div>
                </form>
            @else
