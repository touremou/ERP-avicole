<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-5 text-left">
            <a href="{{ route('clients.show', $client) }}" class="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center text-slate-400 hover:bg-slate-900 hover:text-white transition-all no-underline">
                <i class="fa-solid fa-arrow-left"></i>
            </a>
            <div>
                <h2 class="font-black text-2xl text-slate-800 leading-none uppercase italic tracking-tighter">Modifier {{ $client->name }}</h2>
    ...
    <div class="py-10">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 italic font-bold text-left">
            ...
            <form method="POST" action="{{ route('clients.update', $client) }}">
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <select name="category" required class="w-full bg-slate-50 border-none rounded-2xl p-4 text-xs font-black uppercase shadow-inner outline-none appearance-none cursor-pointer">
                                    <option value="detaillant" {{ old('category', $client->category) === 'detaillant' ? 'selected' : '' }}>Détaillant</option>
                                    <option value="grossiste" {{ old('category', $client->category) === 'grossiste' ? 'selected' : '' }}>Grossiste</option>
                                    <option value="hotel_restaurant" {{ old('category', $client->category) === 'hotel_restaurant' ? 'selected' : '' }}>Hôtel / Restaurant</option>
                                    <option value="revendeur" {{ old('category', $client->category) === 'revendeur' ? 'selected' : '' }}>Revendeur</option>
                                    <option value="autre" {{ old('category', $client->category) === 'autre' ? 'selected' : '' }}>Autre</option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm mb-6">
                    <h3 class="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-6 flex items-center gap-2">
                        <i class="fa-solid fa-toggle-on text-emerald-500"></i> Statut
                    </h3>
                    <div class="flex gap-4">
                        @foreach(['actif' => 'Actif', 'suspendu' => 'Suspendu', 'blackliste' => 'Blacklisté'] as $val => $label)
                    ...
                    @if($client->status === 'blackliste')
                        <p class="text-[8px] text-red-500 mt-3 italic">Un client blacklisté ne peut plus passer de commande.</p>
                    @endif
                </div>
