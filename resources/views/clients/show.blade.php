        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 italic font-bold text-left">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm text-center">
                    <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Total achats</p>
                ...
                <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm text-center">
                    <p class="text-[8px] font-black text-emerald-500 uppercase tracking-widest mb-1">Total payé</p>
                ...
                <div @class(['p-5 rounded-[2rem] border shadow-sm text-center', $client->balance > 0 ? 'bg-red-50 border-red-200' : 'bg-emerald-50 border-emerald-200'])>
                    <p class="text-[8px] font-black uppercase tracking-widest mb-1 {{ $client->balance > 0 ? 'text-red-500' : 'text-emerald-500' }}">Solde dû</p>
                    <p class="text-xl font-black {{ $client->balance > 0 ? 'text-red-600' : 'text-emerald-600' }}">{{ number_format($client->balance, 0, ',', ' ') }}</p>
                    <p class="text-[8px] text-slate-400">{{ setting('general.currency', 'GNF') }}</p>
                </div>
                <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm text-center">
                    <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Nb ventes</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                    <h3 class="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-5 flex items-center gap-2">
                        <i class="fa-solid fa-id-card text-teal-500"></i> Informations
                    </h3>
                    <div class="space-y-4">
                        <div class="flex justify-between">
                            <span class="text-[9px] font-black text-slate-400 uppercase">Type</span>
                            <span class="text-xs font-black text-slate-800 uppercase">{{ $client->type }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-[9px] font-black text-slate-400 uppercase">Catégorie</span>
                            <span @class([
                        ...
                        <div class="flex justify-between">
                            <span class="text-[9px] font-black text-slate-400 uppercase">Téléphone</span>
                        ...
                        <div class="flex justify-between">
                            <span class="text-[9px] font-black text-slate-400 uppercase">Adresse</span>
                            <span class="text-xs font-black text-slate-800 text-right max-w-[60%]">{{ $client->address ?? '—' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-[9px] font-black text-slate-400 uppercase">Statut</span>
                            <span @class([
                                'text-[9px] font-black uppercase px-3 py-1 rounded-full',
                ...
                <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between">
                            <span class="text-[9px] font-black text-slate-400 uppercase">NIF</span>
                            <span class="text-xs font-black text-slate-800">{{ $client->nif ?? '—' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-[9px] font-black text-slate-400 uppercase">RCCM</span>
                            <span class="text-xs font-black text-slate-800">{{ $client->rccm ?? '—' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-[9px] font-black text-slate-400 uppercase">Plafond crédit</span>
                            <span class="text-xs font-black text-purple-600">
                                {{ $client->credit_limit > 0 ? number_format($client->credit_limit, 0, ',', ' ') . ' ' . setting('general.currency', 'GNF') : 'Illimité' }}
                            </span>
                        </div>
                        @if($client->credit_limit > 0)
                        <div class="flex justify-between">
                            <span class="text-[9px] font-black text-slate-400 uppercase">Crédit disponible</span>
                            <span class="text-xs font-black {{ $client->is_over_limit ? 'text-red-600' : 'text-emerald-600' }}">
                                {{ $client->is_over_limit ? 'DÉPASSÉ' : number_format($client->available_credit, 0, ',', ' ') . ' ' . setting('general.currency', 'GNF') }}
                            </span>
                        </div>
                        <div class="mt-3">
                            ...
                            <p class="text-[8px] font-black text-slate-400 mt-1 text-right">{{ round($pct) }}% utilisé</p>
                        </div>
                        @endif
                    </div>
                    ...
                    <div class="mt-6 p-4 bg-slate-50 rounded-2xl">
                        <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Notes</p>
            ...
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                <div class="px-8 py-5 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                    <h3 class="text-[10px] font-black uppercase text-slate-400 tracking-widest flex items-center gap-2">
                        <i class="fa-solid fa-file-invoice text-teal-500"></i> Dernières ventes
                    </h3>
                    @can('commerce.C')
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="text-[8px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                            ...
                            <th class="px-6 py-3 text-center">Paiement</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr>
                            <td colspan="6" class="px-8 py-10 text-center">
                                <p class="text-[10px] text-slate-400 uppercase tracking-widest font-black">Aucune vente enregistrée pour ce client</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
