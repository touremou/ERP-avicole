    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 text-left">
            <div class="flex items-center gap-5">
                ...
                <div>
                    <h2 class="font-black text-2xl text-slate-800 leading-none uppercase italic tracking-tighter">Sources d'Énergie</h2>
                    <p class="text-[10px] font-black text-amber-600 uppercase tracking-[0.2em] mt-2 italic">EDG, Groupe Électrogène, Solaire — Configuration</p>
                </div>
            </div>
        </div>
    </x-slot>
                ...
                <div @class(['p-6 rounded-[2.5rem] border shadow-sm',
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start gap-4">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex items-center gap-1 border-l border-slate-200/50 pl-3">
                                <a href="{{ route('utilities.energy.sources.edit', $source->id) }}" class="w-8 h-8 rounded-xl bg-white/50 text-slate-400 hover:bg-blue-500 hover:text-white flex items-center justify-center transition-all shadow-sm" title="Modifier">
                                    <i class="fa-solid fa-pen text-[10px]"></i>
                                </a>
                                <form method="POST" action="{{ route('utilities.energy.sources.destroy', $source->id) }}" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer définitivement cette source d\'énergie ?');">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="w-8 h-8 rounded-xl bg-white/50 text-slate-400 hover:bg-red-500 hover:text-white flex items-center justify-center transition-all shadow-sm border-none cursor-pointer" title="Supprimer">
                    ...
                    <div class="grid grid-cols-4 gap-4 mt-4">
                        <div class="text-center p-3 bg-slate-50 rounded-xl">
                            <p class="text-[8px] font-black text-slate-400 uppercase">Heures totales</p>
                        ...
                        <div class="text-center p-3 bg-slate-50 rounded-xl">
                            <p class="text-[8px] font-black text-slate-400 uppercase">Gasoil cuve</p>
                    ...
                    @can('ressources.M')
                        ...
                        <form method="POST" action="{{ route('utilities.energy.maintenance', $source) }}" class="mt-4 flex gap-3 items-end">
                            @csrf 
                            @method('PUT')
                            <input type="text" name="maintenance_notes" placeholder="Notes de maintenance (vidange, filtres, etc.)..."
                            ...
                            <button type="submit" class="bg-amber-500 text-white px-5 py-3 rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-amber-600 transition-all border-none cursor-pointer shrink-0">
                                <i class="fa-solid fa-wrench mr-1"></i> Maintenance effectuée
                            </button>
                        </form>
                        @endif
                    @endcan
                ...
                <form method="POST" action="{{ route('utilities.energy.sources.store') }}" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="name" required placeholder="Groupe Perkins 100kVA..."
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="type" required class="w-full bg-white border-none rounded-2xl p-4 text-xs font-black uppercase shadow-sm outline-none appearance-none cursor-pointer">
                                <option value="edg">EDG (Réseau)</option>
                                <option value="groupe">Groupe Électrogène</option>
                                <option value="solaire">Solaire</option>
                            </select>
                        </div>
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Puissance (kVA)</label>
                            <input type="number" name="capacity_kva" step="0.1" min="0" placeholder="Ex: 100"
                                class="w-full bg-white border-none rounded-2xl p-4 text-sm font-black shadow-sm outline-none">
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Marque</label>
                            <input type="text" name="brand" placeholder="Perkins, Caterpillar..."
                                class="w-full bg-white border-none rounded-2xl p-4 text-xs font-black shadow-sm outline-none">
                        </div>
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Modèle</label>
                            <input type="text" name="model" placeholder="1104D-44T..."
                                class="w-full bg-white border-none rounded-2xl p-4 text-xs font-black shadow-sm outline-none">
                        </div>
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Carburant</label>
                            <select name="fuel_type" class="w-full bg-white border-none rounded-2xl p-4 text-xs font-black uppercase shadow-sm outline-none appearance-none cursor-pointer">
                                <option value="">N/A (EDG/Solaire)</option>
                                <option value="gasoil">Gasoil</option>
                                <option value="essence">Essence</option>
                            </select>
                        </div>
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Capacité cuve (L)</label>
                            <input type="number" name="fuel_tank_capacity" min="0" placeholder="Ex: 500"
                                class="w-full bg-white border-none rounded-2xl p-4 text-sm font-black shadow-sm outline-none">
                        </div>
                    </div>
                    <div class="space-y-2">
                        <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-2">Intervalle maintenance (heures)</label>
                    ...
                    <button type="submit" class="bg-amber-500 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-amber-600 transition-all border-none cursor-pointer shadow-lg mt-4">
                        <i class="fa-solid fa-bolt mr-2"></i> Enregistrer
                    </button>
                </form>
            </div>
            @endcan
