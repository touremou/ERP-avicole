                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-slate-50 text-[8px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 italic">
                            ...
                            <th class="px-6 py-5 text-center">Actions</th> {{-- 💡 NOUVELLE COLONNE --}}
                        </tr>
                    </thead>
                        ...
                        <tr class="hover:bg-slate-50/50 transition-all group">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2 opacity-20 group-hover:opacity-100 transition-opacity">
                                    <a href="{{ route('utilities.fuel.edit', $p->id) }}" class="w-6 h-6 bg-slate-100 rounded-lg text-slate-400 hover:bg-blue-500 hover:text-white flex items-center justify-center transition-colors" title="Modifier">
                                        <i class="fa-solid fa-pen text-[9px]"></i>
                                    </a>
                                    <form method="POST" action="{{ route('utilities.fuel.destroy', $p->id) }}" onsubmit="return confirm('Supprimer cet enregistrement d\'achat ?');">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="w-6 h-6 bg-slate-100 rounded-lg text-slate-400 hover:bg-red-500 hover:text-white flex items-center justify-center transition-colors border-none cursor-pointer" title="Supprimer">
