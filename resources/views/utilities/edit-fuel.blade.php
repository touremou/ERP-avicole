        ...
        <div class="bg-orange-50 p-10 rounded-[3rem] border border-orange-200">
            ...
            <form method="POST" action="{{ route('utilities.fuel.update', $purchase->id) }}" class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        <label class="text-[10px] uppercase text-slate-400 ml-2">{{ __("Groupe *") }}</label>
                    ...
                    <div>
                        <label class="text-[10px] uppercase text-slate-400 ml-2">{{ __("Prix Unitaire *") }}</label>
                ...
                <button type="submit" class="w-full bg-orange-500 text-white py-4 rounded-2xl font-black uppercase hover:bg-orange-600 shadow-lg mt-4">{{ __("Enregistrer les modifications") }}</button>
            </form>
        </div>
    </div>
</x-app-layout>