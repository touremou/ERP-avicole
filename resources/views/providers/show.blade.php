            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <a href="{{ route('providers.edit', $provider->id) }}" 
                   class="bg-white border border-slate-200 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase text-slate-600 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition shadow-sm tracking-widest flex items-center italic no-underline">
                    <i class="fas fa-edit mr-2 text-blue-500 group-hover:text-white"></i> Modifier
                </a>
                ...
                <form action="{{ route('providers.blacklist', $provider->id) }}" method="POST" class="inline">
                    ...
                    @else
                        <button type="submit" onclick="return confirm('Placer ce partenaire sur liste noire ?')" 
                            ...
                            <i class="fas fa-ban mr-2"></i> Blacklister
                        </button>
                    @endif
                </form>
                @endcan
                    ...
                    @if($isLocked)
                        <div class="group relative inline-block">
                            <button type="button" class="cursor-not-allowed opacity-50 bg-slate-100 text-slate-400 border border-slate-200 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest italic leading-none">
                                <i class="fas fa-lock mr-2"></i> Désactiver
                            </button>
                            <div class="absolute bottom-full right-0 mb-2 hidden group-hover:block w-48 p-3 bg-slate-900 text-white text-[8px] font-bold uppercase rounded-xl text-center shadow-xl z-50 italic tracking-tighter leading-tight">
                                ...
                                Action impossible : ce partenaire est lié à un lot actif.
                            </div>
                        </div>
                    @else
                        <form action="{{ route('providers.destroy', $provider->id) }}" method="POST" 
                              onsubmit="return confirm('Confirmer l\'archivage ? L\'historique sera conservé dans la corbeille.');" class="inline">
                            @csrf @method('DELETE')
                            <button type="submit" class="bg-red-50 text-red-600 border border-red-100 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase shadow-sm hover:bg-red-600 hover:text-white transition flex items-center tracking-widest italic">
                                <i class="fas fa-archive mr-2"></i> Désactiver
                            </button>
                        </form>
                    @endif
                @endcan
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="space-y-6">
                    <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 text-center relative overflow-hidden">
                        ...
                        <div class="mt-8 pt-8 border-t border-slate-50 space-y-4 text-left">
                            <div class="flex items-center group p-3 bg-slate-50/50 rounded-2xl shadow-inner border border-slate-100">
                                ...
                                <div class="ml-4">
                                    <p class="text-[9px] font-black uppercase text-slate-400 tracking-widest italic leading-none mb-1">ID Interne</p>
                            ...
                            <div class="flex items-center group p-3 bg-slate-50/50 rounded-2xl shadow-inner border border-slate-100">
                                ...
                                <div class="ml-4 text-left">
                                    <p class="text-[9px] font-black uppercase text-slate-400 tracking-widest italic leading-none mb-1">Ligne Directe</p>
                    ...
                    <div @class([
                        ...
                        <div class="text-left">
                            <p class="text-[10px] font-black uppercase opacity-70 mb-1 tracking-widest italic">Indice de Fiabilité</p>
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="bg-white p-10 rounded-[2.5rem] shadow-sm border border-slate-100 text-left">
                        <h3 class="text-[10px] font-black uppercase text-slate-400 mb-8 tracking-[0.2em] flex items-center italic">
                            <span class="w-8 h-[2px] bg-slate-100 mr-3"></span> Informations Administratives
                        </h3>
                        
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                            <div class="relative pl-6 border-l-4 border-blue-500">
                                <p class="text-[10px] font-black text-slate-400 uppercase mb-1 tracking-widest italic leading-none">Secteur</p>
                                <p class="font-black text-slate-800 text-xl tracking-tighter uppercase leading-none">{{ $provider->domain ?? 'Généraliste' }}</p>
                            </div>
                            <div class="relative pl-6 border-l-4 border-emerald-500">
                                <p class="text-[10px] font-black text-slate-400 uppercase mb-1 tracking-widest italic leading-none">Règlement habituel</p>
                                <p class="font-black text-slate-800 text-xl tracking-tighter uppercase leading-none">{{ $provider->payment_terms ?? 'Non spécifié' }}</p>
                            </div>
                            <div class="bg-slate-50 p-6 rounded-3xl border border-slate-100 shadow-inner">
                                <p class="text-[9px] font-black text-slate-400 uppercase mb-2 italic tracking-widest leading-none">Registre (RCCM)</p>
                                <p class="font-mono font-black text-slate-900 text-sm tracking-tighter leading-none">{{ $provider->rccm ?? 'NON DÉFINI' }}</p>
                            </div>
                            <div class="bg-slate-50 p-6 rounded-3xl border border-slate-100 shadow-inner">
                                <p class="text-[9px] font-black text-slate-400 uppercase mb-2 italic tracking-widest leading-none">NIF / IFU</p>
                                <p class="font-mono font-black text-slate-900 text-sm tracking-tighter leading-none">{{ $provider->nif ?? 'NON DÉFINI' }}</p>
                            </div>
                        </div>

                        <div class="mt-12 p-8 bg-slate-50 rounded-[2.5rem] border border-dashed border-slate-200">
                            <p class="text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest italic">Localisation du Siège</p>
                            <p class="text-slate-700 font-bold flex items-start italic tracking-tight">
                                ...
                                {{ $provider->address ?? 'Aucune adresse enregistrée pour ce partenaire.' }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden text-left">
                        <div class="p-8 border-b border-slate-50 bg-slate-50/30 flex justify-between items-center">
                            <div>
                                <h3 class="font-black text-slate-800 uppercase text-[10px] tracking-widest italic leading-none">📦 Journal des Acquisitions</h3>
                                <p class="text-[9px] font-bold text-slate-400 uppercase mt-1 italic leading-none">Performance des lots fournis</p>
                            </div>
                            <span class="px-4 py-1.5 bg-slate-900 text-white rounded-full text-[9px] font-black uppercase italic shadow-lg">
                                {{ $provider->batches->count() }} Lot(s)
                            </span>
                        </div>
                            ...
                            <table class="w-full text-left border-collapse">
                                <thead>
                                    <tr class="text-[9px] font-black uppercase text-slate-400 bg-slate-50/50 tracking-widest italic">
                                        <th class="px-8 py-5">Lot</th>
                                        <th class="px-4 py-5">Espèce</th>
                                        <th class="px-4 py-5 text-center">Initial</th>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="px-8 py-20 text-center">
                                                ...
                                                <p class="text-[10px] font-black text-slate-300 uppercase italic tracking-widest">Aucune donnée historique</p>
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
                    ...
                    <div class="mt-10 py-6 border-t border-slate-100 flex justify-end">
                        <a href="{{ route('trash.index') }}" class="group flex items-center gap-4 bg-slate-50 px-5 py-2.5 rounded-2xl hover:bg-slate-900 transition-all duration-500 border border-dashed border-slate-200 hover:border-slate-800 no-underline">
                            <div class="flex flex-col items-end leading-none">
                                <span class="text-[7px] font-black text-slate-400 uppercase tracking-widest mb-1 group-hover:text-slate-500 italic">Maintenance Partenaires</span>
                                <span class="text-[9px] font-black text-slate-400 uppercase italic group-hover:text-white transition-colors">Consulter les archives</span>
                            </div>
