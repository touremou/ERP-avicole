...
<head>
    <meta charset="UTF-8">
    <title>{{ $sale->reference }} — {{ $sale->type === 'facture' ? 'Facture' : 'Bon de Livraison' }}</title>
    <style>
...
<body onload="window.print()">
    ...
    <table>
        ...
        <tfoot>
            <tr>
                <td colspan="4" style="text-align: right; color: #94a3b8; text-transform: uppercase; font-size: 9px; letter-spacing: 1px;">Sous-total HT</td>
                <td style="text-align: right;">{{ number_format($sale->subtotal, 0, ',', ' ') }}</td>
            </tr>
            @if($sale->tax_rate > 0)
            <tr>
                <td colspan="4" style="text-align: right; color: #94a3b8; text-transform: uppercase; font-size: 9px; letter-spacing: 1px;">TVA ({{ $sale->tax_rate }}%)</td>
                <td style="text-align: right;">{{ number_format($sale->tax_amount, 0, ',', ' ') }}</td>
            </tr>
            @endif
            <tr class="total-row">
                <td colspan="4" style="text-align: right; text-transform: uppercase; letter-spacing: 2px; font-size: 10px;">Total TTC</td>
                <td style="text-align: right; font-size: 15px;">{{ number_format($sale->total_amount, 0, ',', ' ') }} GNF</td>
            </tr>
        </tfoot>
    </table>
    ...
    <div class="payments-box">
        ...
        <div class="payment-line">
            ...
            <span><strong>{{ number_format($payment->amount, 0, ',', ' ') }} GNF</strong></span>
        </div>
        @endforeach
        <div class="payment-line" style="border-top: 1px solid #86efac; margin-top: 5px; padding-top: 5px; font-weight: 900;">
            <span>RESTE DÛ</span>
            <span>{{ number_format($sale->remaining_amount, 0, ',', ' ') }} GNF</span>
        </div>
    </div>
    @endif
