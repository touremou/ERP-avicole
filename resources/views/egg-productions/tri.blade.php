        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="flex justify-between items-end">
                <h2 class="text-3xl font-black text-slate-800 uppercase italic tracking-tighter flex items-center gap-4 m-0">
                    ...
                    <div>
                        <span class="text-sm text-slate-400 block tracking-widest not-italic font-bold">SESSION DE TRI</span>
                        LOT <span class="text-blue-600">{{ $batch->code }}</span>
                    </div>
                </h2>
            ...
            <div class="bg-slate-900 p-10 rounded-[3.5rem] shadow-2xl relative overflow-hidden group">
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                    <div class="text-center md:text-left">
                        <p class="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-1 italic m-0">Total à ventiler</p>
                    ...
                    <div class="flex-1 w-full max-w-md">
                         <div class="flex justify-between mb-2 px-2">
                            <span class="text-[10px] text-slate-500 uppercase font-black italic">Progression du tri</span>
                            <span id="reste_count" class="text-[10px] text-emerald-400 uppercase font-black italic">Reste : 0</span>
                         </div>
            ...
            <form action="{{ route('egg-productions.update-tri', $eggProduction->id) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div class="bg-white p-8 rounded-[3rem] border-2 border-red-50 shadow-sm group hover:border-red-400 transition-all relative overflow-hidden">
                        <label class="block text-[10px] font-black text-red-500 uppercase mb-4 tracking-widest italic leading-none">⚠️ Cassés (Unités)</label>
                    ...
                    <div class="bg-white p-8 rounded-[3rem] border-2 border-orange-50 shadow-sm group hover:border-orange-400 transition-all relative overflow-hidden">
                        <label class="block text-[10px] font-black text-orange-500 uppercase mb-4 tracking-widest italic leading-none">⚙️ Anormaux (Unités)</label>
                ...
                <div class="bg-white p-10 rounded-[4rem] border border-slate-100 shadow-xl shadow-slate-200/50">
                    <div class="flex justify-between items-center mb-10">
                        <div class="text-left">
                            <h3 class="text-sm font-black text-slate-800 uppercase tracking-widest italic m-0">Ventilation Magasin</h3>
                            <p class="text-[10px] text-blue-500 uppercase font-black italic mt-1 m-0">Plateaux de {{ setting('general.eggs_per_tray', 30) }} œufs</p>
                        </div>
                        ...
                        <div id="card_{{ $c }}" class="p-4 bg-slate-50 rounded-[2rem] border-2 border-transparent transition-all duration-300">
                            ...
                            <div class="flex items-center gap-2">
                                <div class="relative flex-1">
                                    ...
                                    <p class="text-[7px] font-black text-slate-300 uppercase mt-1 text-center tracking-widest m-0">Plateaux</p>
                                </div>
                                ...
                                <div class="relative flex-1">
                                    <input type="number" min="0" max="{{ setting('general.eggs_per_tray', 30) - 1 }}" name="grade_{{ $c }}_uni" id="{{ $c }}_uni" 
                                        ...
                                        class="w-full bg-white border border-slate-100 rounded-xl p-2.5 text-xl font-black text-center shadow-inner focus:ring-2 focus:ring-blue-500 italic appearance-none outline-none">
                                    <p class="text-[7px] font-black text-slate-300 uppercase mt-1 text-center tracking-widest m-0">Œufs</p>
                                </div>
                            </div>
                            <div class="mt-3 flex items-center justify-center gap-1.5 bg-blue-600 text-white py-1.5 rounded-lg shadow-sm shadow-blue-200">
                                ...
                                <span class="text-[7px] font-black uppercase italic opacity-80 leading-none">Total</span>
                            </div>
                        </div>
                        @endforeach
                    </div>
                ...
                <div class="flex flex-col gap-4 pt-6">
                    <button type="submit" id="submit-btn" class="w-full bg-slate-900 text-white font-black py-10 rounded-[3.5rem] shadow-2xl uppercase italic tracking-[0.3em] text-sm transition-all hover:bg-blue-600 hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-4 group border-none cursor-pointer">
                        <span>Confirmer le Calibrage</span>
                    ...
                    <a href="{{ route('egg-productions.index') }}" class="w-full bg-white text-slate-400 font-black py-6 rounded-[3rem] border border-slate-100 text-center uppercase tracking-[0.4em] text-[9px] italic hover:text-slate-800 transition-all no-underline flex items-center justify-center">
                        <i class="fa-solid fa-chevron-left mr-2"></i> Retour Dashboard
                    </a>
                </div>
            </form>
            @else
            <div class="bg-white p-20 rounded-[4rem] border border-slate-100 shadow-xl text-center italic font-bold">
                ...
                <p class="text-slate-400 text-[10px] font-black uppercase tracking-widest italic">Vous n'avez pas les droits de modification (M) pour effectuer le tri.</p>
                <a href="{{ route('egg-productions.index') }}" class="inline-block mt-8 px-10 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase italic no-underline">Retour au Dashboard</a>
            </div>
            @endcan
        </div>
    </div>
...
function updateUI() {
    ...
    if (resteCount) {
        if (reste < 0) {
            resteCount.innerText = `Excès : ${Math.abs(reste)} œufs`;
            resteCount.classList.replace('text-emerald-400', 'text-rose-500');
        } else {
            resteCount.innerText = `Reste : ${reste} œufs`;
    ...
    if (brutAlvDisplay) {
        ...
        brutAlvDisplay.innerText = `Cible : ${plateauxComplets} Pl. + ${resteUnites} Un.`;
    }

    if (Math.abs(reste) < 0.1) {
        feedback.innerHTML = "✅ ÉQUILIBRE PARFAIT";
        ...
        feedback.innerHTML = reste > 0 ? `⚠️ MANQUE ${reste} ŒUFS` : `🚫 EXCÈS ${Math.abs(reste)} ŒUFS`;
