        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            @if ($errors->any())
                <div class="mb-6 p-6 bg-red-600 text-white rounded-[2rem] text-[10px] uppercase font-black shadow-lg animate-pulse">
                    <p class="mb-2 border-b border-white/20 pb-2 italic">⚠️ Erreurs de validation :</p>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-2xl font-black text-slate-800 uppercase italic tracking-tighter leading-none">
                            {{ isset($eggProduction) ? '✏️ Tri & Calibrage' : '🥚 Nouvelle Collecte' }}
                        </h2>
                        <p class="text-[10px] font-black text-slate-400 uppercase mt-2 tracking-widest italic leading-none">
                            Bande : <span class="text-blue-500">{{ $batch->code }}</span> • {{ $batch->building->name }}
                        </p>
                    </div>
                </div>
            </div>
                ...
                <form action="{{ isset($eggProduction) ? route('egg-productions.update-tri', $eggProduction->id) : route('egg-productions.store') }}" method="POST" class="space-y-6" id="main-form">
                    ...
                    <div class="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-xl space-y-8 relative overflow-hidden text-left">
                        @if(!isset($eggProduction))
                            <div class="grid grid-cols-2 gap-6">
                                <div>
                                    <label class="block text-[10px] font-black text-slate-400 uppercase mb-3 ml-4 tracking-widest leading-none italic">Plateaux (x30)</label>
                                ...
                                <div>
                                    <label class="block text-[10px] font-black text-slate-400 uppercase mb-3 ml-4 tracking-widest leading-none italic">Unités</label>
                        ...
                        <div class="pt-8 {{ !isset($eggProduction) ? 'border-t border-slate-50' : '' }} text-center">
                            <label class="block text-[10px] font-black uppercase mb-4 tracking-widest italic {{ isset($eggProduction) ? 'text-blue-500' : 'text-emerald-500' }}">
                                {{ isset($eggProduction) ? 'Récolte brute verrouillée (Unités)' : 'Total Œufs Récoltés' }}
                            </label>
                            <input type="number" name="total_eggs_collected" id="total_eggs" 
                                   ...
                                   placeholder="0" min="0" required oninput="validateFlow()" {{ isset($eggProduction) ? 'readonly' : '' }}
                                   class="w-full {{ isset($eggProduction) ? 'bg-slate-100 text-slate-500 cursor-not-allowed' : 'bg-emerald-50 text-emerald-600' }} border-none rounded-[2.5rem] p-8 text-7xl font-black text-center shadow-inner focus:ring-0 italic appearance-none outline-none">
                            
                            <p id="info-display" class="mt-4 text-[11px] font-black text-slate-400 uppercase italic tracking-widest">
                                ≈ {{ number_format(($eggProduction->total_eggs_collected ?? 0) / setting('general.eggs_per_tray', 30), 2) }} Alvéoles
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="bg-white p-8 rounded-[2.5rem] border-2 border-red-50 shadow-sm text-left">
                            <label class="block text-[10px] font-black text-red-500 uppercase mb-3 tracking-widest italic leading-none">⚠️ Cassés / Fêlés</label>
                        ...
                        <div class="bg-white p-8 rounded-[2.5rem] border-2 border-orange-50 shadow-sm text-left">
                            <label class="block text-[10px] font-black text-orange-500 uppercase mb-3 tracking-widest italic leading-none">⚙️ Anormaux / Sales</label>
                    ...
                    <div class="bg-slate-900 p-10 rounded-[4rem] shadow-2xl text-white relative overflow-hidden text-left italic">
                        <div class="flex justify-between items-center mb-8 relative z-10">
                            <h3 class="text-[10px] font-black text-blue-400 uppercase tracking-[0.2em] m-0">⚖️ Répartition par Calibre</h3>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                            @foreach(['xl' => 'Calibre XL', 'l' => 'Calibre L', 'm' => 'Calibre M', 's' => 'Calibre S'] as $key => $label)
                                <div class="p-6 bg-white/5 rounded-[2rem] border border-white/10">
                                    <label class="block text-[9px] font-black text-slate-500 uppercase mb-4 text-center tracking-widest">{{ $label }}</label>
                                    <div class="flex items-center gap-3">
                                        <div class="flex-1">
                                            <input type="number" name="grade_{{ $key }}_alv" id="{{ $key }}_alv" oninput="validateFlow()" 
                                                   ...
                                                   placeholder="0" min="0" class="w-full bg-white/10 border-none rounded-xl p-3 font-black text-2xl text-white text-center focus:ring-2 focus:ring-blue-500 italic outline-none">
                                            <p class="text-[7px] text-slate-500 mt-2 text-center uppercase m-0">Plat. (x{{ setting('general.eggs_per_tray', 30) }})</p>
                                        </div>
                                        ...
                                        <div class="flex-1">
                                            <input type="number" name="grade_{{ $key }}_uni" id="{{ $key }}_uni" oninput="validateFlow()" 
                                                   ...
                                                   placeholder="0" min="0" max="29" class="w-full bg-white/10 border-none rounded-xl p-3 font-black text-2xl text-white text-center focus:ring-2 focus:ring-blue-500 italic outline-none">
                                            <p class="text-[7px] text-slate-500 mt-2 text-center uppercase m-0">Unités</p>
                                        </div>
                                    </div>
                                    <div class="mt-4 pt-4 border-t border-white/5 text-center">
                                        <p class="text-[9px] text-blue-400 font-black uppercase tracking-widest m-0">
                                            Total : <span id="count_{{ $key }}" class="text-white text-xs">0</span> œufs
                                        </p>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div class="flex flex-col gap-4 pt-6">
                        <button type="submit" id="submit-btn" class="w-full {{ isset($eggProduction) ? 'bg-blue-600 hover:bg-blue-700' : 'bg-slate-900 hover:bg-slate-800' }} text-white font-black py-10 rounded-[3rem] shadow-2xl uppercase tracking-[0.3em] text-xs italic transition-all group border-none cursor-pointer">
                            <span class="flex items-center justify-center gap-4">
                                {{ isset($eggProduction) ? 'Confirmer le Calibrage' : 'Enregistrer la Récolte' }}
            ...
            @else
                <div class="bg-white p-20 rounded-[4rem] border border-slate-100 shadow-xl text-center italic">
                    ...
                    <h3 class="text-xl font-black text-slate-800 uppercase italic">Action Verrouillée</h3>
                    <p class="text-slate-400 text-[10px] font-black uppercase tracking-widest leading-none mt-2">Vous n'avez pas la permission ({{ isset($eggProduction) ? 'M' : 'C' }}) requise.</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <script>
        ...
        function validateFlow() {
            ...
            if(feedback && submitBtn) {
                let totalGraded = 0;
                // On récupère dynamiquement la capacité max d'une alvéole moins 1
            let maxUnits = {{ setting('general.eggs_per_tray', 30) - 1 }};

                ['xl', 'l', 'm', 's'].forEach(g => {
                    const alv = parseInt(document.getElementById(g + '_alv')?.value) || 0;
                    const uniInput = document.getElementById(g + '_uni');
                    if(uniInput && parseInt(uniInput.value) > maxUnits) uniInput.value = maxUnits;
                    const uni = parseInt(uniInput?.value) || 0;
                ...
                if (Math.abs(diff) === 0) {
                    feedback.innerText = "✅ ÉQUILIBRE PARFAIT";
                    ...
                    const msg = diff > 0 ? `MANQUE ${Math.abs(diff)}` : `EXCÈS ${Math.abs(diff)}`;
                    feedback.innerText = `⚠️ ${msg} ŒUFS`;
