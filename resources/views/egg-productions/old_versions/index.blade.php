    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-left">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-xl md:text-2xl font-black text-slate-800 uppercase italic tracking-tighter leading-none m-0">🥚 {{ __("Dashboard Production") }}</h2>
            ...
            <div class="-mx-4 px-4 sm:mx-0 sm:px-0">
                <div class="flex md:grid md:grid-cols-5 gap-4 md:gap-6 overflow-x-auto pb-4 snap-x snap-mandatory md:overflow-visible md:pb-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                    ...
                    <div class="min-w-[280px] md:min-w-0 snap-center bg-slate-900 p-6 md:p-8 rounded-[2.5rem] md:rounded-[3.5rem] text-white shadow-xl relative overflow-hidden group text-left border-none">
                        <p class="text-[9px] md:text-[10px] font-black text-slate-500 uppercase mb-2 tracking-widest italic leading-none m-0">{{ __("Flux Entrant (Brut)") }}</p>
                    ...
                    <div class="min-w-[280px] md:min-w-0 snap-center bg-white p-6 md:p-8 rounded-[2.5rem] md:rounded-[3.5rem] border border-orange-100 shadow-lg shadow-orange-500/5 relative group text-left">
                        <div class="absolute top-4 right-4 md:top-6 md:right-6 px-2 py-1 md:px-3 md:py-1 bg-orange-100 text-orange-600 text-[7px] md:text-[8px] font-black rounded-lg italic uppercase tracking-tighter">{{ __("Attente Tri") }}</div>
                        <p class="text-[9px] md:text-[10px] font-black text-orange-400 uppercase mb-2 tracking-widest italic leading-none m-0">{{ __("Réserve Brute") }}</p>
                    ...
                    <div class="min-w-[280px] md:min-w-0 snap-center bg-emerald-500 p-6 md:p-8 rounded-[2.5rem] md:rounded-[3.5rem] text-white shadow-xl shadow-emerald-500/20 relative group text-left overflow-hidden border-none">
                        <p class="text-[9px] md:text-[10px] font-black text-emerald-100 uppercase mb-2 tracking-widest italic leading-none m-0">{{ __("Stock Magasin") }}</p>
                        <h3 class="text-4xl md:text-5xl font-black tracking-tighter m-0 mt-2">{{ number_format(array_sum($stockVendable), 2) }}</h3>
                        <p class="text-[8px] md:text-[9px] text-emerald-50 mt-3 md:mt-4 uppercase italic font-black tracking-tighter m-0">{{ __("Alvéoles (S-XL)") }}</p>
                    ...
                    <div class="min-w-[280px] md:min-w-0 snap-center bg-red-500 p-6 md:p-8 rounded-[2.5rem] md:rounded-[3.5rem] text-white shadow-xl shadow-red-500/20 relative group text-left overflow-hidden border-none">
                        <p class="text-[9px] md:text-[10px] font-black text-red-100 uppercase mb-2 tracking-widest italic leading-none m-0">{{ __("Pertes en Stock") }}</p>
                        ...
                        <p class="text-[7px] md:text-[8px] text-red-50 mt-2 uppercase italic leading-none font-black tracking-tighter m-0">
                            ≈ {{ round($totalPertesAlv * 30) }} {{ __("Œufs (Pertes)") }}
                        </p>
            ...
            <div class="bg-white rounded-[2.5rem] md:rounded-[3.5rem] border border-slate-100 shadow-xl shadow-slate-200/40 overflow-hidden text-left">
                <div class="p-6 md:p-10 border-b border-slate-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50/30">
                    <div class="flex items-center gap-3 md:gap-4">
                        ...
                        <div>
                            <h4 class="text-sm md:text-base font-black uppercase tracking-tight text-slate-800 italic leading-none m-0">📊 {{ __("Performance Technique") }}</h4>
                            <p class="text-[8px] md:text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1 md:mt-2 italic leading-none m-0">{{ __("Indice HDP & Collecte") }}</p>
                        </div>
                    </div>
                    ...
                    <table class="w-full text-left border-collapse min-w-[600px] md:min-w-0">
                        <thead>
                            <tr class="text-[8px] md:text-[9px] font-black text-slate-400 uppercase tracking-widest italic bg-slate-50/80">
                                <th class="px-6 md:px-10 py-4 md:py-5">{{ __("Lot / Bâtiment") }}</th>
                                <th class="px-4 py-4 md:py-5 text-center hidden sm:table-cell">{{ __("Âge") }}</th>
                                <th class="px-4 py-4 md:py-5 text-center">{{ __("Récolte Jour") }}</th>
                                <th class="px-4 py-4 md:py-5 text-center">{{ __("Laying Rate (HDP)") }}</th>
                                <th class="px-6 md:px-10 py-4 md:py-5 text-right">{{ __("Actions") }}</th>
                            </tr>
                        </thead>
                                    ...
                                    <td class="px-6 md:px-10 py-5 md:py-8">
                                        ...
                                        <div class="flex items-center gap-1.5 md:gap-2 mt-3 md:mt-4">
                                            @for($i=0; $i<4; $i++)
                                                ...
                                                @endif
                                            @endfor
                                            <span class="text-[7px] md:text-[8px] text-slate-400 uppercase ml-1 md:ml-2 italic font-black hidden sm:inline">{{ $passagesCount }}/4 {{ __("Passages") }}</span>
                                        </div>
                                    </td>
                                    <td class="px-4 py-5 md:py-8 text-center text-[10px] md:text-xs text-slate-500 italic hidden sm:table-cell">S-{{ ceil($b->age / 7) }}</td>
                                    <td class="px-4 py-5 md:py-8 text-center text-xs md:text-sm text-slate-900 font-black italic">{{ number_format($totalDayEggs) }} <small class="text-[7px] md:text-[8px] opacity-40 uppercase hidden sm:inline">{{ __("Unités") }}</small></td>
                                        ...
                                        @can('production.C')
                                            @if(!$isGraded && $passagesCount < 4)
                                                <a href="{{ route('egg-productions.create', ['batch_id' => $b->id]) }}" class="px-3 md:px-6 py-2 md:py-4 bg-slate-900 text-white rounded-xl md:rounded-2xl text-[8px] md:text-[10px] font-black uppercase shadow-lg shadow-slate-200 hover:bg-emerald-600 transition-all inline-flex items-center gap-1 md:gap-2 italic no-underline whitespace-nowrap">
                                                    <i class="fa-solid fa-plus text-emerald-400"></i> <span class="hidden sm:inline">{{ __("Nouveau Passage") }}</span><span class="sm:hidden">{{ __("Collecter") }}</span>
                                                </a>
                                            @elseif($isGraded)
                                                <span class="px-3 md:px-6 py-2 md:py-4 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-xl md:rounded-2xl text-[8px] md:text-[10px] font-black uppercase shadow-sm inline-flex items-center gap-1 md:gap-2 italic cursor-default whitespace-nowrap">
                                                    <i class="fa-solid fa-check-double"></i> <span class="hidden sm:inline">{{ __("Clôturé") }}</span>
                                                </span>
                                            @endif
                                        @endcan
                                    </td>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10 text-left">
                ...
                <div class="bg-white rounded-[2.5rem] md:rounded-[3.5rem] border border-slate-100 shadow-xl shadow-slate-200/30 overflow-hidden relative">
                    <div class="p-6 md:p-8 border-b border-slate-50 bg-slate-50/20 flex flex-wrap gap-2 justify-between items-center italic">
                        <h4 class="text-xs md:text-sm font-black uppercase tracking-widest text-slate-800 leading-none italic m-0">📅 {{ __("Entrées Brutes") }}</h4>
                        <span class="px-2 py-1 bg-emerald-100 text-emerald-600 rounded-lg text-[7px] md:text-[8px] font-black uppercase italic">{{ __("Flux Entrant") }}</span>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse min-w-[400px] md:min-w-0">
                            <tbody class="divide-y divide-slate-50 text-[9px] md:text-[10px]">
                                @forelse($recentProds as $prod)
                                    <tr class="hover:bg-slate-50 transition-colors font-bold italic">
                                        ...
                                        <td class="px-2 md:px-4 py-4 md:py-5 text-center">
                                            @if($prod->is_graded)
                                                <span class="bg-emerald-50 text-emerald-600 px-2 md:px-3 py-1 md:py-1.5 rounded-lg md:rounded-xl text-[7px] md:text-[8px] font-black border border-emerald-100 italic">{{ __("TRIÉ") }}</span>
                                            @else
                                                <span class="bg-orange-50 text-orange-400 px-2 md:px-3 py-1 md:py-1.5 rounded-lg md:rounded-xl text-[7px] md:text-[8px] font-black border border-orange-100 animate-pulse italic">{{ __("À TRIER") }}</span>
                                            @endif
                                        </td>
                                ...
                                @empty
                                    <tr><td colspan="4" class="p-8 md:p-12 text-center text-slate-300 italic uppercase text-[9px] md:text-[10px] tracking-widest border-none">{{ __("Aucune production") }}</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="bg-white rounded-[2.5rem] md:rounded-[3.5rem] border border-slate-100 shadow-xl shadow-slate-200/30 overflow-hidden italic">
                    <div class="p-6 md:p-8 border-b border-slate-50 bg-slate-50/20 flex flex-wrap gap-2 justify-between items-center italic">
                        <h4 class="text-xs md:text-sm font-black uppercase tracking-widest text-slate-800 leading-none italic m-0">📤 {{ __("Sorties Magasin") }}</h4>
                        <span class="px-2 py-1 bg-red-100 text-red-600 rounded-lg text-[7px] md:text-[8px] font-black uppercase italic">{{ __("Flux Sortant") }}</span>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse min-w-[350px] md:min-w-0">
                            <tbody class="divide-y divide-slate-50 text-[9px] md:text-[10px]">
                                @forelse($recentMovements as $mov)
                                    <tr class="hover:bg-slate-50 transition-colors font-bold italic">
                                        ...
                                        <td class="px-4 py-4 md:py-5 text-center text-red-600 font-black text-[10px] md:text-xs italic">
                                            ...
                                            <div class="text-[7px] md:text-[8px] text-slate-400 mt-1">{{ number_format($mov->quantity, 1) }} {{ __("Alv.") }}</div>
                                        </td>
                                ...
                                @empty
                                    <tr><td colspan="3" class="p-8 md:p-12 text-center text-slate-300 italic uppercase text-[9px] md:text-[10px] tracking-widest border-none">{{ __("Aucune sortie") }}</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
