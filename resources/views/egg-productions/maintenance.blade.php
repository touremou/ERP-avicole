            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h2 class="text-xl font-black text-slate-800 uppercase italic tracking-tighter leading-none m-0">
                        🛠️ Maintenance & Inventaire Magasin
                    </h2>
                    <p class="text-[9px] font-black text-red-500 uppercase tracking-[0.2em] mt-1 italic leading-none animate-pulse m-0">
                        Accès Niveau : Super-Admin (S)
                    </p>
                </div>
            </div>
            <div class="hidden md:block">
                <span class="px-4 py-2 bg-slate-100 text-slate-400 rounded-xl text-[8px] font-black uppercase italic tracking-widest border border-slate-200">Session d'Audit : {{ now()->format('d/m/Y') }}</span>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-12 italic font-bold">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white p-10 rounded-[3rem] shadow-2xl border border-red-50 relative overflow-hidden">
                ...
                <div class="mb-10 p-8 bg-red-600 rounded-[2.5rem] text-white shadow-xl shadow-red-200 relative overflow-hidden text-left italic">
                    <div class="relative z-10">
                        <h4 class="text-sm font-black uppercase tracking-widest mb-2 leading-none m-0">⚠️ Procédure de Rebasage de Stock</h4>
                        <p class="text-[10px] font-bold opacity-90 leading-relaxed uppercase tracking-tight m-0 mt-2">
                            L'enregistrement de ce formulaire **écrasera définitivement** les quantités calculées par l'ERP pour les remplacer par vos valeurs physiques. 
                            Cette opération sera logguée comme "Ajustement Critique" dans le journal de maintenance.
                        </p>
                    </div>
                ...
                <form action="{{ route('stocks.rebase') }}" method="POST" class="space-y-6">
                        ...
                        <div class="py-6 flex items-center justify-between group transition-all hover:bg-slate-50/50 px-4 rounded-2xl">
                            <div class="text-left">
                                ...
                                <div class="flex items-center gap-3 mt-2">
                                    <span class="text-[9px] text-slate-400 uppercase font-black italic bg-slate-100 px-2 py-0.5 rounded">Actuel : {{ number_format($s->current_quantity, 2) }}</span>
                                    <i class="fa-solid fa-right-long text-[8px] text-slate-300"></i>
                                    <span class="text-[9px] text-red-500 uppercase font-black italic">Nouvelle Valeur</span>
                                </div>
                            </div>
                    ...
                    <div class="pt-10 space-y-4">
                        <button type="submit" 
                                onclick="return confirm('🚨 ACTION CRITIQUE : Confirmez-vous l\'alignement du stock système sur le stock physique ?')"
                            ...
                            <span class="tracking-[0.2em] text-xs">Écraser & Synchroniser l'Inventaire</span>
                        ...
                        <a href="{{ route('egg-productions.index') }}" class="block text-center text-[10px] text-slate-400 uppercase tracking-widest hover:text-slate-800 transition-colors italic no-underline font-black">
                            <i class="fa-solid fa-chevron-left mr-1"></i> Abandonner la maintenance
                        </a>
                    </div>
                </form>
            </div>
            ...
            <div class="bg-white p-20 rounded-[4rem] border border-slate-100 shadow-2xl text-center italic">
                ...
                <h3 class="text-2xl font-black text-slate-900 uppercase italic mb-4 tracking-tighter leading-none">Sécurité Système Violée</h3>
                <p class="text-slate-400 text-[11px] font-black uppercase tracking-widest italic max-w-sm mx-auto leading-relaxed">
                    Cette interface de rebasage est verrouillée. Seul un profil disposant de la permission **Maintenance (S)** peut modifier les registres physiques de stock.
                </p>
