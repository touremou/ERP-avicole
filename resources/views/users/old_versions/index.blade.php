<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 text-left">
            <div>
                <h2 class="text-2xl font-black text-slate-800 tracking-tighter uppercase italic leading-none">Gestion des Accès & Rôles</h2>
                <p class="text-slate-500 font-bold text-[10px] uppercase tracking-[0.2em] mt-2 italic">RBAC par Module — Matrice des privilèges industrielle</p>
            </div>
            <div class="flex flex-wrap gap-3">
                <button onclick="document.getElementById('moduleMatrixModal').classList.remove('hidden')"
                    ...
                    <i class="fas fa-th mr-2"></i> Matrice Modules
                </button>
                <button onclick="document.getElementById('roleConfigModal').classList.remove('hidden')"
                    ...
                    <i class="fas fa-shield-halved mr-2 text-blue-500"></i> Rôles & LCMS
                </button>
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-slate-50 text-[8px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 italic">
                            <th class="px-8 py-5 text-left">Utilisateur</th>
                            <th class="px-6 py-5 text-left">Rôle</th>
                            <th class="px-6 py-5 text-center">Permissions globales</th>
                            <th class="px-6 py-5 text-center">Modules accessibles</th>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-all group">
                            ...
                            <td class="px-6 py-5 text-center">
                                ...
                                <p class="text-[7px] text-slate-300 uppercase">modules</p>
                            </td>
                            <td class="px-8 py-5 text-right">
                                @if(auth()->id() !== $user->id)
                                <form action="{{ route('users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Supprimer cet accès ?')">
                                    @csrf @method('DELETE')
                                ...
                                @else
                                    <span class="px-3 py-1 bg-slate-100 rounded-lg text-[7px] font-black text-slate-400 uppercase italic">Moi</span>
                                @endif
                            </td>
                        </tr>
                        @endforeach
    ...
    <div id="moduleMatrixModal" class="hidden fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-[3rem] w-full max-w-6xl shadow-2xl overflow-hidden text-left italic font-bold max-h-[90vh] flex flex-col">
            <div class="px-10 py-8 border-b border-slate-100 flex justify-between items-center shrink-0">
                <div>
                    <h3 class="text-2xl font-black text-slate-800 uppercase tracking-tighter italic leading-none">Matrice des Modules</h3>
                    <p class="text-[9px] text-slate-400 uppercase tracking-widest mt-2">Permissions L/C/M/S par module et par rôle</p>
                </div>
                ...
                <div class="px-10 py-6 overflow-x-auto">
                    <table class="w-full border-collapse min-w-[800px]">
                        <thead class="sticky top-0 bg-white z-10">
                            <tr>
                                <th class="text-left px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest w-40">Module</th>
                                @foreach($roles as $role)
                                <th class="text-center px-2 py-3 min-w-[120px]">
                                    ...
                                    <p class="text-[7px] text-slate-300">{{ $role->users_count }} membres</p>
                                </th>
                                @endforeach
                            </tr>
                        </thead>
                            ...
                            <tr class="hover:bg-slate-50/50">
                                ...
                                <td class="px-2 py-3">
                                    <div class="flex justify-center gap-1">
                                        @foreach(['L' => 'emerald', 'C' => 'orange', 'M' => 'blue', 'S' => 'rose'] as $level => $color)
                ...
                <div class="px-10 py-6 border-t border-slate-100 bg-slate-50 shrink-0">
                    <div class="flex justify-between items-center">
                        <div class="flex gap-4">
                            <span class="text-[8px] font-black uppercase text-slate-400 flex items-center gap-1"><span class="w-4 h-4 bg-emerald-500 rounded text-white text-[8px] flex items-center justify-center">L</span> Lecture</span>
                            <span class="text-[8px] font-black uppercase text-slate-400 flex items-center gap-1"><span class="w-4 h-4 bg-orange-500 rounded text-white text-[8px] flex items-center justify-center">C</span> Création</span>
                            <span class="text-[8px] font-black uppercase text-slate-400 flex items-center gap-1"><span class="w-4 h-4 bg-blue-500 rounded text-white text-[8px] flex items-center justify-center">M</span> Modification</span>
                            <span class="text-[8px] font-black uppercase text-slate-400 flex items-center gap-1"><span class="w-4 h-4 bg-rose-500 rounded text-white text-[8px] flex items-center justify-center">S</span> Suppression</span>
                        </div>
                        <button type="submit" class="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition-all border-none cursor-pointer shadow-xl italic">
                            <i class="fas fa-save mr-2"></i> Appliquer la Matrice
                        </button>
                    </div>
                </div>
            </form>
    ...
    <div class="bg-white rounded-[3rem] w-full max-w-2xl shadow-2xl overflow-hidden italic font-bold text-left max-h-[85vh] flex flex-col">
        <div class="px-10 py-8 border-b border-slate-100 flex justify-between items-center shrink-0">
            <h3 class="text-xl font-black text-slate-800 uppercase tracking-tighter italic leading-none">Rôles & LCMS Global</h3>
        ...
        <div class="flex-1 overflow-auto p-10">
            ...
            <form action="{{ route('roles.store') }}" method="POST" class="mb-8 p-6 bg-slate-50 rounded-2xl">
                @csrf
                <p class="text-[9px] font-black uppercase text-blue-500 mb-4 tracking-widest">Nouveau grade</p>
                <div class="flex gap-3">
                    ...
                    <input type="text" name="display_name" placeholder="NOM DU RÔLE" required class="flex-1 bg-white border-none rounded-xl p-3 text-xs font-black uppercase shadow-sm outline-none">
                    <button type="submit" class="bg-slate-900 text-white px-6 rounded-xl font-black text-[9px] uppercase hover:bg-blue-600 transition-all border-none cursor-pointer italic">Créer</button>
                </div>
            </form>
                ...
                <div class="p-6 bg-white rounded-2xl border border-slate-100 shadow-sm transition-all hover:border-slate-300">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            @if($role->users_count == 0)
                                <button type="submit" form="delete-role-{{ $role->id }}" onclick="return confirm('Confirmer la suppression définitive de ce rôle ?')" 
                                        class="w-6 h-6 flex items-center justify-center bg-red-50 text-red-500 hover:bg-red-500 hover:text-white rounded-lg transition-all cursor-pointer border-none outline-none" title="Supprimer ce rôle vide">
                    ...
                    <div class="flex flex-wrap gap-4">
                        @foreach(['L' => 'Lecture', 'C' => 'Création', 'M' => 'Modification', 'S' => 'Suppression'] as $key => $label)
    ...
    <div id="userModal" class="hidden fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-50 flex items-center justify-center p-6">
        <div class="bg-white rounded-[3rem] w-full max-w-xl shadow-2xl overflow-hidden text-left italic font-bold">
            <div class="px-10 py-8 border-b border-slate-100 flex justify-between items-center">
                <h3 class="text-xl font-black text-slate-800 uppercase tracking-tighter italic leading-none">Nouvel Utilisateur</h3>
            ...
            <div class="p-10">
                <form action="{{ route('users.store') }}" method="POST" class="space-y-6">
                    @csrf
                    <div class="space-y-2">
                        <label class="text-[9px] font-black uppercase text-slate-400 ml-2 tracking-widest">Nom</label>
                    ...
                    <div class="space-y-2">
                        <label class="text-[9px] font-black uppercase text-slate-400 ml-2 tracking-widest">Email</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 ml-2 tracking-widest">Mot de passe</label>
                        ...
                        <div class="space-y-2">
                            <label class="text-[9px] font-black uppercase text-slate-400 ml-2 tracking-widest">Confirmation</label>
                    ...
                    <div class="space-y-2">
                        <label class="text-[9px] font-black uppercase text-slate-400 ml-2 tracking-widest">Rôle</label>
                    ...
                    <button type="submit" class="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-xs uppercase tracking-[0.3em] hover:bg-blue-600 transition-all border-none cursor-pointer italic">
                        <i class="fas fa-user-plus mr-2"></i> Créer l'accès
                    </button>
                </form>
            </div>
        </div>
