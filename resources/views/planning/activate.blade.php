        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-indigo-50 p-8 rounded-[3rem] border border-indigo-200 mb-8">
                <h3 class="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <i class="fa-solid fa-calendar-check"></i> {{ __("Planification d'origine") }}
                </h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="bg-white p-4 rounded-2xl text-center">
                        <p class="text-[8px] font-black text-slate-400 uppercase">{{ __("Type") }}</p>
                        <p class="text-sm font-black text-slate-900 uppercase">{{ $plan->batch_type }}</p>
                    </div>
                    <div class="bg-white p-4 rounded-2xl text-center">
                        <p class="text-[8px] font-black text-slate-400 uppercase">{{ __("Quantité prévue") }}</p>
                        <p class="text-sm font-black text-emerald-600">{{ number_format($plan->planned_quantity) }}</p>
                    </div>
                    <div class="bg-white p-4 rounded-2xl text-center">
                        <p class="text-[8px] font-black text-slate-400 uppercase">{{ __("Fournisseur") }}</p>
            ...
            <form method="POST" action="{{ route('planning.status', $plan) }}">
                ...
                <div class="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 mb-8">
                    <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-8 italic">{{ __("Données d'arrivée réelles") }}</h3>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <label class="block text-[10px] font-black text-emerald-500 uppercase mb-2 ml-1 italic">{{ __("Quantité vivante reçue") }} *</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-black text-red-500 uppercase mb-2 ml-1 italic">{{ __("Mortalité transport") }}</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-black text-slate-500 uppercase mb-2 ml-1 italic">{{ __("Responsable du lot") }} *</label>
                ...
                <div class="bg-emerald-50 p-6 rounded-[2.5rem] border border-emerald-200 mb-8 flex items-start gap-4">
                    ...
                    <div>
                        <p class="text-[10px] font-black text-emerald-700 uppercase tracking-widest mb-1">{{ __("Création automatique du lot") }}</p>
                        <p class="text-[9px] text-emerald-600 normal-case">{{ __("En validant, un lot réel sera automatiquement créé dans le bâtiment") }} {{ $plan->building->name }} {{ __("avec les données saisies. Le protocole et la souche seront repris de la planification.") }}</p>
                    </div>
                </div>
                ...
                <button type="submit" class="w-full bg-emerald-500 text-white font-black py-8 rounded-[2rem] hover:bg-emerald-600 transition-all uppercase tracking-[0.3em] text-[10px] italic shadow-2xl border-none cursor-pointer">
                    <i class="fas fa-rocket mr-2"></i> {{ __("Activer la bande & Créer le lot") }}
                </button>
            </form>
        </div>
    </div>
