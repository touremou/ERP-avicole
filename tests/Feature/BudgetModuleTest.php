...
test('« copier le mois précédent » reporte les budgets sur le mois courant', function () {
    $target = Carbon::create($this->y, $this->m, 1);
    $prev   = (clone $target)->subMonth();

    Budget::create(['category' => 'carburant', 'year' => $prev->year, 'month' => $prev->month, 'amount' => 300000]);

    $this->actingAs($this->manager)
        ->post(route('budgets.copy-previous'), ['year' => $this->y, 'month' => $this->m])
        ->assertRedirect();

    expect((float) Budget::where('category', 'carburant')->forPeriod($this->y, $this->m)->value('amount'))->toBe(300000.0);
});

test('la vue annuelle cumule les budgets des 12 mois et les dépenses de l\'année', function () {
    Budget::create(['category' => 'carburant', 'year' => $this->y, 'month' => 1, 'amount' => 100000]);
    Budget::create(['category' => 'carburant', 'year' => $this->y, 'month' => 2, 'amount' => 150000]);

    Expense::factory()->create([
        'farm_id' => $this->farm->id, 'user_id' => $this->manager->id,
        'category' => 'carburant', 'status' => 'valide', 'amount' => 80000,
        'expense_date' => Carbon::create($this->y, 3, 10)->toDateString(),
    ]);

    $this->actingAs($this->manager)
        ->get(route('budgets.index', ['year' => $this->y, 'mode' => 'year']))
        ->assertOk()
        ->assertSee('250 000', false)  // budget annuel cumulé (100k + 150k)
        ->assertSee('80 000', false);  // dépensé sur l'année
});

test('l\'export PDF du récap renvoie bien un PDF', function () {
    Budget::create(['category' => 'carburant', 'year' => $this->y, 'month' => $this->m, 'amount' => 500000]);

    $resp = $this->actingAs($this->manager)
        ->get(route('budgets.export-pdf', ['year' => $this->y, 'month' => $this->m]))
        ->assertOk();
