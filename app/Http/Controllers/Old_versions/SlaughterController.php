...
class SlaughterController extends Controller
{
    ...
    public function dashboard(SlaughterService $service)
    {
        ...
        $recentResults = SlaughterOrder::where('status', 'termine')
            ->with(['batch', 'result'])->latest('actual_date')->take(10)->get();
    ...
    public function createOrder()
    {
        ...
        // Lots éligibles à l'abattage :
        // - Chair : tous les lots actifs
        // - Ponte / Reproducteur : lots actifs (réforme)
    ...
    public function storeOrder(Request $request)
    {
        ...
        // Vérifier que le lot a assez de sujets
    ...
    public function storeTransformation(Request $request, SlaughterService $service)
    {
        ...
        // Vérifier le stock source côté backend
    ...
    /**
     * Mise à jour d'un produit fini (prix, péremption, seuil alerte).
     */
    ...
    /**
     * Transférer un produit fini vers le stock magasin (table stocks).
     */
    public function transferToStock(Request $request, FinishedProduct $product)
    {
        ...
        // Créer/mettre à jour dans la table stocks (avec farm_id)
        $stockData = [
            'item_name'        => $product->product_name,
            'category'         => 'produits_finis',
        ];

        // Ajouter farm_id si la colonne existe
        ...
        // Déduire du stock produits finis
    ...
    /**
     * Ajustement d'inventaire (correction manuelle).
     */
    ...
    /**
     * Éliminer un produit (périmé, avarié).
     */
